...
// Type Comprehensive Reviews Analysis" run modals + browser loop. ONE
...
export type TypeFlow = 'per-type-bulleted' | 'per-type-nonbulleted';

// One category the run iterates over.
export interface TypeRunTarget {
  typeKey: string;
  label: string;
  urlIds: string[];
}

export interface TypeAiRunModalProps {
  projectId: string;
  flow: TypeFlow;
  categories: ReadonlyArray<TypeRunTarget>;
  // For the non-bulleted flow only: typeKey → that category's bulleted
  // summary text. A category missing here (or empty) is skipped + flagged.
  bulletedSummaryByTypeKey?: Record<string, string>;
  onClose: () => void;
...
const TITLES: Record<TypeFlow, string> = {
  'per-type-bulleted':
...
const BLURBS: Record<TypeFlow, string> = {
  'per-type-bulleted':
    'For each type, merges its competitors’ bulleted critiques into one deduplicated type critique list — and traces each type bullet back to its source reviews. One type at a time; the Type (bulleted) cell + Source Reviews fill in live.',
  'per-type-nonbulleted':
    'Rewrites each type’s bulleted critique summary into flowing prose, one type at a time. Types without a bulleted summary yet are skipped and listed below — run the bulleted flow on them first.',
};

export function TypeAiRunModal({
  projectId,
  flow,
  categories,
  bulletedSummaryByTypeKey,
  onClose,
  onResult,
  ...
  const [results, setResults] = useState<PerTypeResult[]>(() =>
    categories.map((c) => ({
      typeKey: c.typeKey,
      label: c.label,
      status: 'pending',
  ...
  const isNonBulleted = flow === 'per-type-nonbulleted';
  const hasBulleted = (key: string) =>
    ((bulletedSummaryByTypeKey ?? {})[key] ?? '').trim().length > 0;
  const runnableCount = isNonBulleted
    ? categories.filter((c) => hasBulleted(c.typeKey)).length
    : categories.length;
  ...
  function updateOneResult(index: number, partial: Partial<PerTypeResult>) {
    setResults((prev) =>
  ...
  async function handleStart() {
    ...
    for (let i = 0; i < categories.length; i++) {
      ...
      const bulletedSummary = (
        (bulletedSummaryByTypeKey ?? {})[cat.typeKey] ?? ''
      ).trim();
      ...
      let body: {
        flow: TypeFlow;
        // The handler echoes the group key back under `categoryKey` for BOTH
        // the per-category and per-type flows (generic group-key wire field).
        categoryKey: string;
        analysisId: string;
        summary: string;
      ...
      // group key matches the type we dispatched — never paint into the
      // wrong cell.
      if (body.categoryKey === cat.typeKey) {
        onResult({
          typeKey: cat.typeKey,
      ...
      : runState.kind === 'completed'
        ...
        : runState.kind === 'cancelled'
          ...
          : runState.kind === 'error'
            ...
            : `${runnableCount} type${runnableCount === 1 ? "" : "s"} will run${
  ...
  return (
    <div
      role="dialog"
      aria-modal="true"
      aria-labelledby="type-ai-modal-title"
      style={{
        position: 'fixed',
        inset: 0,
    ...
    >
      ...
      >
        ...
        {runState.kind === 'completed' && (
          ...
          >
            <div style={{ fontWeight: 700, marginBottom: '4px' }}>
              ✅ Type analysis job complete
            </div>
            <div style={{ fontSize: '12px', color: '#c9d1d9' }}>
              {tally.cacheHits + tally.freshCalls} of {total} types done
        ...
        >
          <label htmlFor="type-ai-model">Model</label>
          <select
            id="type-ai-model"
            value={modelVersion}
          ...
          <label htmlFor="type-ai-execmode">Execution mode</label>
          <ExecutionModeSelect
            id="type-ai-execmode"
            value={executionMode}
            onChange={setExecutionMode}
            disabled={isRunning || isDone}
            className="type-ai-execmode"
          />
        </div>
        ...
        {(isRunning || isDone) && (
          ...
          >
            {results.map((r, i) => (
              <div
                key={r.typeKey || '__uncategorized__'}
                style={{
                  display: 'grid',
        ...
        <div style={{ display: 'flex', gap: '10px', justifyContent: 'flex-end' }}>
          {!isRunning && !isDone && (
            <>
              ...
              >
                Start ({runnableCount} type{runnableCount === 1 ? '' : 's'})
              </button>
            </>
          )}
          {isRunning && (
...
function statusLabel(
  status: PerTypeResult['status'],
  source: PerTypeResult['source'],
...
function statusLabelStyle(status: PerTypeResult['status']): React.CSSProperties {
  const baseColor =
    status === 'done'
      ? '#3fb950'
      : status === 'error'
