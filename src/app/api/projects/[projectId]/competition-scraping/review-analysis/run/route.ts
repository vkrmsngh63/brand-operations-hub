...
import {
  ...
  type Ctx,
  type VerifyAuthFn,
} from '@/lib/competition-scraping/handlers/review-analysis-run';
import { getAnthropicClient } from '@/lib/competition-scraping/review-analysis/client';

// W#2 P-49 Workstream 5 Session 1 — AI per-product review analysis (POST).
...
// production shim that wires the prod Anthropic client + Prisma + auth.
...
export async function POST(req: NextRequest, ctx: Ctx) {
  // Anthropic client is constructed lazily at first request so the route
  // module can be imported in environments without ANTHROPIC_API_KEY set
  // (e.g. type-check, tests). Errors at construction time surface as 500
  // — they only happen when the env var is missing.
  let anthropicClient;
  try {
    anthropicClient = await getAnthropicClient();
  } catch (error) {
    console.error('[review-analysis-run] Anthropic client init failed:', error);
    return withCors(
      req,
      NextResponse.json(
  ...
  const inner = makeReviewAnalysisRunHandlers({
    prisma,
