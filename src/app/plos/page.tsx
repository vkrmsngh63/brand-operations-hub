...
- Error and failure monitoring`,
  ...
  },
  {
    ...
    shortDesc:
      "Manage day-to-day operations of launched products — inventory, fulfillment, customer service, financials, vendor relationships, and operational workflows.",
    longDesc: `## Inventory & Supply Chain
- Stock level monitoring across all channels
- Reorder point alerts and automated purchase orders
- Supplier relationship management
- Fulfillment center coordination (FBA, 3PL, in-house)

## Customer Service
- Support ticket management and escalation workflows
- Returns and refund processing
- Customer communication templates
- Satisfaction tracking and NPS monitoring

## Financial Operations
- Revenue tracking per product, channel, and brand
- Cost of goods sold (COGS) management
- Profit margin analysis per SKU
...
- Task automation and recurring task scheduling
...
function LongDescRenderer({ text, bright }: { text: string; bright?: boolean }) {
  ...
  for (const line of lines) {
    ...
    if (trimmed.startsWith("## ")) {
      ...
      elements.push(
        <div
          key={key++}
          style={{ fontSize: "11px", color: bright ? "#475569" : "#8b949e", lineHeight: 1.5 }}
        >
          {trimmed}
        </div>
      );
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          {cards.filter((c) => !c.standalone).map((card) => {
            ...
            return (
              <div
                ...
                style={{
                  ...
                  boxShadow: isHovered ? "0 10px 30px rgba(59,130,246,.2)" : "none",
                }}
              >
                {/* Edit button */}
                <button
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <span
                    style={{
                      ...
                      flexShrink: 0,
                      background: isActive
                        ? (isHovered ? "rgba(31,111,235,.15)" : "rgba(31,111,235,.2)")
                        : (isHovered ? "rgba(100,116,139,.15)" : "rgba(139,148,158,.15)"),
                      color: isActive
                        ? (isHovered ? "#1d4ed8" : "#58a6ff")
                        : (isHovered ? "#475569" : "#8b949e"),
                    }}
                  >
                  ...
                  <>
                    ...
                    <div
                      style={{
                        ...
                        borderTop: isExpanded ? `1px solid ${isHovered ? "#cbd5e1" : "#21262d"}` : "1px solid transparent",
                        paddingTop: isExpanded ? "10px" : "0",
                        marginTop: isExpanded ? "2px" : "0",
                      }}
                    >
                      <LongDescRenderer text={card.longDesc} bright={isHovered} />
                    </div>
                  </>
                )}
              </div>
        ...
        {/* ── Standalone Cards (Business Operations) ── */}
        {cards.filter((c) => c.standalone).map((card) => {
          const isHovered = hovered === card.id;
          const isActive = card.badge === "active";
          const isExpanded = expandedCards.has(card.id);
          ...
          return (
            <div key={card.id}>
              ...
              <div
                ...
                style={{
                  position: "relative",
                  background: isHovered ? "#f0f6ff" : "#1a2332",
                  border: `1px solid ${isHovered ? "#3b82f6" : "rgba(31,111,235,.27)"}`,
                  borderRadius: "10px",
                  padding: "24px 22px",
                  cursor: isActive ? "pointer" : "not-allowed",
              ...
              >
                ...
                <div style={{ display: "flex", alignItems: "center", gap: "10px" }}>
                  ...
                  <span style={{ fontSize: "14px", fontWeight: 700, color: isHovered ? "#0f172a" : "#e6edf3" }}>
                    {card.title}
                  </span>
                  <span
                    style={{
                ...
                {/* Short Description */}
                <div style={{ fontSize: "11.5px", color: isHovered ? "#475569" : "#8b949e", lineHeight: 1.55 }}>
                  {card.shortDesc}
                </div>
