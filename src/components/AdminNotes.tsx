...
function RichTextToolbar({ onCommand, onSetFontSize, onSetForeColor, onSaveSelection }: {
...
export default function AdminNotes({ system, systemLabel, systemIcon, backRoute }: AdminNotesProps) {
  ...
  // View mode + combined sort order (server-persisted)
  const [viewMode, setViewMode] = useState<"individual" | "combined">("individual");
  const [combinedSort, setCombinedSort] = useState<"newest" | "oldest">("newest");
  ...
  async function fetchViewPrefs() {
    try {
      const [viewRes, sortRes] = await Promise.all([
        authFetch(`/api/user-preferences/adminnotes_view_mode_${system}`),
        authFetch(`/api/user-preferences/adminnotes_combined_sort_${system}`),
      ]);
      if (viewRes.ok) {
        const { value } = await viewRes.json();
        if (value === "individual" || value === "combined") setViewMode(value);
      }
      if (sortRes.ok) {
        const { value } = await sortRes.json();
        if (value === "newest" || value === "oldest") setCombinedSort(value);
      }
    } catch (err) {
  ...
  async function changeViewMode(mode: "individual" | "combined") {
    setViewMode(mode);
    try {
      await authFetch(`/api/user-preferences/adminnotes_view_mode_${system}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  async function changeCombinedSort(order: "newest" | "oldest") {
    setCombinedSort(order);
    try {
      await authFetch(`/api/user-preferences/adminnotes_combined_sort_${system}`, {
        method: "PUT",
        headers: { "Content-Type": "application/json" },
  ...
  function execCommand(cmd: string, value?: string) {
    ...
    // Restore the selection that was active when the toolbar was clicked
    ...
    // If we just created a link, ensure it opens in a new tab
  ...
  /* ── EDITOR CLICK: open links in new tab ── */
  ...
  return (
    ...
    >
          ...
          >
            <button
              ...
              style={{
                ...
                background: viewMode === "individual" ? "#1f6feb" : "transparent",
                color: viewMode === "individual" ? "#fff" : "#8b949e",
                fontFamily: "inherit", display: "flex", alignItems: "center", gap: "5px",
                transition: "all .15s",
              }}
              title="Show one note at a time with sidebar list"
            >
              📄 Individual
            </button>
            <button
              onClick={() => changeViewMode("combined")}
              style={{
                padding: "6px 12px", fontSize: "11px", fontWeight: 600,
                borderLeft: "1px solid #30363d", border: "none", cursor: "pointer",
                background: viewMode === "combined" ? "#1f6feb" : "transparent",
                color: viewMode === "combined" ? "#fff" : "#8b949e",
                fontFamily: "inherit", display: "flex", alignItems: "center", gap: "5px",
                transition: "all .15s",
              }}
              title="Show all notes one after another"
            >
              📚 Combined
            </button>
          </div>

          {/* Sort dropdown - only in Combined view */}
          {viewMode === "combined" && (
            <select
              value={combinedSort}
              onChange={(e) => changeCombinedSort(e.target.value as "newest" | "oldest")}
              style={{
                padding: "6px 10px", fontSize: "11px", fontWeight: 600,
                background: "#0d1117", border: "1px solid #30363d",
                borderRadius: "6px", color: "#c9d1d9",
                cursor: "pointer", fontFamily: "inherit", outline: "none",
              }}
              title="Sort order"
            >
              <option value="newest">🕒 Newest → Oldest</option>
              <option value="oldest">🕰️ Oldest → Newest</option>
            </select>
          )}
      ...
      <div style={{ flex: 1, display: "flex", minHeight: 0, overflow: "hidden" }}>
        ...
        /* ── COMBINED VIEW ── */
        <div style={{ flex: 1, overflow: "auto", background: "#f3f4f6", padding: "24px 40px 64px" }}>
          {notes.length === 0 ? (
            <div
              style={{
                maxWidth: "820px", margin: "48px auto", padding: "56px 32px",
                textAlign: "center", background: "#fff",
                border: "1px solid #e5e7eb", borderRadius: "12px",
                color: "#6b7280",
              }}
            >
              <span style={{ fontSize: "48px", opacity: 0.5, display: "block", marginBottom: "12px" }}>📝</span>
              <p style={{ fontSize: "14px" }}>
                No notes yet. Switch to Individual view and click &quot;+ New Note&quot; to create one.
              </p>
            </div>
          ) : (
            <div style={{ maxWidth: "820px", margin: "0 auto" }}>
              ...
              >
                ...
                {combinedSort === "newest" ? "Newest first" : "Oldest first"}
              </div>
              ...
              {[...notes]
                .sort((a, b) => {
                  ...
                  return combinedSort === "newest" ? tB - tA : tA - tB;
                })
                .map((note) => (
                  <div
                    key={note.id}
                  ...
                  >
                    ...
                    >
                      ...
                      <button
                        ...
                        title="Open in Individual view to edit"
                        style={{
                          padding: "6px 12px", fontSize: "11px", fontWeight: 600,
                          background: "#fff", border: "1px solid #d1d5db",
                          borderRadius: "6px", color: "#374151",
                    ...
                    {note.content ? (
                      <div
                        className="combined-note-content"
                        style={{
                          fontSize: "14px", lineHeight: 1.65, color: "#111827",
                        }}
                        onClick={(e) => {
                          // Make links inside rendered content clickable
                          let el = e.target as HTMLElement | null;
                    ...
                    {note.attachments.length > 0 && (
                      <div style={{ marginTop: "20px", paddingTop: "16px", borderTop: "1px solid #f3f4f6" }}>
                        <div style={{
                          ...
                          letterSpacing: "1.3px", textTransform: "uppercase", marginBottom: "10px",
                        }}>
                          Attachments
                        </div>
                        <div
                          ...
                          {note.attachments.map((att) => (
                            ...
                            >
                              <div style={{
                                ...
                                {isImage(att.fileType) ? (
                                  <img src={att.publicUrl} alt={att.fileName} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                                ) : isVideo(att.fileType) ? (
                                  <>
                                    <video src={att.publicUrl} style={{ width: "100%", height: "100%", objectFit: "cover" }} />
                                    <div style={{
                              ...
                              <div style={{ padding: "6px 8px", fontSize: "10px", color: "#4b5563", overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>
                                {att.fileName}
                              </div>
                            </div>
                          ))}
      ...
      <style jsx global>{`
        ...
        .combined-note-content a {
          color: #2563eb;
          text-decoration: underline;
          cursor: pointer;
        }
        .combined-note-content a:hover {
          color: #1d4ed8;
        }
        .combined-note-content ul {
          padding-left: 1.8em;
          margin: 0.5em 0;
          list-style-type: disc !important;
        }
        .combined-note-content ol {
          padding-left: 1.8em;
          margin: 0.5em 0;
          list-style-type: decimal !important;
        }
        .combined-note-content ul li,
        .combined-note-content ol li {
          display: list-item !important;
          list-style-position: outside !important;
        }
        .combined-note-content ul ul {
          list-style-type: circle !important;
        }
        .combined-note-content ul ul ul {
          list-style-type: square !important;
        }
      `}</style>
    </div>
