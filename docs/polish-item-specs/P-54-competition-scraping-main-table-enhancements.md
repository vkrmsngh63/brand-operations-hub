...
- **Phase 3 (column reorder + shared persistence) — ✅ BUILT 2026-06-01-b (pending deploy):** R5 per D6. NEW `ProjectTablePreferences` model (additive; SCHEMA-CHANGE-IN-FLIGHT YES → flips NO at the deploy `prisma db push`) + new handler `project-table-preferences.ts` (seed-on-read from UserTablePreferences) + new route + new pure helper `column-order.ts` (`applyColumnOrder` + `moveColumnKey`, +11 node:test) + viewer repointed to the shared endpoint + `columnOrder` state/handler + UrlTable applies the order and adds header drag-to-reorder (`SortableHeaderCell` w/ grip, horizontal SortableContext sharing the one DndContext, id-discriminated drop). Scoreboard: src/lib 1249/1249 (+11), build 71 routes (+1 endpoint), root+ext tsc clean, ext 910/910.
