...
**2026-05-29-d (Fix Session C "Deploy 2" — drag-to-reorder ✅ DEPLOYED-AND-VERIFIED end-to-end on vklf.com via `workflow-2-competition-scraping` → `main` — the Reviews Analysis Table page is now COMPLETE / fully §1-compliant):**

- **2 design decisions (both Recommended + chosen) for drag-to-reorder:** (1) **order-sync across both tables = Synced** — URL (competitor) row order reflects across BOTH the Competitor Content Table and this Reviews Analysis Table (honors Q5 → A "reflects everywhere"); (2) **drag affordance = dedicated grip handle**, NOT whole-row grab — because every cell on this table is click-to-edit, a whole-row grab would conflict with editing.
- **CRITICAL Rule 3 (code-wins-over-docs) finding — the spec-named `CompetitorUrl.sortRank` column NEVER existed.** This spec's §3 schema notes + the Q5 → A answer named `CompetitorUrl.sortRank` as the URL-row persistence column, claiming it was "the SAME column used by the Competitor Content Table." Code-truth audit at Deploy 2 planning found: NO `sortRank` field on the `CompetitorUrl` model in `prisma/schema.prisma`; the Competitor Content Table (`UrlTable.tsx`) actually persists URL order via a per-user `UserTablePreferences.rowOrder` string[] array; the Reviews Analysis page prefixes its OTHER table-preferences keys (`reviewsTable:`) to avoid collision, but `rowOrder` is a single shared scalar. **CORRECTION:** URL-row order persists through the SHARED `UserTablePreferences.rowOrder` field (no schema change), which delivers the Q5 → A "reflects everywhere" intent the (never-existing) `CompetitorUrl.sortRank` column was supposed to provide.
- **field-discriminator on the existing reorder endpoint:** review-row order persists via the already-shipped `CapturedReview.sortRankInReviewsTable` column. Rather than add a new route, the existing `reviews-reorder.ts` handler gained an optional `field` param (default `'sortRank'` = back-compat URL-detail-page order; `'sortRankInReviewsTable'` = this page) so the two orders of the SAME `CapturedReview` rows never collide. Route count stayed 68.
- **Build `194e66f` (11 files +743/-29):** NEW pure helper `reviews-table-reorder.ts` (+ `.test`, +17 cases) for the rank assignment / re-normalization; MODIFIED `competitor-reviews-analysis/page.tsx` (the two @dnd-kit drag surfaces + grip handles + rank-field sort), `reviews-reorder.ts` + `.test` (the `field` param), `url-reviews.ts` + `.test`, `reviews-by-id.ts` + `.test`, `captured-reviews-helpers.test.ts`, `shared-types/competition-scraping.ts`.
- **Schema-change-in-flight:** NO entire session (entry NO → exit NO) — the `CapturedReview.sortRankInReviewsTable` column already shipped in Deploy 1; Deploy 2 uses it + the existing `UserTablePreferences.rowOrder` field for URL-row order; no `prisma db push`.
- **TWO NEW reusable PATTERNS:** "Reorder-field discriminator on a shared reorder endpoint" + "Spec-named storage column may not exist — audit before building" (memorialized in CORRECTIONS_LOG §Entry 2026-05-29-d + REVIEWS_PHASE_2_DESIGN.md §B 2026-05-29-d).
...
**Fix Session C — NEW per-competitor non-bulleted flow + Auto-create non-bulleted button + Excel Export + Drag-to-reorder. SPLIT INTO "Deploy 1" (✅ DEPLOYED-AND-VERIFIED 2026-05-29-c) + "Deploy 2" (drag-to-reorder; ✅ DEPLOYED-AND-VERIFIED 2026-05-29-d). FIX SESSION C FULLY CLOSED — the Reviews Analysis Table page is now COMPLETE / fully §1-compliant.**

Scope ("Deploy 1" items 1-7 ✅ DONE 2026-05-29-c; "Deploy 2" items 8/9/10 ✅ DONE — item 8 the schema COLUMN shipped in Deploy 1; the drag CONSUMERS 9/10 shipped in Deploy 2 on 2026-05-29-d):
...
9. ✅ DONE (Deploy 2, 2026-05-29-d) — Drag-to-reorder URL rows. **CORRECTION (Rule 3):** the spec-named `CompetitorUrl.sortRank` column NEVER existed; URL-row order persists via the SHARED per-user `UserTablePreferences.rowOrder` array (the SAME field `UrlTable.tsx` uses → reflects across both pages per Q5 → A; no schema change). Mirrors the `UrlTable.tsx` @dnd-kit pattern with a dedicated grip handle; child review rows move with their parent.
10. ✅ DONE (Deploy 2, 2026-05-29-d) — Drag-to-reorder review rows within a URL using the already-shipped `CapturedReview.sortRankInReviewsTable`. The existing `reviews-reorder.ts` handler gained an optional `field` discriminator (default `'sortRank'` = back-compat URL-detail-page order; `'sortRankInReviewsTable'` = this page) so the two orders never collide — NO new endpoint (route count stayed 68). Rendered rows sort by the rank fields (NULL ranks fall back to capture order); the Excel export follows on-screen order. NEW pure helper `reviews-table-reorder.ts`.
...
Schema-change-in-flight = YES at Deploy 1 (the new column) → FLIPPED YES → NO at the Deploy 1 push `a24e92c`. **Deploy 2 = NO entire session (the column already shipped; URL-row order uses the existing `UserTablePreferences.rowOrder` field — NOT the spec-named, never-existing `CompetitorUrl.sortRank` column; build `194e66f`).**
...
**Fix Sessions A + B + D + FU-1 + FU-2 + Fix C "Deploy 1" + Fix C "Deploy 2" ✅ DEPLOYED-AND-VERIFIED — THE REVIEWS ANALYSIS TABLE PAGE IS COMPLETE / FULLY §1-COMPLIANT as of 2026-05-29-d.** Fix Session D shipped the traceability-table feature ahead of Fix Session C per the 2026-05-31 re-sequence; FU-1 (edit+delete the traceability box) + FU-2 (deleted-reviews sync bug) shipped 2026-05-31-b (build `7d89d75`); Fix Session C "Deploy 1" (non-bulleted prose flow + Excel export + the `CapturedReview.sortRankInReviewsTable` schema column) shipped 2026-05-29-c (build `a24e92c` + 3 FFs); Fix Session C "Deploy 2" (drag-to-reorder URL rows via the shared `UserTablePreferences.rowOrder` field + review rows via the already-shipped `CapturedReview.sortRankInReviewsTable` through the `field`-discriminated reorder handler) shipped 2026-05-29-d (build `194e66f`). **Nothing remains on this page.** Next: Category page corrective rebuild Sessions 1-3 + Type page Sessions 4-5 (per `docs/polish-item-specs/P-49-W5-S4-category-page.md` and `P-49-W5-S5-type-page.md`).

**Full P-49 W5 corrective work ahead:** Fix A ✅ + Fix B ✅ + **Fix D ✅** + **FU-1 ✅** (edit+delete traceability box) + **FU-2 ✅** (deleted-reviews sync bug) + **Fix C "Deploy 1" ✅** (non-bulleted prose flow + Excel + the schema column) + **Fix C "Deploy 2" ✅** (drag-to-reorder) = **Reviews Analysis Table page CLOSED** + Category 1 + Category 2 + Category 3 + Type 4 + Type 5 (the only work remaining in P-49 W5).
