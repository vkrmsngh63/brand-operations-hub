...
- **FU-1 — EDIT + delete entries in the Overall Analysis traceability table (individually + in bulk) on the URL detail page. DESIGN LOCKED (a.110 follow-up session) — see §4 Q11 RESOLVED.** Director request, EXPANDED at session start from delete-only to **edit + delete**; REVERSES Fix Session D's read-only decision. Locked: delete at 3 levels (complaint row / whole category / single source review); click-to-edit on category names + complaint wording; always-visible bulk-select checkboxes + Delete selected + confirm; re-run warns-then-replaces; PATCH trims/edits `analysisJson.categories` + re-derives the flattened `summary`, NEVER touching `CapturedReview`s. **STATUS: building this session.**
