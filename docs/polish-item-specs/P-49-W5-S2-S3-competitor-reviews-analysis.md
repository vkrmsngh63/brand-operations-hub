...
**Status:** SPEC LOCKED at §1 level (verbatim); **Fix Session A SHIPPED-AND-VERIFIED 2026-05-29** (D-1 through D-7) + **Fix Session B SHIPPED-AND-VERIFIED 2026-05-30** (D-8 + D-9 + D-10 bulleted half + D-11 + Q3 schema gap; build `351342a` + FF1 `add00ad`) + **Fix Session D SHIPPED-AND-VERIFIED 2026-05-31** (RE-SEQUENCED ahead of Fix Session C per director's session-start Rule 14f pick — the "Overall Analysis — Captured Reviews" box on the URL detail page is now a read-only 3-column traceability table [Category / Complaint / source captured reviews + star counts] rendered from a STRUCTURED per-competitor analysis shape; build `4db2b5c` + FF1 `889667e`; director PASS table feature "Both worked", FF1 "Pass"). **Fix Session C STILL PENDING** — D-6 drag-to-reorder + D-7 Excel + D-10 non-bulleted half + the NEW per-competitor non-bulleted AI flow + `CapturedReview.sortRankInReviewsTable` schema column + Q8. **2 NEW follow-up items queued ahead of Fix Session C (a.110):** (FU-1) delete entries from the Overall Analysis traceability table individually + in bulk (NEEDS a design chat — reverses the Fix D read-only decision); (FU-2) deleted-reviews sync bug on the `/competitor-reviews-analysis` table (cache-invalidation fix). Q3 + Q9 + Q10 RESOLVED; §4 carries Q8 (Fix Session C) + the NEW FU-1 delete-from-table design question.
...
**2 NEW follow-up items captured this session (queued ahead of Fix Session C as a.110):**
- **FU-1 — delete entries from the Overall Analysis traceability table (individually + in bulk) on the URL detail page.** Director request; REVERSES this session's read-only decision. NEEDS a design chat first (see §4 NEW open question): delete unit (complaint row? whole category? a single source review in Column 3?); individual + bulk-select UX; the delete trims the stored `analysisJson.categories` via a PATCH WITHOUT touching the underlying `CapturedReview`s.
- **FU-2 — deleted-reviews sync bug.** Deleting captured reviews on a competitor's URL detail page does NOT remove them from the `/competitor-reviews-analysis` table for that product. Root cause (traced): the analysis table page lazy-loads each URL's reviews into a `reviewsByUrl` state cache on row-expand (`competitor-reviews-analysis/page.tsx` ~L519-545) and doesn't invalidate/refetch after a cross-page deletion; dependent summary maps (`summaryByReviewId` / `competitorSummaryByUrlId`) likely also need pruning. Cache-invalidation fix.
...
**Fix Sessions A + B + D ✅ DEPLOYED-AND-VERIFIED.** Fix Session D shipped the traceability-table feature ahead of Fix Session C per the 2026-05-31 re-sequence. **Remaining on this page:** the 2 NEW follow-ups (FU-1 delete-from-table + FU-2 deleted-reviews sync bug — queued as a.110 ahead of Fix C) + Fix Session C (non-bulleted flow + Excel + drag + `CapturedReview.sortRankInReviewsTable` schema column). After those, the Reviews Analysis Table page is functionally compliant with §1 verbatim. Then Category page corrective rebuild Sessions 1-3 + Type page Sessions 4-5 (per `docs/polish-item-specs/P-49-W5-S4-category-page.md` and `P-49-W5-S5-type-page.md`).

**Full P-49 W5 corrective work ahead:** Fix A ✅ + Fix B ✅ + **Fix D ✅** (Reviews Analysis Table) + FU-1 (delete-from-table) + FU-2 (deleted-reviews sync bug) + Fix C + Category 1 + Category 2 + Category 3 + Type 4 + Type 5.

---

## §4 — Open questions (still under discussion)

**Q1 through Q7 RESOLVED 2026-05-28-b + Q10 RESOLVED 2026-05-29 at Fix Session A planning + Q3 & Q9 RESOLVED 2026-05-30 at Fix Session B planning + Fix Session D design decisions RESOLVED 2026-05-31 at session start (table-on-top + notes-below / read-only generated table / full review text in source column — folded into §2 + §3 above)** (answers folded into §2 + §3 above). Open questions:

- **Q11 (NEW 2026-05-31; FU-1 delete-from-table design) — delete entries from the Overall Analysis traceability table individually + in bulk on the URL detail page.** This REVERSES the Fix Session D read-only decision, so it needs a Rule 14f design chat at session start per `feedback_plan_output_shape_before_building.md`. Open sub-decisions: (a) DELETE UNIT — what does a delete remove? a single complaint (bullet) row? a whole category group? a single source review in Column 3 (which would re-render the bullet without that review)? (b) INDIVIDUAL + BULK-SELECT UX — per-row delete affordance + a bulk-select (checkbox/multi-select) mode + a confirm step? (c) PERSISTENCE — the delete trims the stored `analysisJson.categories` (remove the bullet/category/reviewRef) via a PATCH to the per-competitor `ReviewAnalysis` row WITHOUT touching the underlying `CapturedReview`s (the reviews stay; only the analysis-derived table entry is trimmed). Decide at start of the (a.110) follow-up session.
