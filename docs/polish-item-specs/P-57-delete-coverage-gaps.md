...
**Status:** ✅ DEPLOYED-AND-VERIFIED 2026-06-02-g (`session_2026-06-02-g_p57-delete-coverage-gaps`) — **P-57 CLOSED.** W#2 Competition Scraping; competitor URL detail page (`url/[urlId]`). NO schema change (deletes use existing models + rows; zero `prisma db push`). _(Captured 2026-06-02-d as ROADMAP entry P-57; this spec doc created 2026-06-02-g as the session's first artifact per Rule 31 — it did not exist before.)_

**As shipped (TWO deploys):** TWO build commits — `9c4b548` (deploy 1) + `418e6ca` (deploy 2/FF1); `main` went `a6081da → 9c4b548 → 418e6ca`; TWO Rule 9 deploy gates (both "Deploy to main"). **Part A — captured-video delete UI (deploy 1):** a per-video trash control + `ConfirmDeleteDialog` plain variant + a NEW `handleVideoDeleted` optimistic-remove handler mirroring the image card (the backend `videos/[videoId]` DELETE already existed since P-27 Build #5). **Part B — category-label deletion (deploy 1 backend + deploy 2 UI):** a NEW pure helper `src/lib/competition-scraping/category-vocabulary.ts` (category-type → model/column/noun mapping + a `categoryDeletionMessage` formatter; +10 node:test) + a NEW route `src/app/api/projects/[projectId]/competition-scraping/categories/route.ts` (GET = usage count for the confirm; DELETE = cascade-delete the `VocabularyEntry` AND every tagged capture row project-wide, best-effort storage cleanup for images/videos; route count 72 → 73). **The Q2 picker-trash → ✕-on-pill correction (deploy 2/FF1):** a reusable `CategoryPill` component (the pill + an inline ✕ → a count-bearing confirm fetched from the categories route → cascade delete → `window.location.reload()`) wired into all three captured-item cards; the deploy-1 `VocabularyPicker` picker-trash was REVERTED so there is exactly ONE category-delete surface (UI-only; route count UNCHANGED at 73). **Verification:** director real-Chrome on vklf.com — deploy 1 "I was able to delete videos" (videos PASS); deploy 2/FF1 "pass" (the ✕-on-pill category delete works incl. the count confirm + the project-wide cascade). **Scoreboard:** extension `npm test` 915 UNCHANGED / src/lib `node:test` 1353 → 1363 / `npm run build` 72 → 73. See CORRECTIONS_LOG §Entry 2026-06-02-g + COMPETITION_SCRAPING_DESIGN §B 2026-06-02-g + COMPETITION_SCRAPING_VERIFICATION_BACKLOG Deploy session #40.
...
- **Q2 — where does the control live + which vocab types? → INITIALLY Option A (picker trash), CORRECTED to "✕ on the category pill" after a post-deploy discoverability finding.**
  - **First decision (deploy 1):** trash in the `VocabularyPicker` dropdown for the three category types. **Audit-shipped-state CORRECTION (Rule 31):** that recommendation rested on a wrong assumption — the `VocabularyPicker` for content/image/video-category is NOT shown inline on the detail page. It only appears inside the "+ Manually add captured text/image" modals (`CapturedTextAddModal` content-category / `CapturedImageAddModal` image-category), and **video-category has NO picker anywhere**. So the picker trash was undiscoverable + missing for videos. The director reported "I can't see any 🗑 next to any category."
  - **Corrected decision (deploy 2, Rule 14f):** the director chose **✕ on each category pill** (over a per-section "Manage categories" button and a project-level page). A small ✕ now sits on the read-only category pill of every captured text / image / video card. The picker-trash from deploy 1 was REVERTED so there is exactly one category-delete surface. Reusable `CategoryPill` component owns the count-fetch + confirm + cascade + reload; used by all three cards. Non-category vocab stays out of scope.
