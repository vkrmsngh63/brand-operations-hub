...
**2026-05-30-d (`session_2026-05-30-d`) — Session 2 FINISHED + the NEW "Source Reviews" column SHIPPED + 4 director Phase-4 adjustments SHIPPED. TWO deploys (`e2030e8` Source Reviews column [landed together with the held-back `d1659d7` + `fb772ad`] + `c641b3d` 4 adjustments). Director Phase 4 PASS on both. ✅ DONE — the Category page is now essentially CLOSED.**

- **The Source Reviews column ✅ DONE (`e2030e8`).** NEW 14th column `catSourceReviews` (read-only, `categoryLevel`) in `category-table-columns.ts`, inserted after `catBulleted`. NEW client-safe pure helper `buildCategorySourceReviewRows(categories, reviewsById)` in `reviews-traceability.ts` resolves each category bullet's cross-competitor union of reviewIds → product name + stars + merged title/body text + urlId (+3 node:test; kept in the client-safe module, NOT `category-analysis-aggregation.ts`, which imports runtime prompt helpers). `reviews-analysis-by-category/page.tsx` builds a global reviewId→meta map from the eager-loaded reviews + each URL's product name + renders the cell. `UrlDetailContent.tsx` gained a per-review anchor `id={`review-${row.id}`}` + scrollMarginTop + a scroll-on-hash effect so the Source Reviews jump link (`/…/url/<urlId>#review-<id>`) lands on the exact review.
- **The two Category AI flows (`d1659d7` + `fb772ad`, built 2026-05-30-c) DEPLOYED this session** as part of the same Deploy 1 ff-merge — they were ahead of main and landed together with the Source Reviews column, so the WHOLE Category Session 2 feature went live as one unit.
- **4 director Phase-4 adjustments ✅ DONE (`c641b3d`):**
  - **(#1) Top-align the Category name on the banner** (was vertical-center).
  - **(#2) Removed the per-category NON-bulleted prose write-back into competitor notes boxes.** Deleted the append-merge loop in `review-analysis-run-batch.ts`. Category-level prose now lives ONLY in the Category page's "Category Comprehensive (non-bulleted)" column. **Director clarified + confirmed:** the non-bulleted COLUMN keeps its paragraph prose; only the duplication into each in-category competitor's "Your notes — Captured Reviews" box was removed; the Source Reviews column was never prose. The per-COMPETITOR non-bulleted write-back is a different flow and was left intact. **(This supersedes the §3 "Write-back to URL detail" sub-section's per-category directive — the per-category non-bulleted prose no longer writes back to competitor boxes.)**
  ...
  - **(#4) Source Reviews layout restructured to per-bullet sub-rows.** Each bulleted complaint is now its OWN sub-row that extends across the "Category Comprehensive (bulleted)" + "Source Reviews" columns (per-bullet row alignment, mirroring the URL-detail traceability table). The banner is now a multi-`<tr>` block: grip / category-name label / non-bulleted prose rowSpan the whole category; the bulleted text + its source reviews render per sub-row. Replaces the prior single-cell stacked layout. NEW reusable Pattern memorialized — "Per-row alignment across two adjacent table columns via a multi-`<tr>` banner block with rowSpan'd flanking cells" (CORRECTIONS_LOG §Entry 2026-05-30-d; REVIEWS_PHASE_2_DESIGN §B 2026-05-30-d).
- **Category page status = essentially CLOSED.** The two category AI flows + the Source Reviews column + the 4 adjustments all shipped + verified. **Only the optional editable-banner-category-name refinement remains** (the banner category name is a read-only label today; making it editable to rename the whole category group is director's discretion).
- **Schema-change-in-flight NO entire session** (the column reads already-persisted PER_CATEGORY data; the write-back removal is a deletion; no `prisma db push`).
...
> **✅ SHIPPED-STATE UPDATE 2026-05-30-d — read FIRST — the Category page is now essentially CLOSED.** The Session 1 scaffold + polish + fixes (2026-05-30) + the interactive batch (2026-05-30-b) + the two Category AI flows (built 2026-05-30-c, DEPLOYED 2026-05-30-d) + the NEW "Source Reviews" column + 4 director Phase-4 adjustments (all 2026-05-30-d) have SHIPPED + been director-verified live. The §2 2026-05-30-d entry is authoritative for the as-shipped behaviour. Key as-shipped facts that SUPERSEDE notes below:
> - **There are now 14 columns, not 13** — the NEW read-only `catSourceReviews` ("Source Reviews") column was inserted after Column 12 "Category Comprehensive (bulleted)". It shows, per bulleted complaint, the individual reviews (across all in-category competitors) that traced up to that bullet — product name + stars + review text + a jump-to-detail link.
> - **The Source Reviews + bulleted columns render per-bullet sub-rows.** The category banner is a multi-`<tr>` block: grip / category-name label / non-bulleted prose rowSpan the whole category; each bulleted complaint + its source reviews render on their own `<tr>` so they line up row-for-row (the "multi-`<tr>` banner block with rowSpan'd flanking cells" Pattern).
> - **The per-category non-bulleted prose does NOT write back into competitor notes boxes** (Deploy-2 adjustment #2). The "Write-back to URL detail" sub-section below is SUPERSEDED for the per-CATEGORY flow — the category-level prose lives ONLY in the Category page's non-bulleted column. (The per-COMPETITOR non-bulleted write-back flow is unaffected.)
> - **The two "Auto-create …" category AI buttons carry auto-fading HoverTooltips** (the NEW shared `components/HoverTooltip.tsx` with `autoHideMs`).
> - **The banner category name is TOP-aligned** (Deploy-2 adjustment #1).
> - **Only the optional editable-banner-category-name refinement remains open.**
>
> The earlier ✅ SHIPPED-STATE UPDATE 2026-05-30-b facts (now also still true):
