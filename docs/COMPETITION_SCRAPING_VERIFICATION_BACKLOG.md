...
**Status:** DEPLOY LIVE on `main` (commit `bd7b39a`; `dc3a314..bd7b39a` ff-merge); browser sideload + permission re-approval + Walmart context-menu CONFIRMED; form rendering + save flow + cross-platform smoke DEFERRED to the next session (a.23).
...
### Phase 2 — Walmart product-page smoke — PARTIAL (2.1–2.3 ✅; 2.4–2.6 DEFERRED)

| # | Step | Director observation 2026-05-13-b |
|---|---|---|
| 2.1 | Sign in to vklf.com (any session) | ✅ assumed signed-in (not re-checked this session — carries from prior smoke) |
| 2.2 | Walmart product page (URL `https://www.walmart.com/ip/Equate-Extra-Strength-Cool-Heat-Medicated-Patches-5-Count/17056909?classType=VARIANT&athbdg=L1102&from=/search`) | ✅ navigated; valid `/ip/` product page (Equate Cool Heat patches) |
| 2.3 | Right-click main product image → check for "Add to PLOS — Image" in context menu | ✅ **"Add to PLOS — Image" appears** — confirms (i) Chrome accepted 5 new image-CDN host_permission patterns and (ii) `background.ts` context-menu wiring (`chrome.contextMenus.create({ contexts: ['image'] })`) fires in production |
| 2.4 | Click "Add to PLOS — Image" → describe form (image preview thumbnail + saved-URL picker + image-category + Composition + Embedded text + Tags + Save button) | ❌ NOT TESTED — terminal-UI rendering issue interrupted; **resume here next session** |
| 2.5 | Fill form (pick a saved-URL for this Walmart product OR confirm form's behavior when no saved-URL exists yet; fill image-category from vocab, optional Composition / Embedded text / Tags) | ❌ NOT TESTED |
| 2.6 | Click Save → wait for success indicator → navigate to vklf.com URL-detail-page for that Walmart URL → confirm CapturedImage row appears in the image gallery with the correct image-category + metadata | ❌ NOT TESTED — the actual 2-phase upload contract (`fetchImageBytes` from extension origin → `requestImageUpload` Phase 1 → direct PUT to Supabase signed URL Phase 2 → `finalizeImageUpload` Phase 3 → CapturedImage row) is not yet exercised end-to-end live |

### Phase 3 — Cross-platform smoke — ❌ FULLY DEFERRED

Per director's hybrid-verify pick (Option C in Rule 27 forced-picker), the cross-platform sweep is scoped to ebay (next priority — clean historical track record) + optionally etsy + Amazon (Amazon last given P-14 / P-15 / P-20 history on that platform).

| Platform | Status |
|---|---|
| ebay (clean historical track record) | ❌ NOT TESTED — recommended as second platform for next session |
| etsy | ❌ NOT TESTED — third priority |
| Amazon | ❌ NOT TESTED — fourth priority (test last given P-20 cross-platform design open) |

### Why deferred + recovery plan

**Why deferred this session:** terminal-UI rendering issue in director's Codespaces terminal made my step-by-step instructions visually overlap, making each step hard to read. Claude offered Ctrl+L clear-screen + terminal-resize fixes; director chose session-wrap over rendering-debug to keep the verification clean rather than push through a degraded-comms state.

**Recovery plan for next session (a.23):**
1. Director opens a clean Codespaces terminal session and confirms the rendering renders cleanly.
2. Resume Walmart smoke at Step 2.4 above (right-click is still active in director's Chrome; if not, redo Phase 1 sideload).
3. Walk Step 2.4 → 2.5 → 2.6 with small checkpoints (Rule 2 — confirm each step before the next).
4. If the form has a saved-URL picker dependency (i.e., requires a previously-captured URL for this Walmart product), and no such URL exists, the natural fix is: have director capture the URL first via the existing right-click "Add to PLOS — URL/Text" gesture on the same page, then return to the image gesture to attach an image to that captured URL. (Code-read of `image-capture-form.ts` may clarify what the form does when no saved-URLs exist — possible: shows "Save URL first" prompt or empty dropdown.)
5. After Walmart's 2.4-2.6 succeed, move to ebay Phase 2 smoke (same 2.2-2.6 sequence on an ebay product page).
6. Optional: etsy + Amazon to close out the cross-platform sweep.
7. Once all platforms pass: (a.23) flips to ✅ DONE in ROADMAP; the natural next-next session is region-screenshot session 6 OR W#2 polish item, director's pick via Rule 14f.
