...
|---|---|---|---|
| 1 | Open a URL detail page on vklf.com for a real Independent Website URL | URL detail page loads with Captured Images section at bottom | DEFERRED |
| 2 | Locate "+ Manually add captured image" button at right end of Captured Images section h2 row | Green button visible alongside "Captured Images" header | DEFERRED |
| 3 | Click the button → modal opens | Modal opens with drop zone + "or paste an image URL" text field; cursor not auto-focused (lets paste-listener stay active) | DEFERRED |
| 4 (Modality A — drag-drop) | Drag a small (1-2 MB) JPEG/PNG/WebP from desktop onto the drop zone | Dropzone highlights on dragover; on drop, preview thumbnail appears + mime + size text | DEFERRED |
| 5 | Click "Save captured image" | Modal closes; new image appears in gallery; row has `source='manual'` in DB | DEFERRED |
| 6 (Modality B — clipboard paste) | Reopen modal; press Ctrl/Cmd+V with a clipboard image (screenshot or copied web image) | Preview appears with pasted image | DEFERRED |
| 7 | Save → modal closes → new row in gallery | Same as Step 5 | DEFERRED |
| 8 (Modality C — URL of image) | Reopen modal; paste a real public image URL into the URL field; click Fetch image (or press Enter) | Spinner during fetch (~1-3s); preview appears with fetched image | DEFERRED |
| 9 | Save → modal closes → new row in gallery | Same as Step 5 | DEFERRED |
| 10 (Modality D — SSRF defensive spot-check) | Reopen modal; paste `http://localhost/test.png` or `http://192.168.1.1/img.png` into URL field; click Fetch image | Inline error message rejecting the resolve (e.g., "imageUrl resolves to a private network address, which is not allowed") | DEFERRED |
| 11 (Modality E — extension regression) | With Chrome extension active, capture an image via existing right-click "Save to PLOS" or region-screenshot gesture | New row appears in gallery; in DB the `source` column defaults to `'extension'` (not visible in UI; spot-check optional via Supabase Studio) | DEFERRED |
| 12 | Dismiss paths spot-check on at least one modal: Escape / Cancel button / X button / backdrop click | All dismiss the modal (when not submitting / not fetching) | DEFERRED |

**Why this section is pending:** workflow branch isn't deployed to vklf.com; can't run the walkthrough until W#2 → main deploy session #15 brings Slice #3 to live.

**Cross-references:**
- ROADMAP W#2 row (a.33) ✅ SHIPPED-AT-CODE-LEVEL + new (a.34) RECOMMENDED-NEXT W#2 → main deploy session #15
- `COMPETITION_SCRAPING_DESIGN.md` §B 2026-05-15-e (this session's design refinement entry — Slice #3 ship outcome + SSRF design pick + security test coverage + integration architecture)
