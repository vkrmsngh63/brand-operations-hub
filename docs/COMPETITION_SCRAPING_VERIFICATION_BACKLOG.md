...
## P-38 ✅ DONE — Popup auto-pre-select URL when only 1 saved URL exists (NEW 2026-05-19-c — Rule 11 in-session scope-add; SHIPPED-AT-CODE-LEVEL + DEPLOYED + VERIFIED 2026-05-19-c in W#2 → main deploy session #22)

**Status:** ✅ **DONE 2026-05-19-c** on `workflow-2-competition-scraping` then deployed to main via deploy session #22 (commit `a0d5c8a` stacked on P-13's `e217eb9`; Vercel auto-redeploy no-op since web bundle unchanged — fix lives in extension code only).
...
## P-39 ⏳ OPEN — Sticky most-recently-used URL preference (popup, multi-URL case) (NEW 2026-05-19-c — set as (a.45) RECOMMENDED-NEXT)

**Status:** ⏳ **OPEN** — set as (a.45) RECOMMENDED-NEXT for next session per §4 Step 1c forced-picker; see NEXT_SESSION.md for launch prompt.
