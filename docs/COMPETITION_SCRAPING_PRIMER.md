...
| **P-63** | NEW ACTIVE platform-wide item — a self-serve central AI-model registry (models + company + thinking options + pricing) that auto-propagates to EVERY model picker, no hardcoded lists. Supersedes/extends P-52 + ABSORBS both P-52 carry-overs (Opus 4.8 pricing + the W#1 `AutoAnalyze.tsx` migration). **Phase 0 BUILT (commit `6c6959c`, the additive `src/lib/ai-models/` module + provider adapter, +15 node:test) but HELD UNDEPLOYED** (director "Hold, batch with Phase 1"); Phase 1 (the (a.136) RECOMMENDED-NEXT) repoints every picker + deploys `6c6959c` together with the first surface. Canonical = `docs/polish-item-specs/P-63-central-ai-model-registry-self-serve.md` + ROADMAP P-63 + `docs/AI_MODEL_REGISTRY.md` site #4. | ACTIVE (Phase 1 next) |
