...
| **P-63** | ACTIVE platform-wide item — a self-serve central AI-model registry (models + company + thinking options + pricing) that auto-propagates to EVERY model picker, no hardcoded lists. Supersedes/extends P-52 + ABSORBS both P-52 carry-overs (Opus 4.8 pricing + the W#1 `AutoAnalyze.tsx` migration). **Phase 0 + Phase 1 ✅ DEPLOYED-AND-VERIFIED 2026-06-03-f** (3 deploys `256306b`/`474ac4b`/`8d4099e`, all director PASS): the registry now backs all **7** W#2 review-analysis modals + W#1 `AutoAnalyze.tsx` (the model list + pricing live in `src/lib/ai-models/`; a per-record `menus` tag + `getModelsForMenu` accessor keep W#2 Opus-only); plumbing only, ZERO behavior change; the NEW `docs/AI_MODEL_REGISTRY_PRIMER.md` ships the add/remove/edit-a-model catch-up guide. **Phase 2 (the (a.137) RECOMMENDED-NEXT)** = the self-serve "AI Models" admin screen + DB-backed storage (additive Prisma) + the integration-pending popover; confirm D1 + D4 with the director before coding. Canonical = `docs/polish-item-specs/P-63-central-ai-model-registry-self-serve.md` + ROADMAP P-63 + `docs/AI_MODEL_REGISTRY.md` site #4 + `docs/AI_MODEL_REGISTRY_PRIMER.md`. | ACTIVE (Phase 2 next) |
