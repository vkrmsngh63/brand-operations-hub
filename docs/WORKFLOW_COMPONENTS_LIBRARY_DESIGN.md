...
**First-audit-consumer refinement (2026-06-04 — W#1 H-1 slice 1; faithful note, not a rewrite of the frozen structural decisions above).** W#1 (Keyword Clustering) became the FIRST workflow to record audit events (H-1 — action history). Three observations refine — but do NOT change — the frozen structural decisions:
- **The existing shared `AuditEvent` table shape is sufficient — no schema change was needed.** The table already existed in BOTH `prisma/schema.prisma` (line 715) AND the live DB (added 2026-05-06, commit `701775f`, during a W#2 build slice; UNUSED until W#1 consumed it). Its columns match the provisional sketch above closely enough that W#1's first consumer needed no migration. The sketch's `workflowType` discriminator + `payload` JSON + `(projectId, workflowType)` indexing held up against a real consumer's needs.
- **W#1 records server-side via a dedicated keyword-clustering `audit-events` route — NOT the library `useEmitAuditEvent()` hook.** W#1 predates the library `useWorkflowContext` context and has its own auth/userId flow (`page.tsx`); its manual edits flow through `src/hooks/useCanvas.ts` + `src/hooks/useKeywords.ts`. So it wrote its own pure vocabulary (`src/lib/audit-payload.ts`) + a best-effort client sender (`src/lib/audit-recorder.ts`) + a server inbox route (`src/app/api/projects/[projectId]/audit-events/route.ts`) that validates eventType against the vocab and inserts rows. This is the per-workflow-event-vocabulary principle in practice (W#1's vocab lives in W#1 code), just reached without importing the library hook.
