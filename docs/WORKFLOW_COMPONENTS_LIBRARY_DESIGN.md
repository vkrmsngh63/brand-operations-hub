...
**First-audit-consumer refinement (2026-06-04 — W#1 H-1 slice 1; refined 2026-06-04-b — slice 2; faithful note, not a rewrite of the frozen structural decisions above).** W#1 (Keyword Clustering) became the FIRST workflow to record audit events (H-1 — action history). Three observations refine — but do NOT change — the frozen structural decisions:
- **The existing shared `AuditEvent` table shape is sufficient — no schema change was needed** (and still none at slice 2 — manual edits only added rows + three new eventType values, not columns). The table already existed in BOTH `prisma/schema.prisma` (line 715) AND the live DB (added 2026-05-06, commit `701775f`, during a W#2 build slice; UNUSED until W#1 consumed it). Its columns match the provisional sketch above closely enough that W#1's first consumer needed no migration. The sketch's `workflowType` discriminator + `payload` JSON + `(projectId, workflowType)` indexing held up against a real consumer's needs.
- **W#1 records via TWO dedicated W#1 paths — NOT the library `useEmitAuditEvent()` hook.** W#1 predates the library `useWorkflowContext` context and has its own auth/userId flow (`page.tsx`); its manual edits flow through `src/hooks/useCanvas.ts` + `src/hooks/useKeywords.ts`. So it wrote its own pure vocabulary (`src/lib/audit-payload.ts`) + (1) a best-effort CLIENT sender `src/lib/audit-recorder.ts` that POSTs AI-run batches to a server inbox route (`src/app/api/projects/[projectId]/audit-events/route.ts`, which validates eventType against the vocab and inserts rows) [slice 1] + (2) a best-effort, POST-COMMIT SERVER-side recorder `src/lib/audit-recorder-server.ts` that inserts directly inside the keyword-clustering server mutation routes, OUTSIDE the mutation transaction so a recorder failure can never roll back a user's manual edit [slice 2, 2026-06-04-b]. This is the per-workflow-event-vocabulary principle in practice (W#1's vocab lives in W#1 code), just reached without importing the library hook — and it confirms that an audit consumer may need BOTH a client emit path (for client-orchestrated batches) AND a server-side in-route path (for direct server mutations); a future library hook should accommodate both shapes.
- **The library `useEmitAuditEvent()` stub remains a documented no-op** until a genuinely library-composed workflow (one that uses `useWorkflowContext`) declares audit needs. At THAT point, generalize the W#1 route (drop the hardcoded `workflowType='keyword-clustering'` + the W#1-specific vocab) into the shared hook's insert path. Until then, the signature + schema stay PROVISIONAL per the over-prescribing-avoidance lens — W#1's as-built is evidence the shape is on the right track, not a trigger to freeze it.
