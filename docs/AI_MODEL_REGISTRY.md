...
| 4 | **Platform-level AI-model registry (P-63 Phase 0 — the NEW single source of truth in progress)** | `src/lib/ai-models/registry.ts` (+ `types.ts`, `provider-adapter.ts`) | Platform-wide (W#1 + W#2) | Opus 4.8, Opus 4.7, Opus 4.6 (seeded FROM sites #1/#2 — no duplication) | Opus 4.7 (`DEFAULT_MODEL_ID`) | references `MODEL_PRICING` (site #2) | 
