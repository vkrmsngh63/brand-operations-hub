...
- **#4 Supabase Pro upgrade for pool-capacity headroom (NEW 2026-05-04-c):** ❌ NOT STARTED. Director-action item between sessions; not a code change. Estimated cost $25/month; reversible (downgrade allowed).

### Standing instructions for next session

(a) **D3 retry attempt — RECOMMENDED, GATED BY #4.** Same plan as 2026-05-04-b's recommended path but blocked until prerequisite #4 ships. Once Pro upgrade is in place, fresh session resumes from the preserved checkpoint OR starts a new run. ~$30-50 + 3-5 hr wall-clock. Closes the gate in a single run if all four prerequisites verify.
...
**Upgrade Supabase to Pro.** Step-by-step instructions in this session's personalized handoff. Estimated cost $25/month; reversible. Without this, the D3 retry will hit the same pool-exhaustion bottleneck.
