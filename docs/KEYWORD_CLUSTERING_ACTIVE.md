...
## ⚠️ POST-2026-05-05-B-SISTER-LINK-CONSOLIDATION-DRIFT-CLEANUP-SHIPPED STATE (READ FIRST — updated 2026-05-05-b)
...
**Three-step pool investigation executed in dashboard + live test:**

| Step | What we found | Verdict |
|---|---|---|
| (i) Verify Pro tier active | Confirmed active in Supabase dashboard | ✅ |
| (ii) Find pool-size knob + check default | Pool size at default **15** for compute size **"Nano"**; Max client connections fixed at **200** (pgbouncer ceiling) but Postgres `max_connections` is **60** on Nano (revealed by the dashboard's 80% warning when 60 was attempted) | Default-low; tunable but bounded by Nano compute tier |
| (iii) Tune to safe value + small-batch test | Pool size raised to **40** (67% of 60-cap; 13-point buffer below Supabase's 80% warning threshold; covers estimated 22-burst with 18 spare). Test: resume Bursitis Test 2 from 30/291 checkpoint at canvas 124, run 1 batch | Wall did NOT clear; same failure pattern |

**Test result (batch 2 timeline at 11:06:26-11:08:21 PM director local time):**

- Model phase: clean. $0.320, 56,145 input + 5,993 output tokens, 19 operations applied
- Canvas-rebuild server-side: clean. ✓ Canvas rebuilt (124 nodes, 0 removed, 0 new sister links)
- 2 archived keywords correctly identified ("Greg Hayes" proper-name + "Capsella bursa-pastoris" botanical Latin — both correct semantic calls; no quality regression)
- **Post-rebuild UI refresh: FAILED** 3× consecutively with HTTP 500 on `state` then `nodes` then exhausted
- Run paused at batch 2 with same retry-helper-driven pause as 2026-05-04-c

### Vercel log expansion — the actual burst is 55-60+, not 22

Director shared full Vercel runtime logs for the failure window (~23:08:06-23:08:21 UTC). Counted PATCH `/keywords/[id]` calls in the burst:

- **PATCH 200 successes:** ~40+ within a 3-second window (23:08:06.31 → 23:08:09.54)
- **PATCH 500 failures with FATAL: Max client connections reached:** ~18+ in the same window
- **Total burst:** **55-60+ parallel PATCH calls per batch**

This is **2.5-3× larger than the 2026-05-04-c estimate of 22.** That estimate was based on a Vercel view that showed only failures (collapsed/filtered view); tonight's expanded view shows the full burst — both the successes and the failures.

**Implication:** earlier reasoning that "pool size 30-50 would comfortably cover" was incorrect. To actually absorb the burst at Nano compute requires pool size ≥ 60 — but Postgres max_connections IS 60 at this compute tier, so a pool at 60 is at 100% of capacity (Supabase warns: "may result in instability and unreliability"). **No safe pool tune at Nano absorbs a 55-60+ burst.**

### Bonus finding — race condition (P2025)

One of the failed PATCHes (keyword `78c696c5`) returned a **different error type**: `P2025 "No record was found for an update"`. NOT pool exhaustion — a Prisma "record doesn't exist" error. Most likely cause: race condition where the canvas-rebuild `$transaction` archived a keyword (the activity log shows "2 archived keywords" at 11:08:04-11:08:05) while a parallel PATCH was still in-flight against it. Separate from the main pool issue but real. Eliminated for free by code-fix approach (iii) atomic-batch-into-rebuild-transaction; partially eliminated by approach (ii) bulk PATCH endpoint; not necessarily eliminated by approach (i) worker-queue.

### What the pool-tune outcome means for the W#1 PRODUCTION-READINESS GATE

Per standing instructions (item a in the prior STATE block): *"If clears → proceed to D3 retry. **If not → defer D3 and ship parallel-PATCH-burst reduction code-fix per ROADMAP item (d).**"*

**The wall did not clear. We are now on the code-fix path.** Prerequisite #5 of the gate is reframed:

- **#5 Pool-exhaustion mitigation: 🟡 POOL-TUNE TRIED 2026-05-04-d — INSUFFICIENT.** Pool tune from 15→40 reduced failures (~40 of the burst now succeed instead of all failing) but did NOT clear the wall. Code-fix path (item d below) is now PRIMARY and gate-blocking; pool tune to 40 stays in place as a secondary buffer that helps absorb whatever residual burst remains after code-fix lands.

- **NEW prerequisite #5b (code-fix): ❌ NOT STARTED.** Parallel-PATCH-burst reduction. Three approaches (see "Standing instructions for next session" item (a) below).

### Methodology improvements this session

**Rule 14e-extension — real-time deferred-items registry via TaskCreate (NEW — codified as HANDOFF_PROTOCOL.md Rule 26 this session).** Director raised concern that small things falling through cracks across many sessions could prove catastrophic over the long roadmap. The mid-session slip that triggered this: Claude said "I'll capture this as a sanity-check item in the deferred sweep too" without naming the destination doc — a Rule 14e violation (Rule 14e requires destination in the same sentence as the defer). Proposed enhancement: every defer also creates a TaskCreate with `DEFERRED:` prefix in the subject; end-of-session sweep reviews the TaskList (not memory); leftover DEFERRED tasks at end-of-session = automatic CORRECTIONS_LOG entry. Director approved at session mid-point. Wording locked in to HANDOFF_PROTOCOL.md as Rule 26 + cross-references in CLAUDE_CODE_STARTER.md + memory file.

**Recurring pre-flight visibility slip — captured** (Tasks #6 + #7). Claude has repeatedly told director the Resume button surfaces P1-P12 pre-flight check results visibly when in fact it does NOT (per director's repeated correction across multiple sessions). State block summaries describing "pre-flight passed all 12 checks" are post-hoc inferences, not real-time UI observations. Two captures: (a) ROADMAP feature item — make pre-flight visible on Resume click; (b) CORRECTIONS_LOG entry — recurring slip pattern + Rule 14a Read-It-Back applied to UI-flow descriptions.

**Canvas-size visibility deferral** (Task #1). Director noted the workspace top-bar doesn't display canvas node count; currently only readable from Activity Log. Phase-1 UI polish item captured.

**Auto-fire toggle absence finding** (Task #2). Toggle was visible in 2026-05-04-c per the prior STATE block ("cadence=10 + Min canvas=100 honored") but was NOT visible in tonight's overlay. Three possibilities (regression / UI redesign / hidden in sub-section) — investigation deferred to a future session. Did not block tonight's test because consolidation auto-fire only triggers every N batches and we ran only 1 batch.

### Standing instructions for next session

**(a) Parallel-PATCH-burst reduction code-fix — RECOMMENDED, GATE-BLOCKING.** Three approaches (per ROADMAP item d, refreshed with tonight's burst-size correction):

- **(i) Client-side worker queue** — serialize the 55-60+ parallel PATCHes via small worker queue (e.g., 10 simultaneous max). ~10-20 LOC change in AutoAnalyze.tsx. Quality impact: ZERO (same operations, same final state). Speed impact: each batch's apply phase ~3-6 seconds longer (vs. ~90s model phase — negligible). Risk: lowest. Doesn't fix the P2025 race condition.

- **(ii) Bulk PATCH /keywords endpoint collapse** — use the existing `PATCH /keywords` endpoint that accepts arrays. Send all 55-60 updates in 1 request instead of separate ones. Quality impact: ZERO if endpoint covers all needed fields (audit needed). Speed impact: faster (fewer round trips). Risk: medium (audit the bulk endpoint; possibly extend if missing fields). May or may not fix the P2025 race depending on internal sequencing.

- **(iii) Atomic batch into canvas-rebuild $transaction** — fold the keyword updates into the same `$transaction` that already rebuilds the canvas. One transaction, all updates atomic. Quality impact: ZERO. Reliability impact: IMPROVES — eliminates the P2025 race for free. Speed impact: potentially faster (one transaction vs. many round trips). Risk: highest — larger refactor; need to keep transaction within timeout bounds. **Most thorough and reliable per director's standing preference; likely my next-session recommendation pending code review.**

Director chooses approach at start of next session after Claude reads `src/lib/operation-applier.ts` + `src/app/projects/[projectId]/keyword-clustering/components/AutoAnalyze.tsx` + the canvas-rebuild route. Effort: 1-2 sessions.

**(b) `[FLAKE]` visibility investigation** — flake-counter telemetry shipped 2026-05-04 should log `[FLAKE]` lines for every catch-block error. Director's tonight Vercel logs again show full stack traces but **NO `[FLAKE]` lines visible** in the data shared. Same gap as 2026-05-04-c. Investigation deferred (Task carried over from 2026-05-04-c). Independent of (a); can run as standalone session.

**(c) Wrap remaining unwrapped routes** — `/projects` GET (N+1), `/project-workflows/*` GETs, `/admin-notes/*`, `/user-preferences/*`. Lower priority than the apply-pipeline cluster. ~30-50 LOC.

**(d) Pre-flight visibility on Resume** (NEW Task #6 captured this session) — Phase-1 UI polish bundle item, but elevated priority because it affects director's ability to self-verify a pre-run safety check.

**(e) Canvas-size in top-bar** (NEW Task #1 captured this session) — Phase-1 UI polish.

**(f) Auto-fire toggle absence investigation** (NEW Task #2 captured this session) — needs git diff + UI inspection to determine root cause.

**(g) GoTrueClient multi-instance fix** — small refactor (~15 LOC). Carried over.

**(h) Phase-1 UI polish bundle** (6+ items). Carried over.

**(i) V3-era cleanup pass** (deferred from Session E D4). Carried over.

**(j) Action-by-action feedback workflow + Prompt Refining button — EXPLICITLY LAST** per director's 2026-05-03-b directive. ~5-7 sessions.

**Recommendation: next session = (a) parallel-PATCH-burst reduction code-fix design + first-pass implementation.** Most-thorough-and-reliable path per director's standing preference. The W#1 PRODUCTION-READINESS GATE cannot close without this; D3 retry blocked behind it.

### Multi-workflow protocol coordination

- Schema-change-in-flight: stays `No`. Branch: `main`. W#2 still 🆕; no parallel chat. Pull-rebase clean at session start.
- **Cross-workflow surfacing:** the burst-size correction (22 → 55-60+) tightens the platform-wide implication first noted 2026-05-04-c. At Phase 3 (50 workers) the math becomes 50 × 60 = 3,000 simultaneous PATCH connections — uncoverable by any compute tier upgrade. The code-fix isn't optional; it's gate-blocking for Phase 3 across W#1-W#14 because they all share the same DB + pool.
...
**Docs (Group A):**
- `docs/KEYWORD_CLUSTERING_ACTIVE.md` (this STATE block prepended; prior 2026-05-04-c STATE block demoted to historical; header timestamps)
- `docs/ROADMAP.md` (Active Tools row updated; W#1 PRODUCTION-READINESS GATE prerequisite #5 reframed + new #5b code-fix prerequisite added; HIGH-severity "Pool exhaustion" entry updated with burst-size correction; new MEDIUM-severity "P2025 race condition" entry; new MEDIUM-severity "Pre-flight visibility on Resume" entry; standing instructions list refreshed; header)
- `docs/PLATFORM_ARCHITECTURE.md` (§10 "Connection pool exhaustion" entry burst-size correction note appended; header timestamp)
- `docs/HANDOFF_PROTOCOL.md` (NEW Rule 26 — real-time deferred-items registry via TaskCreate, the formal Rule 14e-extension; header timestamp)
- `docs/CLAUDE_CODE_STARTER.md` (Rule 26 cross-reference in Session Management section; header timestamp)
- `docs/CHAT_REGISTRY.md` (new top row + header timestamp)
- `docs/CORRECTIONS_LOG.md` (TWO new entries — Rule 14e mid-session slip + recurring pre-flight visibility slip)
- `docs/DOCUMENT_MANIFEST.md` (header + per-doc flags + this-session summary)

**Memory file:** `/home/codespace/.claude/projects/-workspaces-brand-operations-hub/memory/feedback_deferred_items_registry.md` NEW — operational principle for real-time TaskCreate registry per Rule 26.

**Push status:** doc-only commit; push pending Rule 9 approval. Doc-only push has zero user-visible impact on vklf.com.

### Director's offline action item (between this session and the next)

**None.** Pool tune from 15 → 40 stays in place (no need to revert — it's still helping absorb a portion of the burst even though it doesn't clear the wall; reverting would make things slightly worse). Next session is a Claude Code session for code-fix design + implementation.

**Director's framing through prior sessions:** (Scale-A) → (Scale-0) → (Defense-in-Depth ×3) → (Scale-B) → (Scale-C) → (Scale-D) → (Scale-E build) → (Scale-E D3 partial validation) → (consolidation auto-fire follow-up `2026-05-01-c`) → (HTTP 500 fix verification + auto-fire trip observation `2026-05-02`) → (browser-freeze fix design `2026-05-02-b`) → (DevTools profiling pass — diagnosis rejected `2026-05-02-c`) → (HTTP 500 retry regression investigation — fix shipped `2026-05-02-d`) → (HTTP 500 fix live verification + cold-start canvas-empty finding `2026-05-02-e`) → (cold-start render-layer fix `2026-05-03`) → (cold-start fix live verification — partial `2026-05-03-b`) → (recency-stickiness fix `2026-05-03-c`) → (underlying flake-rate investigation — instrumentation `2026-05-04`) → (rate-fix first pass — withRetry parity for silent helpers + apply-pipeline writes — DEPLOY `2026-05-04-b`) → (D3 retry attempt — partial; pool exhaustion finding ON Pro tier `2026-05-04-c`) → **(pool-tune small-batch test — INSUFFICIENT — code-fix path required; this session `2026-05-04-d`)** → (parallel-PATCH-burst reduction code-fix design + first-pass implementation — RECOMMENDED next).

---

## ⚠️ POST-2026-05-04-C-D3-RETRY-PARTIAL-POOL-EXHAUSTION-FINDING STATE (preserved as historical context — last updated 2026-05-04-c; SUPERSEDED by the pool-tune-small-batch-test-insufficient state above. The 2026-05-04-c finding's standing instruction item (a) was executed 2026-05-04-d with INSUFFICIENT result; the prerequisite #5 framing has been reframed accordingly above. The pool-tune from 15 → 40 stays in place on Pro tier as a secondary buffer; the structural answer is the code-fix that 2026-05-04-d's evidence pinpoints.)

**As of 2026-05-04-c (forty-first Claude Code session, third session of 2026-05-04, follow-up to `session_2026-05-04-b_rate-fix-first-pass`). DOC + LIVE-RUN session — no code changes — but a SIGNIFICANT FINDING surfaced live on production vklf.com.** D3 retry attempt launched after the 2026-05-04-b rate-fix shipped. Batch 1 ran cleanly on the model + apply-pipeline side ($0.312, 15 ops applied, canvas at 124 nodes, 0 new sister links — quiet confirmation V4 prompt paste was correct). Post-rebuild UI refresh exhausted three retry attempts on HTTP 500 errors; the post-rebuild fetch retry helper from 2026-05-02-d paused the run cleanly; Vercel logs revealed the root cause: **`FATAL: Max client connections reached`** — Supabase Postgres connection pool exhaustion. **This is a different failure mode than the rate-fix targets.**

**🚨 CRITICAL CORRECTION post-original-commit (added 2026-05-04-c after director's end-of-session clarification):** the prior STATE block's "director confirmed 2026-05-04-b on Free tier" claim was stale by 2026-05-04-c start. **Director had actually upgraded Supabase to Pro at end of the 2026-05-04-b session.** This means tonight's pool-exhaustion event fired **ON PRO TIER, not Free** — a more serious structural finding than the original framing of "Free-tier capacity ran out; upgrade fixes it." Implications: Pro upgrade alone is not sufficient; either (a) Pro's default pool size needs tuning (Pro exposes the knob; default may be too small for our ~22-parallel-PATCH burst), OR (b) the apply-pipeline's parallel-burst pattern itself needs reduction regardless of pool size, OR (c) some combination. Original framing of "next session = D3 retry after Pro upgrade" REPLACED with "next session = pool-exhaustion investigation on Pro tier (verify Pro active in Supabase dashboard → check pool config → tune OR design parallel-burst reduction); D3 retry follows after the wall is actually clear."

Director chose Path B at the in-session decision fork: pause D3 here. Original wording "prioritize Supabase Pro upgrade FIRST" no longer applies (Pro is already done); revised path is pool-exhaustion investigation on Pro tier in the next session.

### The finding in detail

**The error pattern (raw Vercel runtime logs, 8:30:40 PM):**

| Time | Status | Endpoint | Error message |
|---|---|---|---|
| 20:30:40.15-25 | 500 ×~22 | PATCH /api/projects/{id}/keywords/{kwId} | `Invalid prisma.project.findUnique() invocation: Error in connector: Error querying the database: FATAL: Max client connections reached` |
| 20:30:40.16-18 | 500 ×3 | GET /canvas + /canvas/nodes | Same `Max client connections reached` error |
| 20:30:42.62-63 | 500 ×2 | GET /canvas + /canvas/nodes | Same |
| 20:30:48.04 | 500 ×1 / 200 ×1 | GET /canvas (failed) + /canvas/nodes (recovered) | Same / OK (pool relieved) |

**Mechanical explanation:** When batch 1's apply pipeline finished its 15-op model-driven apply, the wiring layer fired the post-apply: many parallel PATCH /keywords/[id] requests to update keyword placements (one per touched keyword). Vercel serverless spawns multiple instances; each opens a Prisma connection through the Supabase pgbouncer transaction pooler at port 6543. The Free-tier pool capacity was overwhelmed. Postgres returned `FATAL: Max client connections reached` to every request beyond the limit. Each failed request also affected the auth helper's `prisma.project.findUnique` (the silent multiplier wrapped by the 2026-05-04-b rate-fix) — both surface as 500.

**Why the 2026-05-04-b withRetry rate-fix doesn't help:** `withRetry` retries transient errors with codes P1001/P1002/P1008/P2034. Pool exhaustion surfaces as `PrismaClientUnknownRequestError` or `PrismaClientInitializationError` with NO Prisma code at all — just the FATAL message embedded in `error.message`. Even if it WERE matched as transient, retrying immediately would worsen pool pressure (each retry attempts another connection while the pool is full). **Pool exhaustion is a CAPACITY problem, not a transient-flake problem. Retries cannot fix capacity.**

**Implication for prerequisite #3 of the W#1 PRODUCTION-READINESS GATE:** the 2026-05-04-b rate-fix is bounded to transient flakes. It does NOT address pool exhaustion. Prerequisite #3 status reframed to: 🟡 covers transient flakes (still needs natural-data verification); pool exhaustion is a separate failure mode requiring Supabase Pro upgrade for capacity headroom. **NEW prerequisite #4 added** to the gate: Supabase Pro upgrade — without it, D3 cannot complete reliably regardless of code-side fixes.

### Discovery flow (forensic)

1. Director re-pasted updated V4 Initial Prompt (59,498 chars) + Primer (29,731 chars) per 2026-05-04-b standing instructions; Consolidation prompts already-pasted preserved (20,546 + 22,514 chars).
2. Director clicked Discard (not Resume — Resume would have rehydrated OLD V4 prompts from checkpoint and auto-fired batch 1 with them); hard refresh first to expose cold-start banner UI for prerequisite #1 verification opportunity (no banner observed during refresh — no flake fired during the refresh window).
3. Pre-flight passed all 12 checks (P1-P12 including the new P11+P12 from 2026-05-02). Auto-fire cadence=10 + Min canvas=100 honored after Claude flagged the director's initial 3+15 settings as 3× more consolidation overhead than design intent.
4. **Batch 1 model-side: clean.** 56,152 input + 5,462 output tokens; $0.312; 15 operations applied; canvas at 124 nodes (mostly merges/updates rather than ADDs); 0 new sister links — quiet confirmation V4 paste was correct (model didn't emit forbidden ADD_SISTER_LINK ops; if paste had been wrong, applier would have rejected the entire batch atomically).
5. **Batch 1 server-side rebuild: clean.** ✓ Canvas rebuilt (124 nodes, 0 removed, 0 new sister links). Atomic `/canvas/rebuild` POST + multi-second $transaction succeeded.
6. **Post-rebuild UI refresh: failed 3× consecutively.** Attempts 1/3 (2s wait), 2/3 (5s wait), 3/3 (exhausted). All hit HTTP 500 on `fetchCanvas` calls (state + nodes endpoints).
7. The post-rebuild fetch retry helper from 2026-05-02-d fired exactly as designed: paused the run cleanly, preserved server state (the 15-op apply IS canonical on the server), saved checkpoint, surfaced explicit "Refresh and Resume" guidance to director.
8. Director shared Vercel runtime logs with Claude. All ~28 errors in the cluster show identical message: `FATAL: Max client connections reached`. This is the moment the pool-exhaustion finding crystallized.
9. Claude framed the discovery: rate-fix targets transient flakes; pool exhaustion is a separate failure mode. Director chose Path B — pause D3 here; address Pro upgrade first.

### Open question — `[FLAKE]` line visibility

The flake-counter telemetry shipped 2026-05-04 should have logged a `[FLAKE]` line for every catch-block error. Director's Vercel paste from this session showed ~28 errors at 8:30:40 with full stack traces but NO line beginning with `[FLAKE]` visible. Three possibilities:

1. **Vercel UI display issue** — runtime logs view may show only the first console.error per request (the existing `console.error('<X> error:', error)`); the second console.error from `recordFlake` may be hidden in a collapsed per-request view. Test next session: click into a single failed request to expand its full log; look for `[FLAKE]` line.
2. **Flake-counter not firing on these endpoints** — would be a real bug. The 2026-05-04 wiring was supposed to be exact 1-to-1 across all 38 catch blocks of the 19 DB-backed routes. PATCH /keywords/[keywordId] WAS in the wired list (per 2026-05-04 STATE block, +2 recordFlake calls). Re-verify the deploy actually shipped the wiring.
3. **Format issue** — `recordFlake` calls `console.error` which Vercel routes to runtime logs; possible but unlikely.

Captured as deferred investigation item — see "Standing instructions for next session" item (b) below + ROADMAP MEDIUM-severity entry.

### W#1 PRODUCTION-READINESS GATE prerequisite status (after this session)

- **#1 cold-start render-layer fix:** 🟡 PARTIAL (banner UI live verification deferred to natural flake event). Status unchanged this session — page reload during the pre-D3 hard refresh went through normally with no banner observed.
- **#2 recency-stickiness fix:** 🟡 CODE FIX SHIPPED 2026-05-03-c, LIVE on vklf.com via 2026-05-04-b push. Status unchanged this session — only 1 batch ran tonight (insufficient data for per-batch input-cost trajectory verification, which is the success signal across multiple batches).
- **#3 underlying flake-rate fix:** 🟡 FIRST RATE-FIX SHIPPED 2026-05-04-b, LIVE on vklf.com. **Status reframed** — covers transient flakes (still needs natural-data verification); does NOT cover pool exhaustion (now prerequisite #4). Natural-data verification window for transient-flake portion remains open.
- **#4 Supabase Pro upgrade for pool-capacity headroom:** ✅ DONE (director upgraded at end of 2026-05-04-b). **BUT:** tonight's pool exhaustion fired ON Pro tier despite the upgrade — meaning Pro's higher default pool capacity is still insufficient for the ~22-parallel-PATCH-burst pattern, OR Pro's configurable pool-size knob hasn't been tuned higher than the default. **NEW prerequisite #5 (NEW 2026-05-04-c, replaces the simple "Pro upgrade" framing):** pool-exhaustion investigation on Pro tier — verify Pro shows active in Supabase dashboard (5 min); check the pool size config knob and tune higher if at default (5-10 min); test with a small batch run to see if the wall clears. If pool tuning alone clears the wall → D3 retry can proceed. If pool tuning doesn't clear the wall → ship the parallel-PATCH-burst reduction code-fix first (1-2 sessions of code work; was the deferred "companion fix" — now becomes primary).

### Standing instructions for next session

(a) **Pool-exhaustion investigation on Pro tier — RECOMMENDED FIRST.** (Replaces original 2026-05-04-c "D3 retry after Pro upgrade" framing — Pro is already done; the wall is something else.) Three-step plan: (i) Verify Pro tier shows active in Supabase dashboard for the project (5 min). (ii) Check the connection pool size config (Pro tier exposes a configurable knob — default may still be too small for the ~22-parallel-PATCH-burst). If at a small default, increase to a value that comfortably covers the burst (e.g., 30-50). (iii) Re-attempt a small batch run (just 1-3 batches, ~$1-3 cost) to see if the pool wall is cleared. If clear → proceed to D3 retry in the same session OR a follow-up session. If NOT clear → defer D3 and prioritize the parallel-PATCH-burst reduction code-fix (item d below) before D3.

(a.2) **D3 retry attempt — GATED BY (a).** Once the pool wall is verified clear (either by pool tuning OR by the parallel-PATCH-burst reduction shipping), proceed with the full D3 retry per the original 2026-05-04-b plan. ~$30-50 + 3-5 hr wall-clock. Closes the gate in a single run if all prerequisites verify.

(b) **`[FLAKE]` visibility investigation.** Verify the flake-counter telemetry from 2026-05-04 is actually firing on PATCH /keywords/[keywordId] (and the other 18 wired routes). Three-step check: (i) read Vercel logs at the next live error event using expanded per-request view; (ii) if absent, re-confirm `recordFlake` calls present in deployed bundle; (iii) if present in bundle, write a small targeted test that triggers a PATCH error and confirm the `[FLAKE]` line appears in dev-server output. Independent of D3 — can run as a standalone investigation session before the Pro upgrade lands.

(c) **Wrap remaining unwrapped routes** — `/projects` GET (N+1), `/project-workflows/*` GETs, `/admin-notes/*`, `/user-preferences/*`. Lower priority than the apply-pipeline cluster but completes option (c) coverage. ~30-50 LOC; ~5-7 route files.

(d) **Apply-pipeline parallel-PATCH-burst architecture review (NEW 2026-05-04-c).** Investigate whether the ~22 parallel PATCH /keywords/[id] calls per batch can be (i) serialized client-side with a small worker queue (~5-10 LOC change), (ii) collapsed into a single bulk PATCH endpoint (existing PATCH /keywords accepts arrays — director's setup may already use bulk in some cases; needs audit), or (iii) atomically batched into the canvas rebuild transaction (heavier refactor). Even with Pro upgrade, ~22 parallel connections per batch is heavy for normal traffic; reducing parallelism reduces pool pressure linearly. Captured as ROADMAP architectural item.

(e) **GoTrueClient multi-instance fix** — small refactor (~15 LOC). Carried over.
(f) **Phase-1 UI polish bundle** (6+ items). Carried over.
(g) **V3-era cleanup pass** (deferred from Session E D4). Carried over.
...
**Recommendation: prerequisite #4 (Supabase Pro upgrade) is the director's between-sessions action item.** Once that's in place, next session resumes with item (a) D3 retry attempt. Item (b) `[FLAKE]` visibility investigation can run as a small standalone session in parallel.

### Multi-workflow protocol coordination

- Schema-change-in-flight: stays `No`. Branch: `main`. W#2 still 🆕; no parallel chat. Pull-rebase clean at session start.
- **Cross-workflow surfacing:** the pool-exhaustion finding has platform-wide implications — affects every database-touching route across all 14 future workflows AND scales worse with concurrent users (Phase 3 = 50 workers will hit pool exhaustion much harder than today's solo director). Captured in `PLATFORM_ARCHITECTURE.md §10` as platform-level technical debt + in `ROADMAP.md` as HIGH-severity entry.
...
**Docs (Group A):**
- `docs/KEYWORD_CLUSTERING_ACTIVE.md` (this STATE block prepended; prior 2026-05-04-b STATE block demoted to historical; header timestamps)
- `docs/ROADMAP.md` (Active Tools row updated; W#1 PRODUCTION-READINESS GATE prerequisite #3 reframed + new prerequisite #4 added; new HIGH-severity "Pool exhaustion under apply-pipeline burst" entry; new MEDIUM-severity "[FLAKE] visibility puzzle" entry; header)
- `docs/PLATFORM_ARCHITECTURE.md` (§10 gains "🚨 Connection pool exhaustion under apply-pipeline burst (DISCOVERED 2026-05-04-c)" entry; header timestamp)
- `docs/CHAT_REGISTRY.md` (new top row + header timestamp)
- `docs/CORRECTIONS_LOG.md` (INFORMATIONAL entry — process lesson on rate-fix scope vs. pool-exhaustion as a different failure mode)
- `docs/DOCUMENT_MANIFEST.md` (header + per-doc flags + this-session summary)
...
**None.** The Supabase Pro upgrade was already completed at end of 2026-05-04-b. (Original draft of this section said "Upgrade Supabase to Pro" — corrected post-original-commit when director clarified the upgrade had already happened.) Next session's pool-exhaustion investigation is a Claude Code session, not an offline director task.

**Director's framing through prior sessions:** (Scale-A) → (Scale-0) → (Defense-in-Depth ×3) → (Scale-B) → (Scale-C) → (Scale-D) → (Scale-E build) → (Scale-E D3 partial validation) → (consolidation auto-fire follow-up `2026-05-01-c`) → (HTTP 500 fix verification + auto-fire trip observation `2026-05-02`) → (browser-freeze fix design `2026-05-02-b`) → (DevTools profiling pass — diagnosis rejected `2026-05-02-c`) → (HTTP 500 retry regression investigation — fix shipped `2026-05-02-d`) → (HTTP 500 fix live verification + cold-start canvas-empty finding `2026-05-02-e`) → (cold-start render-layer fix `2026-05-03`) → (cold-start fix live verification — partial `2026-05-03-b`) → (recency-stickiness fix `2026-05-03-c`) → (underlying flake-rate investigation — instrumentation `2026-05-04`) → (rate-fix first pass — withRetry parity for silent helpers + apply-pipeline writes — DEPLOY `2026-05-04-b`) → **(D3 retry attempt — partial; pool exhaustion finding; this session `2026-05-04-c`)** → (Supabase Pro upgrade; D3 retry attempt — RECOMMENDED next).

---

## ⚠️ POST-2026-05-04-B-RATE-FIX-FIRST-PASS STATE (preserved as historical context — last updated 2026-05-04-b; SUPERSEDED by the D3-retry-partial / pool-exhaustion finding state above. Rate-fix code from this session remains shipped + LIVE on vklf.com; the 2026-05-04-c finding reframes its scope boundary — it covers transient flakes but not pool exhaustion which is a separate failure mode requiring the Supabase Pro upgrade for capacity headroom.)
...
### What this session shipped to W#1 (and platform-wide infrastructure)

**New code (single commit, ready for Rule 9 push approval):**

- **`src/lib/flake-counter.ts` (NEW, ~140 LOC incl. extensive header comment).** Pure helper module. Exports:
  - `recordFlake(endpoint: string, error: unknown, ctx?: FlakeContext)` — emits a single grep-able `[FLAKE]` line via `console.error`. Safe to call from inside any route's catch block — purely additive; does NOT swallow the error or change downstream control flow.
  - `formatFlakeLine(endpoint, error, ctx)` — pure string-builder used by `recordFlake`; exposed for unit testing without mocking `console`.
  - `extractPrismaCode(error)` — recovers `error.code` if present, returns `"unknown"` otherwise. Defensive against null / undefined / string-thrown / non-string-coded errors.
  - `isTransientCode(code)` — true for the four codes the existing G2 `withRetry` helper would retry (P1001/P1002/P1008/P2034). Mirrors `prisma-retry.ts` but standalone so tests don't import it transitively.
  - `FlakeContext` interface — `{ retried?: boolean; canvasSize?: number; projectWorkflowId?: string }`. Optional fields surface only when present; no falsy-skip on `canvasSize=0`.
  - **Line shape (canonical):** `[FLAKE] endpoint=<METHOD /path> code=<P1001|...|unknown> retried=<true|false> [canvasSize=<N>] [projectWorkflowId=<id>] msg="<first-line of error message, truncated to 200 chars>"`.
  - **Why log lines instead of a DB table:** the whole point of the instrumentation is to measure DB flake rate; writing the measurement to the DB itself would be subject to the same flake. Vercel's log retention (7 days Hobby / 30 days Pro) is enough for a measurement window. Future iterations can route the same call into a different sink by changing this one module.

- **`src/lib/flake-counter.test.ts` (NEW, 28 tests).** `node:test` + `node:assert/strict` pattern matching the other pure-helper test files. Coverage: extractPrismaCode (6 tests — Prisma error / plain Error / null / string / non-string code / empty code), isTransientCode (7 tests — each transient code true; P2002 + P2025 false; "unknown" false), formatFlakeLine (13 tests — minimal Prisma error / retried=true rendered / canvasSize emitted / projectWorkflowId emitted / all extras together / plain Error → code=unknown / null error → empty msg / undefined → empty msg / string-thrown preserved / multiline whitespace collapsed / 500-char truncated to 200 / canvasSize=0 still emitted / explicit retried=false serialized), recordFlake side-effect (2 tests — emits via console.error with all extras / works with no ctx). All 28 pass via `node --test --experimental-strip-types`.

- **All 19 DB-backed API routes wired (additive-only — no behavior change on success path).** Each route gained:
  - One `import { recordFlake } from '@/lib/flake-counter';` line at the top.
  - One `recordFlake(<METHOD /api/path>, error, { ... })` call AT THE TOP of every catch block, BEFORE the existing `console.error('<X> error:', error)` line. The structured `[FLAKE]` line is grep-able for counts; the existing console.error is preserved as the full diagnostic dump for manual inspection.
  - **`retried: true`** on the two routes already wrapped in withRetry: `GET /api/projects/[projectId]/canvas` + `GET /api/projects/[projectId]/canvas/nodes`. Lets analysis separate "wrapped-and-still-failed" (G2 exhausted) from "raw flake" (no retry layer).
  - **`projectWorkflowId`** included whenever it's in scope (which is all the keyword-clustering routes — `/canvas/*`, `/keywords/*`, `/removed-keywords/*`).
  - **`canvasSize`** captured at `/canvas/rebuild` POST as `body.nodes.length` (the payload-size that scales transaction duration). Captured into a `let payloadNodeCount: number | undefined` declared outside the try so the catch can read it. Tests the "transaction-duration scales with payload → pgbouncer pressure" hypothesis directly.
  - **Coverage:** 38 catch blocks ↔ 38 `recordFlake(...)` call sites — exact 1-to-1, verified by parallel `grep -c` runs. The 20th route file (`/api/ai/analyze/route.ts`) doesn't use Prisma — deliberately skipped.
  - Files modified: `/api/projects/[projectId]/canvas/route.ts` (+2 catches), `/canvas/nodes/route.ts` (+4), `/canvas/rebuild/route.ts` (+1 — also captures payloadNodeCount), `/canvas/sister-links/route.ts` (+2), `/canvas/pathways/route.ts` (+2), `/keywords/route.ts` (+4), `/keywords/[keywordId]/route.ts` (+2), `/removed-keywords/route.ts` (+2), `/removed-keywords/[removedId]/restore/route.ts` (+1), `/projects/[projectId]/route.ts` (+3), `/projects/route.ts` (+2), `/project-workflows/[projectId]/route.ts` (+1), `/project-workflows/[projectId]/[workflow]/route.ts` (+2), `/user-preferences/[key]/route.ts` (+2), `/admin-notes/route.ts` (+2), `/admin-notes/[noteId]/route.ts` (+3), `/admin-notes/[noteId]/attachments/route.ts` (+1), `/admin-notes/[noteId]/attachments/[attachmentId]/route.ts` (+1), `/admin-notes/reorder/route.ts` (+1).

**Investigation findings (read-only — these are observations, not code changes):**

1. **Supabase connection mode confirmed (from `.env.local`):**
   - `DATABASE_URL` uses `aws-1-us-east-2.pooler.supabase.com:6543/postgres?pgbouncer=true` — the **transaction pooler**. All runtime queries go through pgbouncer. The pgbouncer hypothesis from the ROADMAP entry is correctly grounded.
   - `DIRECT_URL` uses port 5432 — the direct connection. Used only for migrations (`prisma db push`).
   - **Project ref:** `vyehbgkvdnvsjjfqhqgo`, region `aws-1-us-east-2`.
   - Supabase plan tier itself is NOT visible in `.env.local` — director needs to share that out-of-band when the rate-fix design happens (likely next session). Pro tier offers configurable pool sizing + dedicated pgbouncer; Free tier is fixed-small.
...
2. **withRetry parity audit (read-only inspection of all `src/app/api/` route files):**
   ...
   | Cold-start fetches | 0 wrapped | `/keywords` GET + `/removed-keywords` GET both unprotected. The 2026-05-03 client-side helper retries them, but the server-side rate is unmitigated. |
   | Page-render | 0 wrapped | `/projects` GET (N+1 loop hits Prisma O(N×3) times — once per project + 2× per project for keyword/canvasNode count), `/projects/[id]` GET, `/project-workflows/[id]` GET, `/project-workflows/[id]/[workflow]` GET. |
...
3. **Prisma client lifecycle observation (`src/lib/db.ts`):** the bare singleton pattern with no explicit `$connect()` / `$disconnect()`. No `@prisma/adapter-pg`. Vercel serverless cold starts pay full Prisma-client init cost; subsequent invocations on the same warm container reuse the singleton via the `globalThis` trick. This is one of the investigation-direction (b) items on ROADMAP and may benefit from `@prisma/adapter-pg` audit at the rate-fix session.

### Verification scoreboard (build + tests + lint)

| Check | Result | Note |
|---|---|---|
| `node --test src/lib/*.test.ts` | ✅ 327/327 pass | Was 299; +28 new from `flake-counter.test.ts` |
| `npx tsc --noEmit` | ✅ clean | |
| `npm run build` | ✅ clean | 25 routes |
| `npm run lint` | ✅ 16e/40w (better than baseline 16e/41w) | Zero new errors; net -1 warning vs baseline. (No investigation done into which warning was eliminated — likely an incidental side effect of recordFlake replacing the path that previously triggered a no-floating-promises advisory; not load-bearing.) |

### How the fix behaves in plain language

When a request hits the platform's database and the database fails (transient connection-pool flake, timeout, etc.), the API route's catch block now does TWO things:

1. **Emits a structured "[FLAKE]" log line** — short, single-line, easy to grep from Vercel's log dashboard. Includes the endpoint name, the Prisma error code (or "unknown" for non-Prisma errors), whether the failure happened despite an automatic retry layer (`retried=true` on the 2 wrapped routes; `retried=false` everywhere else), and optional context fields (canvas size for the rebuild route; workspace id whenever known).
2. **Still emits the existing diagnostic dump** — the multi-line `console.error('<route> error:', error)` is preserved unchanged so detailed debugging info remains available.

The success path is completely unchanged. There's no overhead on requests that don't fail. The cost is one extra log line per error already being logged — additive, easy to roll back if it ever becomes noisy.

**What we get from a real measurement window:** once the director uses the platform normally for a day or two with this code live, Vercel's logs will have hundreds-to-thousands of `[FLAKE]` lines. We can grep them to count:
- Per-endpoint flake counts → which endpoints are hottest under the underlying ~25% rate.
- `code=` distribution → which Prisma error codes dominate (P1001 cant-reach vs P2034 deadlock vs P2025 not-found etc.).
- `retried=true` vs `retried=false` ratio → how often G2's retry layer eats a flake vs lets it through.
- For `/canvas/rebuild` specifically: correlation between `canvasSize` and flake frequency → directly tests the "transaction-duration scales with payload → pgbouncer pressure" hypothesis.

That data drives the rate-fix design at the next investigation/fix session.

### What this session deliberately did NOT do

- **No code fix to the underlying flake rate.** Today is measurement-only. Per the standing recommendation for the most-thorough path, ship telemetry → wait for real data → propose a fix grounded in evidence.
- **No `withRetry` parity wrap of the unwrapped routes.** Tempting low-hanging fruit (option c on ROADMAP), but wrapping without measurement could ship a fix that addresses the wrong endpoints. Defer to next session after we see the data.
- **No Prisma client lifecycle change to `lib/db.ts`.** Same reasoning — possibly the right move, but speculative without measurement.
- **No Supabase plan tier change.** Director needs to share current plan tier first; that conversation happens at the next session.
- **No deploy.** Code is local; push gated on Rule 9 approval. Even after push, the new helper has no behavior change on success path — the only effect is additional log lines on errors. Safe to ship at director's discretion.

### Multi-workflow protocol coordination

- **Schema-change-in-flight flag:** stays `No` (no schema work this session).
- **Branch:** `main` (W#1's home).
- **Cross-workflow doc edits:** the platform-wide telemetry helper isn't W#1-specific; it touches the platform observability layer. The `PLATFORM_ARCHITECTURE.md §10` entry is added in this session's doc batch as the cross-workflow surface (every future workflow will benefit from the same telemetry).
- W#2 still 🆕 about-to-start; no parallel chat ran during this session.
- Pull-rebase at session start: clean (already up to date with `33ba13a` on origin/main; local `main` was 1 commit ahead with `9175ab5` from 2026-05-03-c, still unpushed).
- Pull-rebase before commit: standard end-of-session step.

### Files touched this session

**Code (new):**
- `src/lib/flake-counter.ts` (NEW, ~140 LOC)
- `src/lib/flake-counter.test.ts` (NEW, 28 tests)
...
**Docs (Group A + Group B):**
- `docs/KEYWORD_CLUSTERING_ACTIVE.md` (this STATE block prepended; prior 2026-05-03-c STATE block demoted to historical; header)
- `docs/ROADMAP.md` (Active Tools row updated for 2026-05-04; W#1 PRODUCTION-READINESS GATE prerequisite #3 status updated from ❌ NOT STARTED to 🟡 INSTRUMENTATION SHIPPED 2026-05-04 — pending live data + rate-fix design; "NEW HIGH — Underlying ~25% per-endpoint pgbouncer/Prisma flake rate" entry gains "Investigation Findings 2026-05-04" sub-section + recommended sequencing item 1 marked DONE; header)
- `docs/CHAT_REGISTRY.md` (new top row)
- `docs/PLATFORM_ARCHITECTURE.md` §10 (small note about flake-counter platform-wide observability addition)
- `docs/DOCUMENT_MANIFEST.md` (header timestamps + per-doc modified flags + this-session summary)

**Push status:** commit local; push pending director's Rule 9 approval at end-of-session. The push is purely additive on the success path — only the error-handling catch blocks change behavior, and only by emitting one extra structured log line per existing console.error. Safe to push whenever director chooses; the data-collection clock starts after push.

### Standing instructions for next session

The W#1 PRODUCTION-READINESS GATE — D3 RETRY entry on ROADMAP remains the milestone target. Prerequisite #1 (cold-start fix) is 🟡 PARTIAL — happy path live; banner UI folded into D3 run. Prerequisite #2 (recency-stickiness fix) is 🟡 CODE FIX SHIPPED 2026-05-03-c — pending live verification at next D3 attempt. Prerequisite #3 (this session's flake-rate work) is 🟡 INSTRUMENTATION SHIPPED 2026-05-04 — pending live data collection + rate-fix design. Next-session ordering:

(a) **Flake-rate analysis + first fix shipping — prerequisite #3 of the W#1 PRODUCTION-READINESS GATE — RECOMMENDED.** Director shares (1) current Supabase plan tier, (2) any other recent observations of mid-run flake patterns. Claude analyzes the `[FLAKE]` log lines from Vercel for the live data window between this session's push and the next session start (likely 1-3 days of normal director use). Output: a concrete fix proposal grounded in real per-endpoint flake counts + the canvasSize correlation. Most-likely-direction picks from ROADMAP options (a)–(d): Supabase plan-tier upgrade or pgbouncer-pool sizing change (highest-leverage if feasible), withRetry parity wrap of the hottest unwrapped endpoints (cheap and broad), Prisma client lifecycle change via `@prisma/adapter-pg` (medium-effort; medium-leverage), atomic-rebuild transaction differentialization (highest-effort; highest-leverage on the apply-pipeline specifically). ~1-2 sessions for first fix. **Most thorough next step** because the data window is the bottleneck — without it, any fix is a guess.

(b) D3 attempt now — skip the rate-fix step and accept the underlying ~25% pgbouncer flake rate as a known headwind. Faster path; ~$30-50 + 3-5 hr. The recency-stickiness fix (prerequisite #2) gets its live verification AND cold-start banner UI gets its natural-flake confirmation in the same run, but the overall run carries ~$20-30 in extra retry overhead and elevated mid-run-blanking risk. Also produces the first natural data window for `[FLAKE]` lines, which would inform any subsequent rate-fix session.
...
(f) **Action-by-action feedback workflow + Prompt Refining button — EXPLICITLY LAST per director's 2026-05-03-b directive.** ~5-7 sessions total (1 design + 4-6 build).

**Recommendation: (a) — flake-rate analysis + first fix.** Reason: with telemetry now shipping, the bottleneck shifts to "wait for real data → analyze → ship the highest-leverage fix grounded in evidence." Per the standing preference for the most-thorough-and-reliable path, close prerequisite #3 with measurement-driven evidence rather than guessing at the right fix or skipping the gate. Director's framing 2026-05-02-e: *"All these issues need to be fixed."*

**Director's framing through prior sessions:** (Scale-A) → (Scale-0) → (Defense-in-Depth ×3) → (Scale-B) → (Scale-C) → (Scale-D) → (Scale-E build) → (Scale-E D3 partial validation) → (consolidation auto-fire follow-up `2026-05-01-c`) → (HTTP 500 fix verification + auto-fire trip observation `2026-05-02`) → (browser-freeze fix design `2026-05-02-b`) → (DevTools profiling pass — diagnosis rejected `2026-05-02-c`) → (HTTP 500 retry regression investigation — fix shipped `2026-05-02-d`) → (HTTP 500 fix live verification + cold-start canvas-empty finding `2026-05-02-e`) → (cold-start render-layer fix `2026-05-03`) → (cold-start fix live verification — partial `2026-05-03-b`) → (recency-stickiness fix `2026-05-03-c`) → **(underlying flake-rate investigation — instrumentation, this session `2026-05-04`)** → (flake-rate analysis + first fix — RECOMMENDED next).

---

## ⚠️ POST-2026-05-03-C-RECENCY-STICKINESS-FIX STATE (preserved as historical context — last updated 2026-05-03-c; SUPERSEDED by the underlying-flake-rate-investigation state above. Recency-stickiness fix code remains shipped; live verification still deferred to next D3 attempt.)
