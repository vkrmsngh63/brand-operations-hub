...
**Last updated:** May 4, 2026-c (D3 retry attempt — partial; pool-exhaustion finding ON PRO TIER — third session of 2026-05-04, follow-up to `session_2026-05-04-b_rate-fix-first-pass`. **DOC + LIVE-RUN session — no code changes** but a SIGNIFICANT FINDING surfaced live on production vklf.com. **CORRECTION post-original-commit:** director clarified at end-of-session that the Supabase Pro upgrade had ALREADY happened at end of 2026-05-04-b session (the prior STATE block's "director confirmed NOT on Pro" was stale by 2026-05-04-c start). This MEANS: tonight's pool exhaustion fired ON PRO TIER, not Free — a more serious structural finding than initially scoped. D3 retry launched after the 2026-05-04-b rate-fix shipped. Batch 1 ran cleanly model + apply-pipeline side ($0.312, 15 ops applied, canvas at 124 nodes, 0 new sister links — quiet confirmation V4 prompt paste was correct). Post-rebuild UI refresh exhausted three retry attempts on HTTP 500; the post-rebuild fetch retry helper from 2026-05-02-d paused the run cleanly; Vercel logs revealed root cause: **`FATAL: Max client connections reached`** — Supabase Postgres connection pool exhaustion (ON PRO TIER) under the apply-pipeline's parallel keyword PATCH burst (~22 PATCH /keywords/[id] failures within ~0.1 sec at 8:30:40 PM, plus 4 GET /canvas + 2 GET /canvas/nodes failures, all same error). **Different failure mode than the 2026-05-04-b rate-fix targets** — pool exhaustion has no Prisma error code (just the FATAL message); withRetry retries P1001/P1002/P1008/P2034 transient codes; retrying pool exhaustion makes it worse. **Implication of pool exhaustion ON Pro tier:** Pro upgrade alone is not sufficient — either (a) Pro's default pool size needs tuning (Pro exposes a configurable knob; default may be too small for our ~22-parallel-PATCH burst pattern), OR (b) the apply-pipeline's parallel-burst pattern itself needs reduction regardless of pool size, OR (c) some combination. Director chose Path B at the in-session decision fork: pause D3 here. Next-session task pivots from "D3 retry after Pro upgrade" to "pool-exhaustion investigation on Pro tier — verify Pro active + check pool config + tune OR ship parallel-burst reduction code-fix; D3 retry follows after that". **Open question — `[FLAKE]` line visibility:** flake-counter telemetry shipped 2026-05-04 should have logged a `[FLAKE]` line for every catch-block error; director's Vercel paste showed full stack traces but NO line beginning with `[FLAKE]` — could be Vercel UI display issue OR a wiring gap; investigation deferred to next-session item (b). W#1 PRODUCTION-READINESS GATE prerequisite #3 status reframed: covers transient flakes (still needs natural-data verification); does NOT cover pool exhaustion. **NEW prerequisite #4 added:** Supabase Pro upgrade for pool-capacity headroom. Multi-workflow: schema-change-in-flight stays "No"; W#2 still 🆕; no parallel chat. Pull-rebase clean. Doc-only commit; push pending Rule 9 approval.)
...
## ⚠️ POST-2026-05-04-C-D3-RETRY-PARTIAL-POOL-EXHAUSTION-FINDING STATE (READ FIRST — updated 2026-05-04-c)
