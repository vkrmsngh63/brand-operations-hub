...
- **READ SURFACE:** GET `/api/projects/[projectId]/audit-events` — recent history, newest-first. **The Action-History UI tab is now BUILT + DEPLOYED** [slice 3, 2026-06-04-c] — a read-only filterable grouped panel (`HistoryPanel.tsx` + the pure `audit-labels.ts` `describeEvent()` plain-English renderer) reading this GET.
- **KNOWN DATA-COMPLETENESS LIMITATION (slice 3 review, 2026-06-04-c — fix scheduled for slice 4):** the recorded `payload` is **AFTER-ONLY for AI operations + for all rows recorded before slice 3** — the before-state was never captured for AI ops (the pure operation-applier exposes no per-op diffs — see `src/lib/audit-payload.ts` lines 22-27), and the from→to capture added in `5f7aba0` covers ONLY FUTURE MANUAL edits. As a result the History panel can render a change's "to" value but not its "from" for AI/older rows, and a rename row lacks a stable human-anchorable identity (WHICH topic). The director reviewed the deployed panel on real Chrome and (correctly) flagged this as insufficient context. **The fix = H-1 slice 4 (before-state + which-item enrichment for AI ops during the apply pass) — the SAME before-state the future per-action Undo needs; anticipated to STAY schema-NO (richer `payload` JSON into the existing column, not a new schema).**
