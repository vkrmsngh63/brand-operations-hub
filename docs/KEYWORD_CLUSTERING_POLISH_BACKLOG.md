...
- (c) **SLICE 3 — NEXT, (a.141): the Action History UI tab in the AST panel** — a chronological, filterable list of every op (AI + manual) reading the GET `/api/projects/[projectId]/audit-events` endpoint; makes the recorded history VISIBLE for the first time. A PURE READ of the existing GET (NO schema change, very likely NO new route). **Plan the UI shape WITH the director BEFORE coding** (audience / plain change-type labels / columns / filters by source [AI vs manual] + change type / placement) per `feedback_plan_output_shape_before_building` — this is a user-facing screen. The director chose this at the §4 Step 1c forced-picker ("H-1 slice 3 — History screen (Recommended)").
- (d) Per-action undo design session BEFORE build — some ops easy reverses (ADD_TOPIC ↔ DELETE_TOPIC), others compose nontrivially (SPLIT_TOPIC, MERGE_TOPICS with auto-reparenting). FUTURE slice (slice 4+). Will also need the before-state enrichment that slice 2 left as after-only snapshots.
