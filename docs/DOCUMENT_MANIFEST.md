...
| 6 | `ROADMAP.md` | Development execution plan — completed work + remaining phases | 2026-05-01-b (Scale Session E D3 partial validation — Active Tools row replaced with D3 outcome summary + 6-option Next-Session menu (consolidation follow-up recommended). SIX new ROADMAP entries appended: bulk-delete `-` icon error bug, Delete All Selected feature on every keyword table, sister-link op deferral architectural item, Q5 → B touch-semantics refinement architectural item, pre-flight consolidation-prompt-check polish, Bursitis Test naming clarification. Header timestamp.) | ✅ YES |
| 7 | `CORRECTIONS_LOG.md` | Append-only log of mistakes + extracted patterns | 2026-05-01-b (Scale Session E D3 partial validation — TWO new entries prepended: HIGH-severity decideTier dormant-stability force-pin design-implementation mismatch surfaced mid-D3-run + INFORMATIONAL Supabase HTTP 500 errors on fetchCanvas during batch 17 retry storm. Header timestamp.) | ✅ YES |
| 8 | `CHAT_REGISTRY.md` | Chronological log of chats + URLs + work-summaries (post-Ckpt-9: Claude Code sessions use session-identifier format) | 2026-05-01-b (Scale Session E D3 partial validation — new top row for session_2026-05-01-b_scale-session-e-d3-validation; twenty-ninth Claude Code session. Header timestamp.) | ✅ YES |
...
| 12 | `DOCUMENT_MANIFEST.md` | This file — ground-truth doc registry | 2026-05-01-b (Scale Session E D3 partial validation — header timestamps + per-doc Modified-this-session flags + this-session summary updated to reflect D3 outcome.) | ✅ YES |
