...
| 2 | `PLATFORM_ARCHITECTURE.md` | Technical architecture — routes, schema, auth, file structure, tech debt | 2026-05-04-b (Rate-fix first pass — §10 gains "🛡️ Server-side withRetry parity — first-pass closed (SHIPPED 2026-05-04-b)" entry capturing the silent-helper wrap in `lib/auth.ts` + `lib/workflow-status.ts` + apply-pipeline writes wrap across ~9 route files. Combined coverage ~4% → ~80%+ of authenticated paths. Idempotency tradeoff for unique-constraint-free creates documented as accepted vs 500. Header timestamp updated. Prior 2026-05-04 entry preserved.) | ✅ YES |
...
| 6 | `ROADMAP.md` | Development execution plan — completed work + remaining phases | 2026-05-04-b (Rate-fix first pass — Active Tools row updated for 2026-05-04-b with full session summary + Next Session list reordered: (a) D3 retry attempt RECOMMENDED (was "skip prerequisite #3" track on prior list — promoted now that all prerequisites have shipped code), (b) wait 1-3 days for natural data, (c) wrap remaining unwrapped routes, (d) open second rate-fix track — Prisma lifecycle or Supabase plan-tier, (e-h) other items unchanged; W#1 PRODUCTION-READINESS GATE prerequisite #3 status updated from 🟡 INSTRUMENTATION SHIPPED 2026-05-04 to 🟡 FIRST RATE-FIX SHIPPED 2026-05-04-b — pending live verification at next D3 attempt OR via natural [FLAKE] data window; gate completion estimate revised to "1 session remaining (D3 attempt itself)"; "🟡 INSTRUMENTATION SHIPPED 2026-05-04" flake-rate entry header changed to "🟡 FIRST RATE-FIX SHIPPED 2026-05-04-b" with new "Status update 2026-05-04-b" sub-section; recommended sequencing item 2 marked DONE. Header timestamp.) | ✅ YES |
| 7 | `CORRECTIONS_LOG.md` | Append-only log of mistakes + extracted patterns | 2026-05-03 (cold-start render-layer fix — ONE new INFORMATIONAL entry: React `react-hooks/purity` rule traces through same-file useCallback into setState calls, even post-await; mid-session 3-error lint regression on KeywordWorkspace fixed with IIFE wrapper. Reporting-precision lesson on the rule's tracing scope; NOT a mistake. Not modified this session.) | NO |
| 8 | `CHAT_REGISTRY.md` | Chronological log of chats + URLs + work-summaries (post-Ckpt-9: Claude Code sessions use session-identifier format) | 2026-05-04-b (Rate-fix first pass — header updated with fortieth Claude Code session summary; prior 2026-05-04 row demoted to "Previously updated"; new top row added in registry table.) | ✅ YES |
...
| 12 | `DOCUMENT_MANIFEST.md` | This file — ground-truth doc registry | 2026-05-04-b (Rate-fix first pass — header timestamps + per-doc Modified-this-session flags + this-session summary updated.) | ✅ YES |
...
- `docs/KEYWORD_CLUSTERING_ACTIVE.md` — new POST-2026-05-04-B-RATE-FIX-FIRST-PASS STATE block prepended; prior POST-2026-05-04-UNDERLYING-FLAKE-RATE-INVESTIGATION STATE block demoted to historical; header timestamp.
