...
- **P-49 W#2 Reviews Phase 2 (IN-FLIGHT — Fix Sessions A + B ✅ DEPLOYED; Fix Session C + NEW Fix Session D + Category Sessions 1-3 + Type Sessions 4-5 remaining)** — remaining work: Fix Session C on the Reviews Analysis Table page (Fix C this upcoming session — non-bulleted flow + Excel + drag + 1 more schema column) + **NEW Fix Session D** (captured 2026-05-30 from director's §1 addendum — restructure the "Overall Analysis — Captured Reviews" box from free-text bullets into a 3-column traceability table: category / bullet-point critique / ALL associated captured reviews + star counts; needs a per-competitor bulleted prompt redesign to emit structured category→bullet→source-reviewId output + box rendering change; slotted AFTER Fix Session C per director's sequencing picker; full spec in `docs/polish-item-specs/P-49-W5-S2-S3-competitor-reviews-analysis.md` §1 addendum + §3 "Fix Session D") + the 5-session corrective rebuild from 2026-05-28 (Category Sessions 1-3 + Type Sessions 4-5) pushed back behind the Fix Sessions + Phase 4 verification per session + opportunistic polish. Estimate ~7 more sessions until P-49 W5 closes (Fix C + Fix D + Category 1 + Category 2 + Category 3 + Type 4 + Type 5).
