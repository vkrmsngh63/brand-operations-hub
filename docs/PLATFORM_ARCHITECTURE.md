...
**Activity tracking side-effect (post-Ckpt-5; extended 2026-05-07 for W#2):** All KC workflow-data routes (keywords, canvas nodes, pathways, sister-links, rebuild) call `markWorkflowActive(projectId, "keyword-clustering")` after successful mutations; W#2 url-route mutations (POST `.../urls`, PATCH/DELETE `.../urls/[urlId]`) call `markWorkflowActive(projectId, "competition-scraping")`. This flips `ProjectWorkflow.status` from `"inactive"` to `"active"` on first meaningful use and refreshes `lastActivityAt`. GET endpoints and canvas state (pan/zoom) do NOT trigger activity tracking. The project-scoped vocabulary route does NOT call `markWorkflowActive` — it lacks a single workflow context (any workflow can call it); the calling workflow's own routes handle activity stamping.
