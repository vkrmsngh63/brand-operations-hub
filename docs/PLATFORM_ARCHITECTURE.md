...
**Last updated:** April 29, 2026 (Defense-in-Depth Audit Implementation Session 1 — Option β. Two new pure helpers in `src/lib/`: `canvas-rebuild-guard.ts` (G1 payload-sanity for `/canvas/rebuild`, 50% shrink threshold) + `prisma-retry.ts` (G2 retry-on-transient for Prisma codes P1001/P1002/P1008/P2034 with backoff [100ms,500ms]). New top-level folder `eslint-rules/` with custom rule `no-prop-reads-in-runloop.js` codifying the AutoAnalyze.tsx:163 invariant as a build-time gate; wired into `eslint.config.mjs` as a local plugin. `/canvas/rebuild` POST + `/canvas/nodes` GET routes hardened with the new guards (additive — happy-path callers unchanged). `AutoAnalyze.tsx` gained 4× `@runloop-reachable` annotations + R2 post-Reconcile-Now diff-empty WARN. 30 new src/lib unit tests + 13 ESLint rule tests; build clean.)
