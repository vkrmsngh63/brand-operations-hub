...
**Last updated:** May 4, 2026-c (D3 retry attempt — partial; pool-exhaustion finding — third session of 2026-05-04, follow-up to `session_2026-05-04-b_rate-fix-first-pass`. **§10 gains "🚨 Connection pool exhaustion under apply-pipeline burst (DISCOVERED 2026-05-04-c)"** entry capturing the discovered failure mode: ~22 parallel PATCH /keywords/[id] calls per batch overwhelm Supabase Free-tier pool capacity → `FATAL: Max client connections reached` returned by Postgres. Different failure mode than the 2026-05-04-b rate-fix targets (no Prisma error code; just FATAL message). Three-layer fix: Supabase Pro upgrade (primary), apply-pipeline parallel-PATCH-burst reduction (companion), pre-flight Pro-tier check (low priority). Phase 3 scale implication: 50 concurrent workers will hit pool exhaustion much harder than today's solo director — Pro upgrade necessary but not sufficient. Header timestamp.)
