...
**Last updated:** April 28, 2026 (Scale Session 0 — empirical validation; Outcome C fired; §10 Known Technical Debt updated to (1) reflect Scale Sessions B–E now activated as the build path AND (2) add NEW HIGH-severity entry for canvas-blanking bug observed at batches 70 + 134 of the 2026-04-28 full-Bursitis V3 run.)
...
- 🚨 **Canvas-Blanking Intermittent Bug (NEW 2026-04-28; HIGH severity; investigation pending).** Empirically observed twice in the 2026-04-28 full-Bursitis V3 run on Sonnet 4.6 (batches 70 and 134 of 151 batches). Symptom: between batch N and batch N+1, the wiring layer sent ~19,929 tokens of input to Anthropic instead of the expected ~80-200k tokens — canvas state was missing from the prompt. Model rebuilt a small canvas from scratch using only the 8 keywords in that batch. Reconciliation pass correctly caught the symptom and flagged 84 keywords/event as Reshuffled (off-canvas). **Cascade:** 168 keywords across the 2 events silently abandoned for the rest of the run because batch queue is fixed at run-start. **Likely cause space (not yet diagnosed):** React state staleness between batches (`nodesRef.current` momentarily empty when `buildOperationsInputTsv` is called); server-side rebuild API race; cancel/restart artifact; Anthropic prompt-cache mis-handling. **Investigation plan:** code reading on `src/lib/auto-analyze-v3.ts` `buildOperationsInputTsv` + `AutoAnalyze.tsx` `runLoop` + canvas-rebuild API + DB query for actual canvas state right now + tracing how refs are populated/refreshed between batches. **Fix design TBD pending diagnosis.** Cross-references: `ROADMAP.md` "Canvas-Blanking Intermittent Bug" section + `KEYWORD_CLUSTERING_ACTIVE.md` POST-2026-04-28-SCALE-SESSION-0-OUTCOME-C STATE block. RELATED to but DISTINCT from P3-F7 silent-placements + ghost-AI-Sorted bugs (P3-F7 was about status-flip drift in `doApply`; this is about the wiring layer not sending canvas state to the model in the first place).
