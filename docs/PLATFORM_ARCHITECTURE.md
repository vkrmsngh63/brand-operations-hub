...
**Last updated:** May 13, 2026 (**Platform-UI fix-pair SHIPPED on `main` — ROADMAP Active Tools (a.12) closed.** Single code commit covers two director-flagged items surfaced during the 2026-05-12-b W#2 → main deploy session #6 browser verification. **(i) W#2 workflow card click handler.** `src/app/projects/[projectId]/page.tsx` `WORKFLOW_DEFS[1]` flipped from `{ active: false, route: null }` to `{ active: true, route: "competition-scraping" }`. The existing `handleWorkflowClick` callback already routes via `active + route`, so the one-line data change is the full fix — clicking the Competition Scraping card on the per-Project workflow grid now navigates to `/projects/[id]/competition-scraping` instead of the Ckpt 9.5-era "coming soon" toast. §3 routes table `/projects/[projectId]` row updated to reflect the dual-active state (Keyword Analysis + Competition Scraping). **(ii) `/plos` reshape via Rule 14f forced-picker — director picked Option A: server-side redirect to `/projects`.** Original 1,525-line workflow-cards landing replaced with a five-line `redirect("/projects")` server component (`next/navigation` per `node_modules/next/dist/docs/01-app/03-api-reference/04-functions/redirect.md` — 307 temporary redirect by default). Every entry point to `/plos` (Dashboard PLOS card, typed URL, bookmark, `/plos/notes` back-button) now lands on `/projects`. §3 routes table `/plos` row updated. Original landing preserved in git at commit `23a5985`; restorable via `git show 23a5985:src/app/plos/page.tsx`. **Follow-on edit (Rule 15 autonomous, Rule 10 flagged):** `/projects` page's "← Back" button retargeted from `/plos` to `/dashboard` so that clicking Back from `/projects` no longer no-op-loops via the redirect. Caught + corrected in-session before push as a small slip from the Option-A description's "no orphaned links elsewhere" claim. **Verification scoreboard:** root tsc + build clean (51 routes — exact baseline parity; `/plos` still in route list as a static-prerendered redirect). No schema changes. No §10 changes. No new API surface. Header timestamp.)
**Last updated in session:** session_2026-05-13_w2-card-click-and-plos-redirect (Claude Code, on `main` branch)
**Previously updated:** May 7, 2026-h (**W#2 Chrome extension build session 3 SHIPPED — Module 1 URL-capture content script for the 4 shopping platforms (Amazon, Ebay, Etsy, Walmart).** §1 directory structure: extended the `extensions/competition-scraping/` tree with new `src/lib/platform-modules/` folder (per-platform DOM-pattern modules per `STACK_DECISIONS §15 Q7` — 4 modules today + registry routing), new `src/lib/content-script/` folder (vanilla DOM components for floating "+ Add" button + URL-add overlay form + "already saved" recognition icon + detail-page overlay + orchestrator + typed messaging), new `src/lib/url-normalization.ts` (comparison-time `?`-stripping per `COMPETITION_SCRAPING_DESIGN.md §B` 2026-05-07-g end-of-session addendum item 3), new `src/entrypoints/content.ts` (WXT `defineContentScript` matching the 4 platform sites), and updated `src/entrypoints/background.ts` (chrome.contextMenus registration + onClicked routing per §5 guardrail #6). §3 routes table: zero PLOS-side route changes this session. **Verification scoreboard:** extension `npm run compile` clean; extension `npm test` 146/146 pass (104 new across url-normalization + 4 platform modules + registry); extension build clean (~822 KB unpacked: prior ~603 KB + ~219 KB new content-scripts/content.js chunk); extension lint clean; root tsc + build clean (49 routes — same baseline; zero new); 393/393 src/lib tests pass — exact baseline parity; lint 13e/39w — exact baseline parity. Header timestamp.) **Previously updated:** May 7, 2026-f (**W#2 Chrome extension build session 1 SHIPPED — WXT init + auth shell + GET /api/projects CORS.** Single code commit `5b4a3e8` on `workflow-2-competition-scraping`. **CODE session — no schema, no DB writes, no live run, no deploy. Manual smoke test deferred per director's mid-session directive to defer ALL manual testing to ONE dedicated post-coding verification session.** §1 / §2 directory structure: NEW top-level `extensions/` folder for non-web-app PLOS clients; first occupant `extensions/competition-scraping/` with WXT 0.20 + React 19 + Supabase JS, npm package manager (deviates from `STACK_DECISIONS §12.2` pnpm prescription; deviation captured in `COMPETITION_SCRAPING_DESIGN.md §B`). §3 routes table: `/api/projects` row updated — added OPTIONS handler + `withCors` wrap on GET so the extension's `Authorization: Bearer` preflight passes; same-origin web callers unaffected (the cross-workflow edit was surfaced explicitly per `MULTI_WORKFLOW_PROTOCOL §3` discipline since `/api/projects` is shared infrastructure used by W#1 too, then taken as a Rule-15 autonomous call since technically required for any non-web-app client to ever call the endpoint). Root `tsconfig.json` updated — `extensions` added to `exclude` so root tsc doesn't try to type-check extension-specific globals (chrome namespace, defineBackground, etc.) — each non-web client has its own tsconfig. **Verification scoreboard:** extension build clean (`.output/chrome-mv3/` produced + `competition-scraping-extension-0.1.0-chrome.zip` ~590 KB); root PLOS tsc + build clean (49 routes — same baseline; CORS edits added zero new routes); 393/393 src/lib tests pass — exact baseline parity; lint project-wide 13e/39w — exact baseline parity. No schema changes; no §10 changes. Header timestamp.)
...
**Previously updated:** May 7, 2026-b (**W#2 PLOS-side viewer slice (a.2) SHIPPED — image gallery + full-size viewer modal on the `/url/[urlId]` detail page.** Single code commit `2ab891e` on `workflow-2-competition-scraping`. **CODE session — no schema, no DB writes, no live run, no deploy.** §3 routes table: existing `urls/[urlId]/images` GET row updated — server-side now mints both a 200×200 contain-fit thumbnail signed URL (Supabase on-the-fly transform) and a full-size signed URL per row (1-hour TTL each), embedded in the response so the gallery + modal render in a single round-trip. Shared types `src/lib/shared-types/competition-scraping.ts` extended additively: new `CapturedImageWithUrls = CapturedImage & { thumbnailUrl, fullSizeUrl }`; `ListCapturedImagesResponse` re-pointed to that array shape. Bare `CapturedImage` unchanged for PATCH / finalize / etc. New client component `ImageViewerModal.tsx` (full-size image left + metadata sidebar right with Category / Source / Dimensions / Added on / Composition / Embedded text / Tags; arrow-key prev/next; Esc + backdrop + ✕ close paths; body scroll-lock; close-button auto-focus on mount; key-prop remount on prev/next instead of synchronous setState-in-effect to satisfy the same react-hooks/purity rule that slice (a.1) flagged). `UrlDetailContent.tsx` `ImageCountPlaceholder` swapped for `CapturedImagesGallery` (200×200 thumbnail grid via `<img loading="lazy">` with onError fallback to a "Image failed to load" red tile; "screenshot" badge marks `sourceType === "region-screenshot"` rows; click-thumbnail-to-open-modal). Visual verification deliberately deferred per the standing 2026-05-07 director call until the Chrome extension provides a data path; slice (a.2) verification section appended to `docs/COMPETITION_SCRAPING_VERIFICATION_BACKLOG.md` with 14 walked-through tests + seed-data prerequisites. No new routes; no new pages; no auth surface change. **Verification scoreboard:** 393/393 src/lib tests pass (no test changes); tsc clean; build clean (49 routes; same as slice (a.1)); lint at exact baseline parity (16e/40w; zero new errors or warnings). No schema changes; no §10 changes. Header timestamp.)
...
**Previously updated:** May 7, 2026 (**W#2 PLOS-side viewer slice (a.1) SHIPPED — `/url/[urlId]` detail page + 4 new GET read paths.** Single code commit `439a240` on `workflow-2-competition-scraping`. **CODE session — no schema, no DB writes, no live run, no deploy.** §3 routes table gains: 4 new W#2 GET handlers — `urls/[urlId]` (read single URL — extends existing route file with previously POST/PATCH/DELETE-only handlers), `urls/[urlId]/text` (list captured text rows for one URL — extends), `urls/[urlId]/sizes` (list sizes for one URL — extends), `urls/[urlId]/images` (list captured images for one URL — NEW route file under `urls/[urlId]/images/` since the path didn't exist as a collection-level route before; sibling to the existing `requestUpload` + `finalize` per-action sub-routes); plus 1 new page route `/projects/[projectId]/competition-scraping/url/[urlId]` (the per-URL detail page). All 4 GETs apply the existing W#2 §11.2 cross-cutting items uniformly: `verifyProjectWorkflowAuth("competition-scraping")` + projectWorkflowId scoping (forged cross-Project urlIds return 404 not row data); `withRetry()` on every Prisma call; `recordFlake()` on every catch; `chrome-extension://*` CORS preflight via OPTIONS handler. No `markWorkflowActive` on GETs (read-only paths don't bump workflow status). Activity-tracking note in §3 unchanged. New page composes `useWorkflowContext()` + `<WorkflowTopbar>` from the components library; renders an in-page `UrlDetailContent` component with parallel four-fetch + cancelled-flag race guard + sub-breadcrumb + URL metadata read-only grid + sizes sub-section + sortable captured-text table + image-count placeholder + "Open original URL ↗" button preserving the prior new-tab affordance. Shared types at `src/lib/shared-types/competition-scraping.ts` extended with `ReadCompetitorUrlResponse` + `ListCompetitorSizesResponse` + `ListCapturedTextsResponse` + `ListCapturedImagesResponse` (additive). UrlTable click-row rewired in CompetitionScrapingViewer parent: `window.open(url, '_blank')` → `router.push('/url/[urlId]')`; the prior new-tab open is preserved as an explicit button on the detail page itself. **Visual verification deliberately deferred per director's call** until the Chrome extension provides a data path; the 12-step pending checklist + seed-data prerequisites are captured in NEW Group B doc `docs/COMPETITION_SCRAPING_VERIFICATION_BACKLOG.md` (per-slice append-only). **Verification scoreboard:** 393/393 src/lib tests pass; tsc clean; build clean (49 routes; was 47 — +1 page route + 1 API list route); lint at exact baseline parity (16e/40w; zero new errors or warnings). No schema changes; no §10 changes. Header timestamp.)
...
**Previously updated:** May 8, 2026 (**W#2 API-routes session-3 SHIPPED — third and final slice of the recommended 3-session split for §11; closes out the §11.1 route table.** 2 new W#2 API routes added to §3 routes table: `/api/projects/[projectId]/competition-scraping/reset` (POST — admin-only via `verifyProjectAuth` Project-ownership gate; type-the-project-name `confirmProjectName` body guard; DB transaction deletes the W#2 ProjectWorkflow row [cascades CompetitorUrl + CompetitorSize + CapturedText + CapturedImage] + W#2-originated VocabularyEntry rows [other workflows' entries preserved per §A.11 + `PLATFORM_REQUIREMENTS.md §8.4`]; storage wipe via `wipeProjectImages()` helper outside the transaction with janitor backstop on partial failure); `/api/cron/competition-scraping-janitor` (GET — daily Vercel cron at 03:00 UTC per `COMPETITION_SCRAPING_STACK_DECISIONS.md §3`; auth via `Authorization: Bearer ${CRON_SECRET}`, denies all requests when env var unset so the path can't be hit publicly; reads `CapturedImage.storagePath` rows into a Set, walks the bucket via new `listAllStorageImages()`, classifies orphans via pure `findOrphans()` helper [no DB row + createdAt > 24h = orphan; younger files in grace window for in-flight uploads], bulk-deletes via new `deleteStorageImages()`). Activity-tracking note in §3 left unchanged — both new routes are admin/cron paths that don't surface to workers and don't bump workflow status. New top-level repo file `vercel.json` with single `crons` entry (`/api/cron/competition-scraping-janitor` at `0 3 * * *`). New env var `CRON_SECRET` added to §9 (server-side; Vercel sends it as Bearer token automatically when set; janitor denies all requests if unset). `competition-storage.ts` extended with `listAllStorageImages()` + `deleteStorageImages()` (per §10.4 portability — all Supabase Storage SDK calls stay in this file); `competition-storage-helpers.ts` extended with pure `findOrphans()` + `ORPHAN_GRACE_MS` constant (11 new unit tests). §7 storage-bucket entry's "Janitor" sub-bullet flipped from "session-3 work" to "SHIPPED 2026-05-08." Page wiring at `src/app/projects/[projectId]/competition-scraping/page.tsx`'s `handleResetConfirm` flipped from throw → `authFetch` POST + page reload on success. **Verification scoreboard:** 380/380 src/lib tests pass (was 369 baseline on this branch; +11 new from `findOrphans` tests; note: prior session-2 doc said "382" but actual baseline on branch is 369 — small drift captured in `CORRECTIONS_LOG.md`). tsc clean. build clean (47 routes; was 45 — added `reset` + `cron/competition-scraping-janitor`). lint at exact baseline parity (16e/40w; zero new errors or warnings). No schema changes; no §10 changes. Header timestamp.)
**Last updated in session:** session_2026-05-08_w2-api-routes-session-3 (Claude Code, on `workflow-2-competition-scraping` branch)
**Previously updated:** May 7, 2026 (**W#2 API-routes session-2 SHIPPED — second slice of the recommended 3-session split for §11.** 8 new W#2 API routes added to §3 routes table: `urls/[urlId]/sizes` (POST/OPTIONS); `sizes/[sizeId]` (PATCH/DELETE/OPTIONS); `urls/[urlId]/text` (POST/OPTIONS — idempotent on clientId); `text/[textId]` (PATCH/DELETE/OPTIONS); `urls/[urlId]/images/requestUpload` (POST/OPTIONS — Phase 1 of two-phase upload, signed Supabase Storage URL, no DB write); `urls/[urlId]/images/finalize` (POST/OPTIONS — Phase 2, verifies file landed before DB row creation, idempotent on clientId); `images/[imageId]` (PATCH/DELETE/OPTIONS — DB + best-effort storage cleanup); `reconcile` (GET/OPTIONS — periodic 5-min reconciliation per stack-decisions §8.3.2). Two §11.1 spec deviations decided autonomously per Rule 15 + flagged via deferred TaskList: image RPC paths reshaped from colon-suffix `images:requestUpload`/`images:finalize` to slash-based `images/requestUpload`/`images/finalize` (Next.js folder convention + repo precedent + URL-encoding cleanliness); finalize body extended with mimeType + sourceType (required) + fileSize + imageCategory (optional). Both echoed from extension's WAL — server re-derives storagePath rather than trusting client-supplied path. New §7 Shared systems entry "Storage: `competition-scraping` bucket" — private, signed-URL-only, helper wrapper at `src/lib/competition-storage.ts` per §10.4 portability requirement; pure helpers split into `src/lib/competition-storage-helpers.ts` (22 unit tests). Bucket created via Supabase dashboard with director walkthrough at session-2 start. Activity-tracking side-effect note in §3 extended to cover all session-2 mutation routes (sizes/text/images-finalize/images-PATCH-DELETE) — `images/requestUpload` is read-only-from-DB-perspective so it doesn't trigger activity tracking. No schema changes; no §10 changes. Header timestamp.)
...
**Previously updated:** May 7, 2026 (**W#2 API-routes session-1 SHIPPED — first slice of the recommended 3-session split for §11.** 3 new W#2 API routes added to §3 routes table: `/api/projects/[projectId]/competition-scraping/urls` (GET / POST / OPTIONS); `/api/projects/[projectId]/competition-scraping/urls/[urlId]` (PATCH / DELETE / OPTIONS); `/api/projects/[projectId]/vocabulary` (GET / POST / OPTIONS — project-scoped, used by W#2 today + future workflows per `PLATFORM_REQUIREMENTS.md §8.4`). All routes apply §11.2 cross-cutting items uniformly: `verifyProjectWorkflowAuth` for urls / `verifyProjectAuth` for vocabulary; `withRetry()` on every Prisma call; `markWorkflowActive("competition-scraping")` after url mutations; `recordFlake()` on every catch; `chrome-extension://*` CORS preflight via OPTIONS handler. New shared-types module `src/lib/shared-types/competition-scraping.ts` (single source of truth for both PLOS routes + future Chrome extension per stack-decisions §12.3) + new pure CORS helper `src/lib/cors.ts` + Next-aware response factories `src/lib/cors-response.ts`. Activity-tracking side-effect note in §3 extended to cover W#2 url routes. No schema changes; no §10 changes. Header timestamp.)
**Previously updated:** May 6, 2026 (**W#2 schema landed in live Supabase Pro DB.** First W#2 PLOS-side implementation session. 7 new tables (CompetitorUrl, CompetitorSize, CapturedText, CapturedImage, VocabularyEntry, WorkerAssignment Phase 2, AuditEvent Phase 3) + 2 back-relations (Project.vocabularyEntries, ProjectWorkflow.competitorUrls) per `COMPETITION_SCRAPING_STACK_DECISIONS.md §9.1` pushed via `prisma db push` (additive only; W#1 tables untouched; no `--force-reset`). New page at `/projects/[projectId]/competition-scraping` (~200 LOC) composing 7 library components per `COMPETITION_SCRAPING_DESIGN.md §A.14`. §3 routes table updated with the new W#2 page route. §10 known tech debt: `Assignment` table entry refined — schema for the cross-workflow `WorkerAssignment` table is now in the DB (designed for Phase 2; W#2 is the first declarer); `AuditEvent` table entry refined — schema is now in the DB (designed for Phase 3; W#2 is the first declarer); both still NOT YET wired into a Phase 2 / Phase 3 build. Header timestamp.)
**Previously updated:** May 5, 2026-c (**Shared Workflow Components Library — Phase-1 SHIPPED.** Cross-workflow / platform-wide build session on `main`. New top-level architecture surface: library lives at `src/lib/workflow-components/` with barrel export at `@/lib/workflow-components`. 11 source files (1 hook + 8 React components + 2 pure-logic helpers + 1 types module + 1 barrel) + 2 unit-test files (13 tests) + 1 smoke-test page at `src/app/components-smoke-test/page.tsx`. §10 line 477 placeholder flipped from "designed, build pending" → "✅ Phase 1 SHIPPED 2026-05-05-c." W#2's PLOS-side build is structurally unblocked. Phase 2 components (`<AdminReviewControls>`, `useEmitAuditEvent()`) remain — built when Phase 2 turn-on is scheduled. Header timestamp.)
**Last updated in session:** session_2026-05-05-c_components-library-phase-1-build (Claude Code, on `main`)
**Previously updated:** May 5, 2026 (Atomic-batch fold-in shipped + verified live across 50 batches + 5 consolidations on production vklf.com — `0caa200`. **§10 "Connection pool exhaustion under apply-pipeline burst" entry marked ADDRESSED for Phase 1:** approach (iii) atomic-batch-into-rebuild-transaction shipped per the 2026-05-04-d standing instructions. The 55-60+ parallel PATCH burst that exhausted Supabase max_connections=60 is structurally GONE — server-side `/canvas/rebuild` route now folds keyword updates + archives into the same `$transaction` as the canvas rebuild (extended to `{timeout: 30000, maxWait: 5000}`); client-side `AutoAnalyze.tsx doApplyV3` removes the fire-and-forget per-id PATCH burst and the sequential archive POST loop. P2025 race condition (the 2026-05-04-d MEDIUM finding) eliminated for free. **Phase 3 scaling concern flagged for follow-up:** at canvas 700+ × 50 workers, the single-transaction connection-hold model becomes its own ceiling — captured as deferred architectural item. Header timestamp.)
**Last updated in session:** session_2026-05-05_atomic-batch-fold-in-shipped-and-verified (Claude Code)
**Previously updated:** May 4, 2026-d (Pool-tune small-batch test — INSUFFICIENT — fourth session of 2026-05-04. **§10 "Connection pool exhaustion" entry updated with burst-size correction:** the actual parallel-PATCH burst is **55-60+ per batch, NOT the 22 estimated 2026-05-04-c.** Vercel log expansion in tonight's session counted ~40 successful + ~18 failed PATCHes in a 3-second window during Bursitis Test 2 batch 2 (19 operations applied). Earlier estimate was based on a Vercel view showing only failures. Pool tune from default 15 → 40 was tried tonight (67% of Nano compute's max_connections=60 cap; comfortably below Supabase's 80% warning threshold) and was INSUFFICIENT — same FATAL: Max client connections reached recurred. Pool tune at Nano compute is structurally insufficient; the apply-pipeline parallel-PATCH-burst reduction code-fix is now PRIMARY and gate-blocking. **Phase 3 implication tightened:** 50 workers × 60 burst = 3000 simultaneous connections at peak — uncoverable by any compute tier upgrade Supabase offers (largest 16XL caps at ~12,000 max client connections but per-pool default is still small). Code-fix (worker queue / bulk endpoint / atomic-batch-into-rebuild-transaction) is the only structural answer. **NEW MEDIUM finding:** P2025 race condition between canvas-rebuild $transaction's archive operation and parallel keyword PATCHes — captured as separate ROADMAP entry. Header timestamp.)
**Last updated in session:** session_2026-05-04-d_pool-tune-small-batch-test-insufficient (Claude Code)
**Previously updated:** May 4, 2026-c (D3 retry attempt — partial; pool-exhaustion finding ON PRO TIER — third session of 2026-05-04, follow-up to `session_2026-05-04-b_rate-fix-first-pass`. **§10 gains "🚨 Connection pool exhaustion under apply-pipeline burst (DISCOVERED 2026-05-04-c)"** entry capturing the discovered failure mode: ~22 parallel PATCH /keywords/[id] calls per batch — burst size corrected 2026-05-04-d to 55-60+. Different failure mode than the 2026-05-04-b rate-fix targets (no Prisma error code; just FATAL message). **CORRECTION post-original-commit:** director clarified Pro upgrade was already done at end of 2026-05-04-b; tonight's failure was on Pro tier, not Free. Revised fix path was: tune Pro pool-size config knob (probable primary fix), AND/OR ship apply-pipeline parallel-PATCH-burst reduction code-fix. Pool tuning was tried 2026-05-04-d and was INSUFFICIENT — code-fix is now confirmed primary.)
**Previously updated in session:** session_2026-05-04-c_d3-retry-partial-pool-exhaustion-finding (Claude Code)
**Previously updated:** May 4, 2026-b (Rate-fix first pass — second session of 2026-05-04, follow-up to instrumentation pass. **§10 gains "🛡️ Server-side withRetry parity — first-pass closed (SHIPPED 2026-05-04-b)"** entry capturing the rate-layer fix: `lib/auth.ts` (`verifyProjectAuth` + `verifyProjectWorkflowAuth`) + `lib/workflow-status.ts` (`markWorkflowActive` + `ensureProjectWorkflow`) helpers wrapped in `withRetry()` (universal silent-multiplier finding from 2026-05-04 audit) + apply-pipeline writes (POST `/canvas/rebuild` $transaction; POST/PATCH/DELETE `/canvas/nodes`; PATCH `/canvas`; `/canvas/sister-links` + `/canvas/pathways` creates/deletes; keywords routes; removed-keywords + restore) all wrapped. Combined coverage ~4% → ~80%+ of authenticated apply-pipeline paths. Idempotency tradeoff for unique-constraint-free creates documented. Header timestamp.)
**Previously updated:** May 4, 2026 (Underlying flake-rate investigation — first investigation session of W#1 PRODUCTION-READINESS GATE prerequisite #3. §10 gains "📊 Platform-wide flake-rate telemetry (SHIPPED 2026-05-04)" entry: new pure helper `src/lib/flake-counter.ts` (~140 LOC) + 28 unit tests; all 19 DB-backed API routes wired (38 catch blocks ↔ 38 `recordFlake(...)` call sites — exact 1-to-1; additive only); 2 wrapped routes record with `retried: true`; `/canvas/rebuild` POST captures `body.nodes.length` as canvasSize. Investigation findings preserved as platform-level facts: pgbouncer hypothesis grounded (transaction pooler at port 6543); withRetry parity ~4% at endpoint layer; silent multiplier in `lib/auth.ts` + `lib/workflow-status.ts` helpers; Prisma client uses bare singleton with no connection-lifecycle management. Helper is workflow-agnostic — every future DB-backed route across all 14 PLOS workflows uses the same telemetry surface. Cross-references to ROADMAP entry + KEYWORD_CLUSTERING_ACTIVE STATE block + existing `prisma-retry.ts`.)
...
│   └── middleware.ts
├── extensions/                    — non-web-app PLOS clients (W#2 introduced 2026-05-07-f)
│   └── competition-scraping/      — W#2 Chrome extension (WXT 0.20 + React 19 + Supabase JS)
│       ├── package.json           — extension's own deps + scripts (npm; isolated from root)
│       ├── wxt.config.ts          — manifest config + react module
...
│       └── src/
│           ├── entrypoints/
│           │   ├── popup/         — sign-in + signed-in screens (React + CSS)
│           │   ├── background.ts  — service worker (chrome.contextMenus + onClicked routing as of 2026-05-07-h)
│           │   └── content.ts     — NEW 2026-05-07-h — WXT defineContentScript matching the 4 shopping platforms
│           └── lib/
│               ├── supabase.ts    — Supabase client + chrome.storage.local adapter
│               ├── auth.ts        — signIn / signOut / getSession / getAccessToken
│               ├── api-client.ts  — Bearer-header authedFetch (extended 2026-05-07-h with listCompetitorUrls + createCompetitorUrl)
│               ├── color-palette.ts        — 20-color WCAG-AA palette (added 2026-05-07-g)
│               ├── highlight-terms.ts      — parse/dedup/merge (added 2026-05-07-g)
│               ├── popup-state.ts          — chrome.storage.local I/O (added 2026-05-07-g)
│               ├── platforms.ts            — 7-option platform list for popup picker (added 2026-05-07-g)
│               ├── url-normalization.ts    — NEW 2026-05-07-h — `?`-stripping per §B 2026-05-07-g item 3
│               ├── platform-modules/       — NEW 2026-05-07-h — per-platform DOM-pattern modules
│               │   ├── types.ts            — shared PlatformModule interface
│               │   ├── amazon.ts           — /dp/{ASIN} + /gp/product/{ASIN} matcher + canonical extractor
│               │   ├── ebay.ts             — /itm/{listing-id} matcher + canonical extractor
│               │   ├── etsy.ts             — /listing/{id} matcher + canonical extractor (preserves locale prefix)
│               │   ├── walmart.ts          — /ip/{slug}/{id} matcher + canonical extractor
│               │   └── registry.ts         — getModuleByPlatform + getModuleByHostname suffix-match
│               └── content-script/         — NEW 2026-05-07-h — content-script UI components (vanilla DOM)
│                   ├── styles.ts           — CSS string + ensureStylesInjected
│                   ├── floating-add-button.ts  — 300ms hover delay + per-session × dismiss
│                   ├── already-saved-icon.ts   — green ✓ to LEFT of competitor links
│                   ├── already-saved-overlay.ts — top-right floating banner, 5s auto-dismiss
│                   ├── url-add-form.ts     — modal form with Save/Cancel/Esc/backdrop close
│                   ├── messaging.ts        — typed ContentScriptMessage protocol
│                   └── orchestrator.ts     — main runOrchestrator entry
├── package.json
...
**Added in 2026-05-07-f:** `extensions/` top-level folder for the W#2 Chrome extension and any future non-web-app PLOS clients (mobile app, desktop tool, etc.) per `PLATFORM_REQUIREMENTS.md §10.1`. Each non-web client lives in its own subfolder with its own `package.json` + build pipeline; `src/lib/shared-types/` is the single source of truth for any types shared between the PLOS Next.js app and the non-web client. Root `tsconfig.json` excludes `extensions/` so the root tsc doesn't try to type-check extension-specific globals (chrome namespace, defineBackground, etc.) — each client has its own tsconfig.

**Added in 2026-05-07-h:** content-script layer for the W#2 Chrome extension — `extensions/competition-scraping/src/lib/platform-modules/` (per-platform DOM-pattern modules per `STACK_DECISIONS §15 Q7`; 4 modules today: amazon, ebay, etsy, walmart) + `extensions/competition-scraping/src/lib/content-script/` (vanilla DOM components for the floating "+ Add" button, URL-add overlay form, "already saved" recognition icon + detail-page overlay, plus orchestrator + typed messaging) + `extensions/competition-scraping/src/lib/url-normalization.ts` (`?`-stripping comparison-time helper per `COMPETITION_SCRAPING_DESIGN.md §B` 2026-05-07-g end-of-session addendum item 3) + `extensions/competition-scraping/src/entrypoints/content.ts` (WXT `defineContentScript` matching `https://*.{amazon,ebay,etsy,walmart}.com/*`). The content-script chunk bundles to ~219 KB compressed; each new platform added in future sessions extends the registry.ts list additively. Manifest expanded with `https://*.{amazon,ebay,etsy,walmart}.com/*` host_permissions + `contextMenus` permission — Chrome will require user re-approval at unpacked-reload time.
...
| `/plos` | Yes | **(RESHAPED 2026-05-13)** — Server-side redirect to `/projects`. Original 1,525-line workflow-cards landing replaced with a minimal `redirect("/projects")` server component per director directive — director wanted PLOS to open directly to the Projects page without the workflow-cards intermediate step. Every entry point (Dashboard PLOS card, typed URL, bookmark, `/plos/notes` back-button) now lands on `/projects`. Reversible by restoring the prior page from git history (`git show 23a5985:src/app/plos/page.tsx`). |
...
| `/projects/[projectId]` | Yes | **(Built for real in Ckpt 9.5; W#2 card wired 2026-05-13)** — Project detail page showing Project name + description + 15-card workflow grid (14 launch workflows + Business Operations) with per-workflow status badges. Clicking Keyword Analysis card → `/projects/[id]/keyword-clustering`. **Clicking Competition Scraping card → `/projects/[id]/competition-scraping` (NEW 2026-05-13 — `WORKFLOW_DEFS[1]` flipped from `active: false, route: null` to `active: true, route: "competition-scraping"` now that W#2 has had a real page live on main since 2026-05-08).** Other 13 cards still show "coming soon" toast. Handles 404/403/loading gracefully. **Note:** Ckpt 6 docs originally claimed this was built in Ckpt 6, but the file never existed on disk until Ckpt 9.5 — see CORRECTIONS_LOG for the Pattern 7 recurrence details. |
...
| `/projects/[projectId]/competition-scraping` | Yes | **(NEW 2026-05-06)** — W#2 Competition Scraping & Deep Analysis page. Composes 7 components from the Shared Workflow Components Library at `@/lib/workflow-components`: `useWorkflowContext()` hook + `<WorkflowTopbar>` + `<StatusBadge>` + `<DeliverablesArea>` (Resources sub-section) + `<CompanionDownload>` + `<ResetWorkflowButton>` + `<ResetConfirmDialog>` + `<WorkerCompletionButton>`. Skips `<NotReadyBanner>` (W#2 declared "always ready") and `<AdminReviewControls>` (W#2 declared `reviewCycle: 'skip'`). Custom multi-table viewer for the content area + API routes per `COMPETITION_SCRAPING_STACK_DECISIONS.md §11` + admin reset endpoint wiring + Detailed User Guide content authoring all deferred to follow-up sessions. As of 2026-05-07 the URL table's click-row navigates IN-APP to the per-URL detail page (the row below) instead of opening the competitor's website in a new tab. |
| `/projects/[projectId]/competition-scraping/url/[urlId]` | Yes | **(NEW 2026-05-07)** — W#2 per-URL detail page. Address fixed by `COMPETITION_SCRAPING_STACK_DECISIONS.md §10`; deep-linkable. Composes the same library chrome (`useWorkflowContext()` + `<WorkflowTopbar>`) as the workflow root + a custom `UrlDetailContent` component with: sub-breadcrumb (`Competition Scraping › [Platform] › [URL]`, first two segments are Links); URL metadata read-only grid + customFields sub-grid; "Open original URL ↗" button preserving the prior new-tab affordance; read-only "Sizes / Options" sub-section; sortable "Captured Text" table (Content Category / Text-wrapping / Tags / Added On) with `(N)` count badge; image-count placeholder ("N images captured for this URL — full-size viewer ships in slice (a.2)"). Parallel four-fetch via `Promise.all` against the four GET read paths shipped today (rows below); cancelled-flag race guard prevents stale fetches from clobbering newer state. Loading / 404 / 403 / per-section error states. Image rendering itself ships in slice (a.2); inline editing in (a.3); per-column filter dropdowns in (a.4). |
...
|---|---|---|---|
| `/api/projects` | GET / POST / OPTIONS | List user's Projects (sorted by aggregate lastActivityAt) / Create. **OPTIONS + CORS added 2026-05-07-f** so the W#2 Chrome extension's `Authorization: Bearer` preflight passes. Same-origin web callers unaffected. | `verifyAuth` |
...
| `/api/projects/[projectId]/competition-scraping/urls` | GET / POST / OPTIONS | List W#2 competitor URLs (optional `?platform=` filter) / create (idempotent on `(workflow, platform, url)` unique constraint) / CORS preflight for `chrome-extension://*` | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/urls/[urlId]` | GET / PATCH / DELETE / OPTIONS | Read single URL (NEW 2026-05-07 — supports the per-URL detail page; verifies projectWorkflowId scope so forged urlIds from another Project return 404 not row data) / update / delete (cascades to Sizes/Text/Images via Prisma; idempotent — already-deleted returns success) | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/urls/[urlId]/sizes` | GET / POST / OPTIONS | List sizes for one URL (NEW 2026-05-07 — ordered by sortOrder ASC, addedAt ASC; verifies parent URL belongs to this Project's W#2 before reading) / create a CompetitorSize under the URL (no idempotency layer per stack-decisions §9.2) | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/sizes/[sizeId]` | PATCH / DELETE / OPTIONS | Update / delete a size; two-step ownership check via `findFirst({ id, competitorUrl: { projectWorkflowId } })` then update-by-id; DELETE idempotent (already-deleted → success) | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/urls/[urlId]/text` | GET / POST / OPTIONS | List captured-text rows for one URL (NEW 2026-05-07 — ordered by sortOrder ASC, addedAt ASC; verifies parent URL belongs to this Project's W#2 before reading) / create a CapturedText under the URL (idempotent on `clientId` via create-then-catch-P2002 — extension WAL retry path) | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/urls/[urlId]/images` | GET / OPTIONS | List captured-image rows for one URL (NEW 2026-05-07; UPDATED 2026-05-07-b for slice (a.2) — ordered by sortOrder ASC, addedAt ASC; returns metadata + storagePath + a 200×200 contain-fit `thumbnailUrl` (Supabase on-the-fly transform signed URL, 1-hour TTL) + a full-resolution `fullSizeUrl` (signed URL, 1-hour TTL) per row, both minted server-side via the `competition-storage.ts` helper). Sibling routes `requestUpload` + `finalize` (already shipped session-2) handle the two-phase upload write path | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/text/[textId]` | PATCH / DELETE / OPTIONS | Update / delete a captured text; same two-step ownership pattern as sizes; clientId is immutable | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/urls/[urlId]/images/requestUpload` | POST / OPTIONS | Phase 1 of two-phase upload per stack-decisions §3 — validates MIME (jpeg/png/webp) + size (≤5 MB) + sourceType, generates UUIDv4 capturedImageId, composes `{projectId}/{urlId}/{capturedImageId}.{ext}` storagePath, requests Supabase signed upload URL via `requestUploadUrl()` helper. **No DB write here** — row is created at finalize. Idempotency: each call mints a fresh capturedImageId; orphan storage paths from retries are swept by the daily janitor (session-3) | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/urls/[urlId]/images/finalize` | POST / OPTIONS | Phase 2 of two-phase upload — re-derives storagePath from path params + body's mimeType + sourceType (echoed from extension's WAL); calls `verifyUploadedFile()` to confirm bytes landed in storage before any DB write; creates the CapturedImage row. Idempotent on clientId via create-then-catch-P2002 | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/images/[imageId]` | PATCH / DELETE / OPTIONS | Update editable metadata (imageCategory, composition, embeddedText, tags, sortOrder) — capture-shape fields immutable; DELETE removes DB row + best-effort `deleteImage()` storage cleanup (storage failure does NOT abort DB delete; orphans handled by janitor) | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/reconcile` | GET / OPTIONS | Periodic reconciliation per stack-decisions §8.3.2 — required `?platform=` query; returns `{ platform, urlCount, textCount, imageCount, lastModified }`. Three parallel `aggregate({ _count: { _all: true }, _max: { updatedAt: true } })` calls; lastModified is max across the three child tables, epoch-fallback when no rows exist for the scope | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/reset` | POST | Admin reset per stack-decisions §11.1 + §A.11 — body `{ confirmProjectName }` (must equal `Project.name`, server-side defense-in-depth); DB transaction deletes the W#2 ProjectWorkflow row (cascades CompetitorUrl + CompetitorSize + CapturedText + CapturedImage) + W#2-originated VocabularyEntry rows (other workflows' entries on the same Project preserved per `PLATFORM_REQUIREMENTS.md §8.4`); storage wipe via `wipeProjectImages()` outside the transaction (janitor backstops orphan files on partial failure). Admin gate via Project ownership; web-only (no CORS preflight) | `verifyProjectAuth` |
| `/api/projects/[projectId]/vocabulary` | GET / POST / OPTIONS | Project-scoped vocabulary entries per `PLATFORM_REQUIREMENTS.md §8.4` (any workflow can READ + ADD); `?type=` required on GET; POST is idempotent upsert on `(projectId, vocabularyType, value)` unique constraint | `verifyProjectAuth` |
| `/api/cron/competition-scraping-janitor` | GET | Daily Vercel cron at 03:00 UTC per stack-decisions §3 — auth via `Authorization: Bearer ${CRON_SECRET}` (Vercel sends automatically when env var set; route denies all requests when env var unset so the path can't be hit publicly). Reads `CapturedImage.storagePath` rows into a Set, walks the bucket via `listAllStorageImages()`, classifies orphans via pure `findOrphans()` helper (no DB row + createdAt > 24h = orphan; younger files in grace window for in-flight uploads), bulk-deletes via `deleteStorageImages()`. Idempotent across runs | `Bearer ${CRON_SECRET}` |
...
**Activity tracking side-effect (post-Ckpt-5; extended 2026-05-07 for W#2 session-1 + session-2):** All KC workflow-data routes (keywords, canvas nodes, pathways, sister-links, rebuild) call `markWorkflowActive(projectId, "keyword-clustering")` after successful mutations; W#2 mutations (POST `.../urls`, PATCH/DELETE `.../urls/[urlId]`, POST `.../sizes`, PATCH/DELETE `.../sizes/[sizeId]`, POST `.../text`, PATCH/DELETE `.../text/[textId]`, POST `.../images/finalize`, PATCH/DELETE `.../images/[imageId]`) call `markWorkflowActive(projectId, "competition-scraping")`. This flips `ProjectWorkflow.status` from `"inactive"` to `"active"` on first meaningful use and refreshes `lastActivityAt`. GET endpoints, canvas state (pan/zoom), and the `images/requestUpload` POST (no DB write) do NOT trigger activity tracking. The project-scoped vocabulary route does NOT call `markWorkflowActive` — it lacks a single workflow context (any workflow can call it); the calling workflow's own routes handle activity stamping.
...
### Storage: `competition-scraping` bucket (NEW 2026-05-07 — W#2 API-routes session-2)
- Supabase Storage, **private** (signed-URL access only — no public RLS path)
- Created via the Supabase dashboard with director walkthrough at session-2 start; bucket exists in production
- **Path:** `{projectId}/{competitorUrlId}/{capturedImageId}.{ext}` per `COMPETITION_SCRAPING_STACK_DECISIONS.md §3` — per-Project folder so admin reset can wipe a Project's images via list+remove cycle without affecting other Projects' images
- **Helper wrapper:** `src/lib/competition-storage.ts` per `PLATFORM_REQUIREMENTS.md §10.4` portability requirement — every Supabase Storage SDK call lives in this single file. Future swap to S3 only replaces this file. Exports: `requestUploadUrl()` (Phase 1 of two-phase upload — generates signed upload URL), `verifyUploadedFile()` (server-side presence check before creating the DB row at finalize), `getThumbnailUrl()` (1-hour signed URL with Supabase on-the-fly transform to 200×200 contain-fit), `getFullSizeUrl()` (1-hour signed URL, original resolution), `deleteImage()` (single-file remove for per-row DELETE), `wipeProjectImages()` (admin reset — paginated list-walk + batched remove).
- **Pure helpers:** `src/lib/competition-storage-helpers.ts` (split from `competition-storage.ts` so node:test loads it without resolving the `@/` alias — same cors-style split pattern as 2026-05-07 CORRECTIONS_LOG entry). Exports: `composeStoragePath()`, `extensionFromMimeType()`, `splitStoragePath()`, `isAcceptedMime()`. 22 unit tests at `src/lib/competition-storage-helpers.test.ts`.
- **Accepted MIME types per §3:** `image/jpeg`, `image/png`, `image/webp`. Reject `image/svg+xml` (XSS risk) + `image/heic` / `image/heif` (browser compatibility). Server-side enforcement at the requestUpload route — bucket-level toggles deliberately left OFF per the §3 spec posture.
- **Size cap per §3:** 5 MB per upload. Server-side enforcement at requestUpload (`fileSize > 5_242_880` returns 413). Bucket-level cap left OFF (same posture as MIME).
- **Upload-token TTL:** Supabase SDK default (~2 hours; SDK's `createSignedUploadUrl` does not expose `expiresIn`). The §3 spec said 5 minutes — practical security posture is unchanged because the URL is single-use (upload invalidates the token) and the extension uses it within seconds.
- **View-URL TTL:** 1 hour, re-issued per page render (inexpensive — single Supabase API call per render).
- **Janitor:** daily Vercel cron at 03:00 UTC (`vercel.json` `crons` entry) — handler at `src/app/api/cron/competition-scraping-janitor/route.ts`. Sweeps storage files with no matching `CapturedImage.storagePath` row whose `createdAt` is older than the 24h grace window (the grace prevents deleting in-flight uploads where the `:finalize` call may still be on its way). Orphan classification is the pure `findOrphans()` helper in `competition-storage-helpers.ts` (11 unit tests). Storage SDK calls (`listAllStorageImages()`, `deleteStorageImages()`) live in `competition-storage.ts` per the §10.4 portability boundary. Auth via `Authorization: Bearer ${CRON_SECRET}` (Vercel sends this automatically when the env var is set; route denies all requests if unset so the path can't be hit publicly). **SHIPPED 2026-05-08 in W#2 API-routes session-3** — pending `CRON_SECRET` env var add in Vercel project settings before the cron runs in production.
...
CRON_SECRET=...                    # NEW 2026-05-08 — server-side; Vercel sends as Bearer token to /api/cron/* automatically when set; W#2 janitor denies all requests if unset
```
...
- 🟡 **Assignment table schema landed 2026-05-06 (W#2 PLOS-side build first slice); not yet wired.** `WorkerAssignment` table now exists in the live Supabase Pro DB per `COMPETITION_SCRAPING_STACK_DECISIONS.md §9.1` — fields: `userId`, `projectId`, `workflow`, `subScope?`, `status`, `assignedAt`, `assignedBy`, `completedAt`. Hard-rule unique constraint per `(projectId, workflow, subScope)` per W#2 §A.2. **What's still missing:** the four-way (User × Workflow × Project × subScope) assignment UI + admin assignment-management routes + permission middleware on every API route + `Phase 2 turn-on` orchestration. All deferred to a future Phase-2 build session.
...
- 🟡 **Audit-trail table schema landed 2026-05-06 (W#2 PLOS-side build first slice); helper + emission wiring still missing.** `AuditEvent` table now exists in the live Supabase Pro DB per `COMPETITION_SCRAPING_STACK_DECISIONS.md §9.1` — fields: `workflowType`, `projectId`, `userId`, `eventType`, `payload Json`, `timestamp`. Phase 2 still requires the shared emission helper (`useEmitAuditEvent()` is currently a Phase-2 stub no-op per `WORKFLOW_COMPONENTS_LIBRARY_DESIGN.md §3.10`) + opt-in declarations per workflow + the actual call sites in operation-applier paths (per the W#1 ROADMAP NEW HIGH "Auto-Analyze action history table + per-action undo" entry from 2026-05-05-d).
...
- ✅ **Shared Workflow Components Library — Phase 1 SHIPPED 2026-05-05-c.** Reframed 2026-05-04 from the earlier "Shared Workflow-Tool Scaffold" required-shell concept to a bottom-up library of reusable React components and hooks per `PLATFORM_REQUIREMENTS.md §12` (rewritten in `session_2026-05-04_workflow-tool-scaffold-design`). Phase-1 build session shipped 9 components at `src/lib/workflow-components/` (`useWorkflowContext()`, `<WorkflowTopbar>`, `<StatusBadge>`, `<NotReadyBanner>`, `<DeliverablesArea>`, `<CompanionDownload>`, `<ResetWorkflowButton>` + `<ResetConfirmDialog>`, `<WorkerCompletionButton>`) plus a smoke-test page at `/components-smoke-test`. W#2's PLOS-side build is structurally unblocked. Phase 2 components — `<AdminReviewControls>`, `useEmitAuditEvent()` — built when Phase 2 turn-on is scheduled. Full design captured in `docs/WORKFLOW_COMPONENTS_LIBRARY_DESIGN.md` (§A frozen + §B append-only refinements).
...
- ✅ **Connection pool exhaustion under apply-pipeline burst (DISCOVERED 2026-05-04-c; ADDRESSED for Phase 1 via atomic-batch fold-in 2026-05-05).** **2026-05-05 update:** approach (iii) atomic-batch-into-rebuild-transaction shipped (`0caa200`) and verified live across 50 batches + 5 consolidations on production vklf.com via D3 retry on Bursitis Test 2. The 55-60+ parallel PATCH burst is structurally gone — server-side `/canvas/rebuild` route folds `keywordUpdates` + `archiveKeywords` into the same `$transaction` as the canvas rebuild (with explicit `{timeout: 30000, maxWait: 5000}`); client-side `AutoAnalyze.tsx doApplyV3` removes the fire-and-forget per-id PATCH burst (the actual fan-out source was `KeywordWorkspace.tsx:627`'s `forEach`) and the sequential archive POST loop. ~24,000+ keyword updates flowed through the fold-in across 50 batches with zero pool-exhaustion errors. P2025 race condition (the 2026-05-04-d MEDIUM finding) eliminated for free. Pool tune from default 15 → 40 stays in place as a secondary buffer. **Phase 3 scaling concern remains:** at canvas 700+ topics × ~3,500 keywords per batch × 50 concurrent workers, the single-transaction connection-hold model becomes its own ceiling (deferred architectural item — needs revisit before Phase 3, likely via per-keyword chunking inside the route OR a streaming-update model OR a different concurrency strategy entirely). Below is the original DISCOVERED text preserved for historical context. — 🚨 **Connection pool exhaustion under apply-pipeline burst (DISCOVERED 2026-05-04-c — live D3 retry attempt; fires ON PRO TIER per post-original-commit correction; POOL-TUNE TRIED 2026-05-04-d INSUFFICIENT; burst-size corrected 2026-05-04-d from 22 → 55-60+).** **2026-05-04-d update:** director executed three-step pool investigation tonight (Pro confirmed; compute revealed as Nano; pool 15 → 40 = 67% of Nano's 60-cap max_connections); small-batch test on Bursitis Test 2 batch 2 produced same FATAL: Max client connections reached. Vercel log expansion revealed actual burst is **55-60+ parallel PATCH calls per batch** (~40 successful + ~18 failed in a 3-second window) — not the 22 estimated 2026-05-04-c. Pool tune at Nano compute is structurally insufficient (any safe pool size ≤ 48 leaves ~10+ failures). Pool 40 stays in place as secondary buffer. **Code-fix path (parallel-PATCH-burst reduction) is now PRIMARY and gate-blocking** — three approaches (worker queue / bulk PATCH endpoint / atomic-batch-into-rebuild-transaction); approach (iii) atomic-batch eliminates the P2025 race condition for free + scales structurally for Phase 3. **P2025 race condition discovered same session** as separate MEDIUM finding (canvas-rebuild $transaction archives keyword while parallel PATCH still in-flight against it). Tightened Phase 3 math: 50 workers × 60 burst = 3000 simultaneous connections — uncoverable by any compute upgrade Supabase offers. Discovered live during the D3 retry attempt 2026-05-04-c. After batch 1's model-side + server-side apply succeeded cleanly ($0.312, 15 ops, canvas at 124 nodes), the post-rebuild UI refresh hit ~28 cascading HTTP 500s within ~0.1 seconds at 8:30:40 PM (~22 PATCH /keywords/[id] failures + 6 GET /canvas + /canvas/nodes failures), ALL with the same root error: `FATAL: Max client connections reached`. The Supabase connection pool through the pgbouncer transaction pooler at port 6543 was overwhelmed by the apply-pipeline's parallel keyword-update burst. **CORRECTION post-original-commit:** the original entry framed this as "Free-tier capacity ran out; Pro upgrade fixes it." Director clarified at end-of-session that Pro upgrade was already in place at end of 2026-05-04-b — tonight's failure fired ON Pro tier. More serious structural finding than the original framing suggested. **This is a CAPACITY problem, not a transient-flake problem** — it surfaces as `PrismaClientUnknownRequestError` / `PrismaClientInitializationError` with no Prisma error code, just the FATAL message embedded in `error.message`. **The 2026-05-04-b withRetry rate-fix structurally cannot help** — `withRetry` retries transient codes P1001/P1002/P1008/P2034; pool exhaustion has no code. Retries actively make it worse (each retry adds connection pressure). **Three layers of fix needed (REVISED post-correction):** (a) ✅ **Supabase Pro upgrade — DONE 2026-05-04-b** (necessary but not sufficient; tonight's failure proves Pro alone doesn't clear the wall under the burst pattern), (a-prime) **Tune Pro tier's pool size config knob (PROBABLE PRIMARY FIX — NEW)** — Pro exposes a configurable pool-size knob; default may still be too small for our ~22-parallel-PATCH burst; ~5-10 min Supabase dashboard change to increase from default to e.g., 30-50, (b) **apply-pipeline parallel-PATCH-burst reduction (PROMOTED FROM DEFERRED-COMPANION TO PRIMARY CODE-SIDE PATH if (a-prime) doesn't clear the wall)** — investigate serializing the ~22 parallel PATCH /keywords/[id] calls per batch via small worker queue OR using existing bulk PATCH /keywords endpoint OR atomically batching into the canvas rebuild transaction (heavier refactor), (c) **add pool-config visibility to pre-flight runner** — surface active Supabase plan tier + configured pool size at run start with warning if pool looks too small for expected burst. **Phase 3 implication (zoom-out per Rule 16):** at 50 concurrent workers (Phase 3 scale target per `PLATFORM_REQUIREMENTS.md §1`), pool exhaustion will be vastly worse than today's solo-director load. Pro upgrade is necessary but not sufficient — the apply-pipeline parallel-burst pattern itself needs revisiting before Phase 3, OR the connection pool needs tuning beyond Pro's defaults. Architectural concern that propagates to W#2-W#14 because they share the same DB + pool. Cross-references: `KEYWORD_CLUSTERING_ACTIVE.md` POST-2026-05-04-C-D3-RETRY-PARTIAL-POOL-EXHAUSTION-FINDING STATE block. `ROADMAP.md` HIGH-severity "Pool exhaustion under apply-pipeline burst" entry + W#1 PRODUCTION-READINESS GATE prerequisite #4. `CORRECTIONS_LOG.md` 2026-05-04-c INFORMATIONAL entry on rate-fix scope vs. discovered failure mode.

- 🛡️ **Server-side withRetry parity — first-pass closed (SHIPPED 2026-05-04-b — first rate-fix on top of the 2026-05-04 instrumentation; W#1 PRODUCTION-READINESS GATE prerequisite #3 first fix).** Implements option (c) "withRetry parity extension" from the flake-rate ROADMAP entry, scoped to the silent multiplier helpers + apply-pipeline writes identified by the 2026-05-04 audit. **What's now wrapped in `withRetry()`:** (1) **`lib/auth.ts`** — `verifyProjectAuth`'s `prisma.project.findUnique` + `verifyProjectWorkflowAuth`'s `prisma.projectWorkflow.upsert` (these helpers fire on EVERY authenticated request — universal silent-multiplier finding); (2) **`lib/workflow-status.ts`** — `markWorkflowActive`'s findUnique + create + both update paths + `ensureProjectWorkflow`'s upsert (these fire on every authenticated mutation); (3) **apply-pipeline writes** — POST `/canvas/rebuild` (G1 `canvasNode.count` pre-check + the multi-second `$transaction(ops)` connection-holder), PATCH `/canvas` (canvasState.upsert), POST/PATCH/DELETE `/canvas/nodes` (transaction + transaction + deleteMany), POST/DELETE `/canvas/sister-links`, POST/DELETE `/canvas/pathways`, POST/PATCH/DELETE `/keywords`, PATCH/DELETE `/keywords/[keywordId]`, POST `/removed-keywords`, POST `/removed-keywords/[id]/restore`. **Combined coverage:** ~4% (2 of ~50 endpoint-method combos pre-2026-05-04-b) → ~80%+ of authenticated apply-pipeline paths post-2026-05-04-b. **All `recordFlake` call sites in wrapped routes record `retried: true`** so analysis can separate "wrapped-and-still-failed" (G2 exhausted) from "raw flake" (no retry layer) in the `[FLAKE]` log volume. **Idempotency tradeoff documented:** several `create` calls without unique constraints (sisterLink, pathway, keyword single-create + createMany, removedKeyword) accept rare retry-after-partial-commit duplicates as preferable to 500s — P1001/P2034 cases are safe (DB never committed); P1002/P1008 cases COULD theoretically duplicate if commit succeeded but response timed out; admin can delete duplicates in seconds; future sessions can add unique constraints or idempotency keys if field data shows recurring duplicates. **Routes deliberately NOT wrapped this round** (out of scope for first fix; flagged for future sessions): `/projects` GET (page-render N+1 hits Prisma O(N×3) per page-render), `/projects/[id]` GET, `/project-workflows/*` GETs, `/admin-notes/*`, `/user-preferences/*`. Cold-start GETs `/keywords` GET + `/removed-keywords` GET deliberately left server-unwrapped because already defended at client layer by 2026-05-03 cold-start retry helper. **Why this lives in PLATFORM_ARCHITECTURE not just KEYWORD_CLUSTERING_ACTIVE:** the wrapped helpers in `lib/auth.ts` + `lib/workflow-status.ts` are platform-shared — every authenticated API route in every future workflow benefits from the helper-level wrap automatically. Cross-references: `ROADMAP.md` "🟡 FIRST RATE-FIX SHIPPED 2026-05-04-b — Underlying ~25% per-endpoint pgbouncer/Prisma flake rate" entry. `KEYWORD_CLUSTERING_ACTIVE.md` POST-2026-05-04-B-RATE-FIX-FIRST-PASS STATE block. `src/lib/prisma-retry.ts` (the existing G2 helper). Live verification pending at next D3 attempt OR via natural `[FLAKE]` data window.

- 📊 **Platform-wide flake-rate telemetry (SHIPPED 2026-05-04 — first investigation session of the underlying ~25% pgbouncer/Prisma flake-rate fix; W#1 PRODUCTION-READINESS GATE prerequisite #3).** New pure helper `src/lib/flake-counter.ts` (~140 LOC) exporting `recordFlake(endpoint, error, ctx?)` — emits a single grep-able `[FLAKE]` line via `console.error` with stable shape `[FLAKE] endpoint=<METHOD /path> code=<P1001|...|unknown> retried=<true|false> [canvasSize=<N>] [projectWorkflowId=<id>] msg="<200-char-truncated>"`. All 19 DB-backed API routes wired with `recordFlake(...)` calls in their catch blocks (38 catch blocks ↔ 38 call sites — exact 1-to-1; additive only — runs BEFORE the existing `console.error` so the pre-existing diagnostic dump is preserved). Two routes already wrapped in withRetry (`GET /canvas` + `GET /canvas/nodes`) record with `retried: true` so analysis can separate "wrapped-and-still-failed" from "raw flake". `/canvas/rebuild` POST captures `body.nodes.length` payload size as `canvasSize` to test the "transaction-duration scales with payload → pgbouncer pressure" hypothesis. 28 unit tests in `src/lib/flake-counter.test.ts`. The 20th route file (`/api/ai/analyze/route.ts`) doesn't use Prisma — deliberately skipped. **Investigation findings 2026-05-04 (preserved here as platform-level facts, not just W#1):** (a) Supabase confirmed using transaction pooler at port 6543 with `?pgbouncer=true` (DATABASE_URL); migrations use direct port 5432 (DIRECT_URL); pgbouncer hypothesis correctly grounded — every runtime DB call goes through the pooler. (b) **withRetry parity is ~4%** at the endpoint layer — only `/canvas` GET + `/canvas/nodes` GET have G2 coverage; everything else (apply-pipeline writes, cold-start fetches, page-render, admin-notes, user-prefs) is unwrapped. (c) **Hidden silent multiplier:** auth + activity-tracking helpers in `lib/auth.ts` (verifyProjectAuth + verifyProjectWorkflowAuth) and `lib/workflow-status.ts` (markWorkflowActive + ensureProjectWorkflow) make Prisma calls on EVERY authenticated request and are themselves unwrapped — a flake here surfaces as a 500 even on the 2 routes that DO have withRetry. (d) Prisma client at `src/lib/db.ts` uses bare singleton with no `$connect`/`$disconnect` lifecycle; no `@prisma/adapter-pg`. **Why this lives in PLATFORM_ARCHITECTURE not just KEYWORD_CLUSTERING_ACTIVE:** the helper is workflow-agnostic; every future DB-backed route across all 14 PLOS workflows should use the same telemetry surface. Adding a new endpoint = one `import` + one `recordFlake(...)` call per catch block. Cross-references: `ROADMAP.md` "🟡 INSTRUMENTATION SHIPPED 2026-05-04 — Underlying ~25% per-endpoint pgbouncer/Prisma flake rate" entry (with the "Investigation Findings 2026-05-04" sub-section). `KEYWORD_CLUSTERING_ACTIVE.md` POST-2026-05-04 STATE block (full session narrative). `src/lib/prisma-retry.ts` (the existing G2 helper).
