...
**Last updated:** May 4, 2026-c (D3 retry attempt — partial; pool-exhaustion finding ON PRO TIER — third session of 2026-05-04, follow-up to `session_2026-05-04-b_rate-fix-first-pass`. **§10 gains "🚨 Connection pool exhaustion under apply-pipeline burst (DISCOVERED 2026-05-04-c)"** entry capturing the discovered failure mode: ~22 parallel PATCH /keywords/[id] calls per batch overwhelm Supabase pool capacity → `FATAL: Max client connections reached` returned by Postgres. Different failure mode than the 2026-05-04-b rate-fix targets (no Prisma error code; just FATAL message). **CORRECTION post-original-commit:** director clarified Pro upgrade was already done at end of 2026-05-04-b; tonight's failure was on Pro tier, not Free. Revised fix path: tune Pro pool-size config knob (probable primary fix, ~5-10 min Supabase dashboard change), AND/OR ship apply-pipeline parallel-PATCH-burst reduction code-fix (promoted from deferred-companion to primary code-side path). Phase 3 scale implication: 50 concurrent workers will hit pool exhaustion much harder than today's solo director — Pro upgrade necessary but not sufficient. Header timestamp.)
