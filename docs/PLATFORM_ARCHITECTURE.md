...
- 🆕 **Shared Workflow Components Library — designed, build pending.** Reframed 2026-05-04 from the earlier "Shared Workflow-Tool Scaffold" required-shell concept to a bottom-up library of reusable React components and hooks per `PLATFORM_REQUIREMENTS.md §12` (rewritten in `session_2026-05-04_workflow-tool-scaffold-design`). Design captured in `docs/WORKFLOW_COMPONENTS_LIBRARY_DESIGN.md`. First build session ships the components needed to unblock W#2's PLOS-side build (`useWorkflowContext()`, `<WorkflowTopbar>`, `<StatusBadge>`, `<DeliverablesArea>` with Resources sub-section, `<CompanionDownload>`, `<ResetWorkflowButton>` + `<ResetConfirmDialog>`); Phase 2 components built when Phase 2 turn-on is scheduled.
