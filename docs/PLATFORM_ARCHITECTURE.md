...
| `/api/projects/[projectId]/competition-scraping/urls/[urlId]/images` | GET / OPTIONS | List captured-image rows for one URL (NEW 2026-05-07 — ordered by sortOrder ASC, addedAt ASC; returns metadata + storagePath; signed-URL minting for the actual image bytes is slice (a.2)'s concern). Sibling routes `requestUpload` + `finalize` (already shipped session-2) handle the two-phase upload write path | `verifyProjectWorkflowAuth("competition-scraping")` |
