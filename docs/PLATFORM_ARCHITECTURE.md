...
| `/api/projects/[projectId]/competition-scraping/urls/[urlId]` | PATCH / DELETE / OPTIONS | Update / delete (cascades to Sizes/Text/Images via Prisma; idempotent — already-deleted returns success) | `verifyProjectWorkflowAuth("competition-scraping")` |
| `/api/projects/[projectId]/competition-scraping/urls/[urlId]/sizes` | POST / OPTIONS | Create a CompetitorSize under the URL; verifies parent ownership before insert; no idempotency layer (sizes are explicit user actions per stack-decisions §9.2) | `verifyProjectWorkflowAuth("competition-scraping")` |
...
| `/api/projects/[projectId]/competition-scraping/urls/[urlId]/text` | POST / OPTIONS | Create a CapturedText under the URL; idempotent on `clientId` via create-then-catch-P2002 (extension WAL retry path) | `verifyProjectWorkflowAuth("competition-scraping")` |
