...
- **Janitor:** daily Vercel cron for orphan cleanup is session-3 work — sweeps storage files with no matching `CapturedImage.storagePath` row OR rows older than 24h that never got finalized.
