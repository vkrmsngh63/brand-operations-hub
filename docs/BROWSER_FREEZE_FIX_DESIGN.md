...
**Status update 2026-05-02-d (HTTP 500 retry regression investigation session):** the structural mechanism described above is now defended against by `src/lib/post-rebuild-fetch-retry.ts`, shipped this session. The post-rebuild canvas refresh now retries on its own (max 3 attempts; 2s, 5s waits) instead of triggering a whole-batch retry on failure; on persistent failure, the batch is marked complete server-side (it IS), checkpoint is saved, and the run pauses with explicit refresh-and-Resume guidance. Both the visible (cyclic stableId references caught by applier guards) AND silent (per-(projectWorkflowId, stableId) upsert collisions overwriting attempt-1 content) corruption modes are eliminated at the recovery layer. **Hypothesis A's freeze-causation question is still open** — the structural mechanism is defended, but whether that mechanism actually caused last week's freeze is a live-verification question. A Bursitis Test 2 resume run with ≥30 batches at canvas ≥105 topics is the next-session task. Three outcomes are possible: (1) run reaches ≥105 topics cleanly without freeze → Hypothesis A confirmed as freeze cause, freeze concern closes; (2) run reaches ≥105 topics, the post-rebuild refresh exhausts retries (rare), the run pauses cleanly with the new guidance message → fix works as designed, freeze caused by something else (Hypothesis B or C); (3) run freezes again at similar topic count → Hypothesis A ruled out, investigate Hypothesis B (extend instrumentation to React reconciliation + SVG paint) per §9.5.2. **Cross-references:** `src/lib/post-rebuild-fetch-retry.ts` (full module-level rationale), `ROADMAP.md` "🟡 CODE FIX SHIPPED 2026-05-02-d" entry, `KEYWORD_CLUSTERING_ACTIVE.md` POST-2026-05-02-d STATE block.
