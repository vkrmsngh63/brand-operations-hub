...
**Last updated:** April 24, 2026 (Phase 1g-test follow-up Part 3 — Session 2b — investigations-only continuation; P3-F8 canvas-layout regression diagnosed — React port dropped HTML tool's four-job layout engine (content-driven height + holistic push-down + auto-layout-on-link + pathway separation); Session 3 P3-F8 scope locked in per Q1/Q2/Q3 answers (one-shot port of all four functions, layout-pass runs after every batch, pathway separation included not deferred); Task 5 prompt review complete — all 7 line refs verified, Change 3 math bug fixed, Change 2/4/5 polished, Q4/Q5/Q6 design questions resolved; docs-only commit + `keyword_sorting_tool_v18.html` tracked per Option A clean-split timing)
...
- 🎯 **Phase 1g-test follow-up REMAINING (for next session):**
  1. **Execute Session 3 code fixes** per §Phase 1g-test Part 3 session plan below — scope cumulative from Sessions 2 + 2b: post-batch reconciliation pass (P3-F7 safety net) + salvage-ignored-keywords mechanism with refined Change 6 template + `RemovedKeyword` table + soft-archive flow (Removed Terms fix) + `CanvasState.nextNodeId` stale-counter fix + P3-F8 four-function canvas-layout port (one commit) + Opus 4.7/Haiku 4.5 dropdown + cost-tracker failed-attempt fix + B1 settings persistence.
...
**Session 3 — Tier-1 code fixes (Session 2b sign-off received 2026-04-24; scope locked in below):**
...
- **Salvage-ignored-keywords mechanism — locked in 2026-04-24 Session 2b.** Targeted follow-up prompt per `AUTO_ANALYZE_PROMPT_V2_PROPOSED_CHANGES.md` Change 6 (Session-2b-refined wording: IRRELEVANT_KEYWORDS auto-archive language per Q5 resolution). Tool code writes flagged keywords to `RemovedKeyword` table with `removedSource='auto-ai-detected-irrelevant'` + `aiReasoning` populated from model's returned reason. Paired with post-batch reconciliation as the primary+backup pair for P3-F7.
- **Removed Terms fix — Option B per director 2026-04-24 Session 2.** New `RemovedKeyword` table scoped to `ProjectWorkflow` with fields `{id, projectWorkflowId, keyword, volume, tags, topic, canvasLoc, removedAt, removedBy, removedSource ('manual'|'auto-ai-detected-irrelevant'), aiReasoning (nullable), createdAt}`. Remove action = transaction: copy Keyword row to `RemovedKeyword`, delete Keyword row. Restore = reverse. `ASTTable.tsx` `removedTerms` state replaced with DB-backed fetch. Modal UI filters/badges by source to distinguish manual vs AI-auto removal. `removedSource` + `aiReasoning` fields forward-compatible with future Auto-Remove Irrelevant Terms button feature (still DEFERRED per director — the button is a proactive full-canvas scan UI, distinct from the salvage-mechanism per-batch auto-archive which is live in this session).
- **`CanvasState.nextNodeId` stale-counter investigation + fix — from Session 2 DB query.** Max CanvasNode.id = 104 but nextNodeId = 5. Investigate how new node IDs are assigned; either repair counter to `max(id) + 1` or confirm counter is unused + remove.
- **P3-F8 canvas-layout port — locked in 2026-04-24 Session 2b, one-shot commit of all four functions:**
  ...
  - **Deferred to follow-up session (NOT in Session 3):** `baseY`/`y` separation for clean collapse/expand. Q2 was narrowly about pathway separation; `baseY` was not asked about.
