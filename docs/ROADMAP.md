...
| W#2 Competition Scraping & Deep Analysis | 🔄 Active dev — schema live 2026-05-06; first PLOS-side page slice live; **API-routes session-1 SHIPPED 2026-05-07** (read paths + `urls` POST/PATCH/DELETE + vocabulary endpoints); **API-routes session-2 SHIPPED 2026-05-07** (sizes + text + image two-phase upload + reconcile + `competition-storage.ts` helper + Supabase storage bucket created via dashboard); session-3 (admin reset + janitor cron) + custom multi-table viewer + Detailed User Guide content authoring + Chrome extension build remain | `workflow-2-competition-scraping` | 2026-05-07 w2-api-routes-session-2 (CODE session — single commit on `workflow-2-competition-scraping`. **No schema; no DB writes; no live run; no deploy. ONE external-state action: created the Supabase `competition-scraping` private storage bucket via the dashboard with director walkthrough.** 11 new/modified files, ~1450 LOC: shared types at `src/lib/shared-types/competition-scraping.ts` extended with sizes/text/images/reconcile wire shapes + MIME + size constants; new `src/lib/competition-storage.ts` Supabase Storage SDK wrapper (`requestUploadUrl`, `verifyUploadedFile`, `getThumbnailUrl`, `getFullSizeUrl`, `deleteImage`, `wipeProjectImages`) per `PLATFORM_REQUIREMENTS.md §10.4` portability requirement; new `src/lib/competition-storage-helpers.ts` (pure helpers — `composeStoragePath`, `extensionFromMimeType`, `splitStoragePath`, `isAcceptedMime`) split out of competition-storage.ts so node:test loads it without resolving `@/` aliases (cors-style split, same pattern as 2026-05-07 CORRECTIONS_LOG entry); 22 new unit tests at `src/lib/competition-storage-helpers.test.ts`; 5 new W#2 API route files: sizes collection (`urls/[urlId]/sizes/route.ts` POST + OPTIONS) + sizes per-row (`sizes/[sizeId]/route.ts` PATCH + DELETE + OPTIONS); text collection (`urls/[urlId]/text/route.ts` POST + OPTIONS — idempotent on clientId via create-then-catch-P2002) + text per-row (`text/[textId]/route.ts` PATCH + DELETE + OPTIONS); images requestUpload (`urls/[urlId]/images/requestUpload/route.ts` POST + OPTIONS — issues 5-min signed Supabase upload URL; no DB write) + images finalize (`urls/[urlId]/images/finalize/route.ts` POST + OPTIONS — verifies file landed in storage; creates DB row; idempotent on clientId) + images per-row (`images/[imageId]/route.ts` PATCH + DELETE + OPTIONS — DELETE removes row + best-effort storage cleanup; orphans handled by session-3 janitor); reconcile (`reconcile/route.ts` GET + OPTIONS — returns urlCount/textCount/imageCount/lastModified for periodic 5-min reconciliation per §8.3.2). All routes apply §11.2 cross-cutting items uniformly: `verifyProjectWorkflowAuth("competition-scraping")` (`verifyProjectAuth` only for vocabulary); two-step ownership check via `findFirst` with relation filter for sub-resource PATCH/DELETE (e.g., size → competitorUrl → projectWorkflowId — Prisma's update-where doesn't accept relation filters directly so we findFirst-then-update; cleanest, race-resilient via P2025 catch); `withRetry()` on every Prisma call; `markWorkflowActive()` after every mutation; `recordFlake()` on every catch with FlakeContext-valid keys only (storagePath/platform packed into console.error message instead — TS flagged the FlakeContext shape); `chrome-extension://*` CORS preflight via OPTIONS handler. **Two §11.1 spec deviations decided autonomously per Rule 15** (no user-visible difference) **with end-of-session §11.1 doc-batch update:** (1) image RPC paths reshaped from colon-suffix `images:requestUpload` / `images:finalize` to slash-based `images/requestUpload` / `images/finalize` for Next.js folder convention alignment + repo precedent (canvas/sister-links etc.) + URL-encoding cleanliness; (2) `images/finalize` body extended with `mimeType` + `sourceType` (required — schema columns require them; server re-derives storagePath from them rather than keeping intermediate state) + `fileSize` + `imageCategory` (optional — landed at finalize so PATCH isn't required for happy-path image uploads). Both echoed from the extension's WAL — no new client knowledge required. Shared types updated in lockstep. **Verification scoreboard:** 382/382 src/lib tests pass (was 360; +22 new from competition-storage-helpers.test.ts); tsc clean; build clean (45 routes; was 37 — added 8 new W#2 API routes); lint at exact baseline parity (16e/40w; zero new errors or warnings). **External-state action:** created the Supabase `competition-scraping` private storage bucket via the Supabase dashboard with director walkthrough — single bucket, OFF on Public/RestrictFileSize/RestrictMIMEtypes toggles per §3 (server-side enforcement is the canonical posture). **Out of scope (deferred to session-3):** admin reset endpoint (POST `.../competition-scraping/reset`); Vercel daily janitor cron for storage-orphan cleanup per §3. Schema-change-in-flight stays "No" — API + helper-only session, no schema changes. Multi-workflow: pull-rebase clean at session start; no parallel chat. One commit local; push pending Rule 9 approval — push to `workflow-2-competition-scraping` does not deploy vklf.com.) | (a) **API routes session-3 — admin reset endpoint + janitor cron — RECOMMENDED FIRST.** POST `.../competition-scraping/reset` (admin-only; type-the-project-name guard; wipes all W#2 data + storage files for the Project per §A.13 — uses the existing `wipeProjectImages` helper from session-2's `competition-storage.ts`) + Vercel daily cron for storage-orphan cleanup per §3 (wires the existing `vercel.json` cron pattern; new file). Wires the page's existing `handleResetConfirm` (today throws). ~80-150 LOC. Closes out the §11.1 route table. / (b) **Custom React multi-table viewer for content area** — workflow's own concern per `PLATFORM_REQUIREMENTS.md §12.6` shared component pattern #2. Platforms → URLs → captured-rows nav + sort/filter + image expand. Builds against today's GET `.../urls` + session-2's text/image/sizes endpoints. Multi-session. Today's placeholder block in the page can be replaced incrementally. / (c) **Detailed User Guide content authoring** per `COMPETITION_SCRAPING_STACK_DECISIONS.md §13.1.2`. Plain-language step-by-step + screenshots + printable PDF. Independent of (a) + (b). / (d) **W#2 Chrome extension build** — multi-session (5-7 sessions): WXT init → auth (`signInWithPassword`) → Module 1 (URL capture with floating "+ Add" button) → Module 2 (text + image + region-screenshot) → image upload (signed-URL — extension consumes session-2's `images/requestUpload` + `images/finalize`) → WAL + reconciler (consumes session-2's `reconcile` GET) + offline queue per `PLATFORM_REQUIREMENTS.md §10.1.1` → distribution. Can run in PARALLEL with (a)/(b) — all extension-callable endpoints are now live on this branch (session-1 + session-2). | No |
...
3. **W#2 PLOS-side build** (Phase-1 library available since 2026-05-06 merge) — schema migration via `prisma db push` ✅ DONE 2026-05-06 (flag flipped Yes → No). API routes per `COMPETITION_SCRAPING_STACK_DECISIONS.md §11` in 3 sessions: **session-1 ✅ DONE 2026-05-07 (commit `4166a4f`)** — read paths + `urls` POST/PATCH/DELETE + vocabulary endpoints (3 routes; 7 endpoints across GET/POST/PATCH/DELETE/OPTIONS; CORS preflight for chrome-extension origin; idempotent POST on the unique constraint); **session-2 ✅ DONE 2026-05-07** — sizes endpoints + text endpoints + image-upload two-phase flow (`images/requestUpload` + `images/finalize` per the slash-based path adoption) + reconcile GET + `src/lib/competition-storage.ts` helper wrapper + `src/lib/competition-storage-helpers.ts` (pure helpers, 22 unit tests) + Supabase `competition-scraping` private storage bucket created via dashboard with director walkthrough; session-3 — admin reset endpoint + janitor cron. Then: custom React content component for the multi-table viewer (composed alongside imported library components per `PLATFORM_REQUIREMENTS.md §12.6` shared component pattern #2 — content area is the workflow's own concern) → admin reset wired to the library's `<ResetConfirmDialog>` once session-3's endpoint ships → image expand viewer → Detailed User Guide content authoring (per §13.1.2) → (Phase 2) admin assignment UI.
