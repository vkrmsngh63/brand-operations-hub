...
| W#2 Competition Scraping & Deep Analysis | 🔄 Active dev — design phase | `workflow-2-competition-scraping` (created by W#2's first session 2026-05-04) | 2026-05-04 w2-workflow-requirements-interview (DESIGN-ONLY session — produced `docs/COMPETITION_SCRAPING_DESIGN.md` with §A frozen + §B empty append-only per Rule 18; ran Platform-Truths Audit per Rule 19 with director-approved batch of 7 additions to `PLATFORM_REQUIREMENTS.md` (§1.4, §2.2.1, §6 example + §6.6, §8.4, §10.1, §10.2, §12.6); pre-interview directive scan per Rule 21 found NO prior W#2-specific directives; updated `DATA_CATALOG.md` §6.1 (7 provisional entries) + §7.2.2 (filled W#2 row in Cross-Tool Data Flow Map). Cross-doc compliance: only W#2-relevant edits per MULTI_WORKFLOW_PROTOCOL Rule 3 ownership; no W#1 sections touched; commit pending end-of-session.) | (a) **W#2 Stack-and-Architecture session — RECOMMENDED FIRST.** Resolves the 13 deferred implementation questions in `COMPETITION_SCRAPING_DESIGN.md` §A.17: extension framework choice (Manifest V3 + vanilla / React-in-extension / Plasmo / WXT); auth pattern (long-lived API token vs OAuth device flow); image storage flow (`competition-scraping` Supabase bucket + signed-URL helper); region-screenshot mechanism (`chrome.tabs.captureVisibleTab` + canvas crop vs html2canvas); URL-add gesture (Shift+Click recommended); Highlight Terms color palette (~20 distinct, WCAG AAA); add-text gesture; polling cadence Phase 1 → Realtime Phase 2; schema design (sets schema-change-in-flight flag during the session per MULTI_WORKFLOW_PROTOCOL Rule 4); PLOS route structure; API route structure; build pipeline + distribution. Output: new Group B doc `COMPETITION_SCRAPING_STACK_DECISIONS.md` + updates to design doc §B. Design + decisions only; no code. ~1-2 sessions. / (b) **Shared Workflow-Tool Scaffold design + build (cross-workflow concern, NOT W#2-only).** Per `PLATFORM_REQUIREMENTS.md §12.4` must be built before W#2 PLOS-side build begins. Incorporates §12.6 extension-points (always-visible deliverables, custom React content components, external-client companion pattern). Multiple sessions. / (c) **W#2 PLOS-side build** — multi-session: schema migration → routes → custom React content component → admin reset → image expand viewer → (Phase 2) admin assignment UI. Uses scaffold from (b). / (d) **W#2 Chrome extension build** — multi-session: shell → auth → Module 1 capture flow → Module 2 capture flow → image upload → offline queue → distribution. Can run in parallel with (c) once stack is locked. | No (interview was design-only; no schema work; schema-change flag will flip to "Yes" during the W#2 Stack-and-Architecture session per MULTI_WORKFLOW_PROTOCOL Rule 4) |
...
**Workflow Requirements Interview status:** ✅ COMPLETED 2026-05-04 (session_2026-05-04_w2-workflow-requirements-interview). Deliverable: `docs/COMPETITION_SCRAPING_DESIGN.md` with §A frozen + §B empty append-only.
...
**Recommended next session sequence (per `COMPETITION_SCRAPING_DESIGN.md` §A.18):**

1. **W#2 Stack-and-Architecture session** (recommended FIRST after this design doc ships). Resolves 13 deferred implementation questions in §A.17: extension framework (Manifest V3 + vanilla / React-in-extension / Plasmo / WXT); auth pattern (long-lived API token vs OAuth device flow); image storage; region-screenshot mechanism; URL-add gesture; Highlight Terms color palette; polling cadence; schema design (will flip schema-change-in-flight flag); PLOS + API route structure; build pipeline + distribution. Output: new Group B doc `COMPETITION_SCRAPING_STACK_DECISIONS.md` + updates to design doc §B. Design + decisions only; no code. ~1-2 sessions.
2. **Shared Workflow-Tool Scaffold design + build** (cross-workflow concern, NOT W#2-only). Incorporates §12.6 extension-points. Multiple sessions.
3. **W#2 PLOS-side build** — schema migration → routes → custom React content component → admin reset → image expand viewer → (Phase 2) admin assignment UI. Uses scaffold from #2.
4. **W#2 Chrome extension build** — can run in parallel with #3 after stack is locked. Multiple sessions: shell → auth → Module 1 → Module 2 → image upload → offline queue → distribution.
