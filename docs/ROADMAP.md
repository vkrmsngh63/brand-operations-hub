...
|---|---|---|---|---|---|
| W#1 Keyword Clustering | 🔄 Active dev — stabilization (tracked toward 🎯 W#1 PRODUCTION-READINESS GATE — D3 RETRY entry below; 3 prerequisites in dependency order: cold-start verification [#1 PARTIAL — happy path confirmed live 2026-05-03-b; banner UI deferred to natural flake event] → recency-stickiness fix [#2 🟡 CODE FIX SHIPPED 2026-05-03-c — pending live verification at next D3 attempt] → underlying flake-rate fix [#3 NEXT] → D3 retry attempt) | `main` | 2026-05-03-c recency-stickiness-fix (CODE-FIX session — no live run, no deploy yet. Two-part fix per INPUT_CONTEXT_SCALING_DESIGN.md §6 D3 partial validation outcome: (1) sister-link op deferral — V4 prompt drops ADD_SISTER_LINK + REMOVE_SISTER_LINK from per-batch operation vocabulary + applier gains regularBatchMode: true flag mirroring consolidationMode + AutoAnalyze.tsx wires the flag at three regular-batch apply call sites; (2) Q5→B touch-semantics refinement — recordTouchesFromOps stamps only topics whose own structural identity changed. Defense in depth: prompt restriction + applier rejection + tests. 299/299 src/lib tests pass (was 284; +15 new — 7 applier regularBatchMode tests + 8 touch-tracker structural-identity tests). tsc clean; build clean (25 routes); lint at exact baseline parity (16e/41w; zero new). Multi-workflow: schema-change-in-flight stays "No"; W#2 still 🆕 about-to-start; no parallel chat. Pull-rebase clean at session start. Commit local; push pending Rule 9 approval. Director MUST re-paste the updated V4 Initial Prompt + Primer into the Auto-Analyze UI before the next D3 attempt.) | (a) **Underlying flake-rate investigation — prerequisite #3 of the W#1 PRODUCTION-READINESS GATE — RECOMMENDED.** Measure rate per-endpoint with structured telemetry; investigate Supabase plan tier / pgbouncer pool sizing / Prisma client management / server-side withRetry parity; ~2-3 sessions. **Most thorough next step** because it's the last named gate prerequisite — closing it lets D3 attempt with all three prerequisites verified rather than running D3 with the flake rate as a known headwind. Director needs to share Supabase plan tier info during this session. / (b) D3 attempt now — skip prerequisite #3 and accept ~$20-30 retry overhead + elevated mid-run-blanking risk; faster path. / (c) GoTrueClient multi-instance fix — ~15 LOC. / (d) Phase-1 UI polish bundle. / (e) V3-era cleanup pass (deferred from Session E D4). / (f) **Action-by-action feedback workflow + Prompt Refining button — EXPLICITLY LAST per director's 2026-05-03-b directive** (consolidates the 2026-04-20 button entry per Rule 24); ~5–7 sessions (1 design + 4–6 build). | No |
...
3. ❌ **Underlying ~25% pgbouncer/Prisma flake-rate fix** — measure with structured telemetry; investigate Supabase plan tier / pgbouncer pool sizing / Prisma client management / server-side `withRetry` parity audit. ~2-3 sessions. (Item (b) on the W#1 standing instructions.) **Most-thorough path requires this; fastest path optionally skips it and accepts higher cost + risk during D3 run.**
...
- **Most thorough path (recommended per `feedback_recommendation_style.md`):** 4-6 sessions = (1) cold-start verification + (1-2) recency-stickiness fix + (2-3) underlying flake-rate investigation + fix + (1) D3 attempt
- **Fastest path:** 2-3 sessions = (1) cold-start verification + (1-2) recency-stickiness fix + (1) D3 attempt with elevated cost (~$20-30 in flake-retry overhead per the 2026-05-02-e empirical data) and elevated risk of mid-run blanking events that today's cold-start fix mitigates but doesn't eliminate
