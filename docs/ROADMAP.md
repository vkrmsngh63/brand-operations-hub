...
|---|---|---|---|---|---|
| W#1 Keyword Clustering | 🔄 Active dev — stabilization (tracked toward 🎯 W#1 PRODUCTION-READINESS GATE — D3 RETRY entry below; 3 prerequisites in dependency order: cold-start verification [#1 PARTIAL — happy path confirmed live 2026-05-03-b; banner UI deferred to natural flake event] → recency-stickiness fix [#2 🟡 CODE FIX SHIPPED 2026-05-03-c — pending live verification at next D3 attempt] → underlying flake-rate fix [#3 🟡 INSTRUMENTATION SHIPPED 2026-05-04 — pending live data + first rate-fix shipping] → D3 retry attempt) | `main` | 2026-05-04 underlying-flake-rate-investigation (CODE + DOCS session — no live run, no deploy yet. Measurement-instrumentation pass producing structured per-endpoint flake telemetry across all 19 DB-backed API routes. New pure helper `src/lib/flake-counter.ts` (~140 LOC) + 28 unit tests; all 19 DB-backed routes wired (38 catch blocks ↔ 38 recordFlake call sites — exact 1-to-1; additive only); 2 wrapped routes record with `retried: true`; `/canvas/rebuild` POST captures `body.nodes.length` as canvasSize. Investigation findings: Supabase confirmed using transaction pooler (port 6543, pgbouncer=true) — pgbouncer hypothesis grounded; withRetry parity audit shows ~4% endpoint coverage AND silent unprotected hot helpers in `lib/auth.ts` + `lib/workflow-status.ts`; Prisma client uses bare singleton with no connection-lifecycle management. 327/327 src/lib tests pass (was 299; +28 new); tsc clean; build clean (25 routes); lint at baseline-or-better parity (16e/40w; zero new errors; net -1 warning). Multi-workflow: schema-change-in-flight stays "No"; W#2 still 🆕 about-to-start; no parallel chat. Pull-rebase clean at session start. Commit local; push pending Rule 9 approval. Push is purely additive on success path — only error-path catch blocks emit one extra log line; safe to deploy whenever director chooses; data-collection clock starts after push.) | (a) **Flake-rate analysis + first rate-fix — prerequisite #3 of the W#1 PRODUCTION-READINESS GATE — RECOMMENDED.** Director shares Supabase plan tier; Claude analyzes `[FLAKE]` log lines from Vercel for the live data window since this session's push (likely 1-3 days of normal use); proposes concrete fix grounded in real per-endpoint counts + canvasSize correlation; picks highest-leverage option from ROADMAP (a)–(d): Supabase plan-tier upgrade or pool-sizing change / withRetry parity wrap of hottest unwrapped endpoints / Prisma client lifecycle change via @prisma/adapter-pg / atomic-rebuild differentialization. ~1-2 sessions for first fix. **Most thorough next step** because the data window is the bottleneck — without it any fix is a guess. / (b) D3 attempt now — skip prerequisite #3 fix, accept ~25% pgbouncer flake rate as headwind + use the run as the natural data window for `[FLAKE]` lines. ~$30-50 + 3-5 hr; faster path. / (c) GoTrueClient multi-instance fix — ~15 LOC. / (d) Phase-1 UI polish bundle. / (e) V3-era cleanup pass (deferred from Session E D4). / (f) **Action-by-action feedback workflow + Prompt Refining button — EXPLICITLY LAST per director's 2026-05-03-b directive** (consolidates the 2026-04-20 button entry per Rule 24); ~5–7 sessions (1 design + 4–6 build). | No |
...
3. 🟡 **Underlying ~25% pgbouncer/Prisma flake-rate fix (INSTRUMENTATION SHIPPED 2026-05-04 — pending live data + first rate-fix shipping)** — measurement-instrumentation pass landed 2026-05-04. New pure helper `src/lib/flake-counter.ts` + 28 unit tests; all 19 DB-backed API routes wired with `recordFlake(endpoint, error, ctx)` calls in their catch blocks (38 catch blocks ↔ 38 call sites — exact 1-to-1; additive only). Each `[FLAKE]` log line includes endpoint + Prisma code + retried flag + optional canvasSize + optional projectWorkflowId + truncated msg, grep-able from Vercel logs. **Investigation findings (2026-05-04):** Supabase confirmed using transaction pooler (port 6543, `?pgbouncer=true`); withRetry parity audit shows only 2 of ~50 endpoint-method combinations have G2 coverage (~4%); auth + activity-tracking helpers in `lib/auth.ts` + `lib/workflow-status.ts` are unwrapped silent multipliers behind even the protected routes; Prisma client at `src/lib/db.ts` uses bare singleton with no `$connect`/`$disconnect` lifecycle. **Pending:** (a) push of telemetry to vklf.com to start the data-collection clock, (b) ~1-3 days of normal director use to populate the `[FLAKE]` log volume, (c) next session to analyze + ship the highest-leverage of options (a)–(e) from the "NEW HIGH" entry below grounded in real per-endpoint counts + canvasSize correlation. (Item (a) on the W#1 standing instructions; was item (b) on prior session's list — promoted to recommended after recency-stickiness fix shipped.) **Estimated ~1-2 more sessions for first rate-fix; (b)-(d) options can stack over time afterward.**
...
- **Most thorough path (recommended per `feedback_recommendation_style.md`):** 4-6 sessions total — as of 2026-05-04: cold-start verification + recency-stickiness fix + flake-rate INSTRUMENTATION are all DONE; remaining = (1-2) flake-rate analysis + first rate-fix grounded in live `[FLAKE]` data + (1) D3 attempt. **2-3 sessions to gate completion from the 2026-05-04 endpoint.**
- **Fastest path:** 1-2 sessions remaining = (1) D3 attempt now with elevated cost (~$20-30 in flake-retry overhead per 2026-05-02-e empirical data) and elevated risk of mid-run blanking events that today's cold-start fix mitigates but doesn't eliminate. The D3 run itself produces the first natural data window for `[FLAKE]` lines, which would inform any subsequent rate-fix session.
...
1. ✅ **First investigation session — DONE 2026-05-04** — measurement instrumentation shipped (`src/lib/flake-counter.ts` + 28 tests + all 19 DB-backed API routes wired with `recordFlake` in their catch blocks). withRetry parity audit completed (read-only inspection — see Investigation Findings below). Supabase connection mode confirmed (transaction pooler at port 6543; pgbouncer hypothesis grounded). Supabase plan tier itself NOT yet shared by director — needs to come at the next session. Pending: push telemetry to vklf.com to start the data-collection clock; 1-3 days of normal use populates the `[FLAKE]` log volume.
2. **First fix session (NEXT — recommended):** analyze the live `[FLAKE]` data window from Vercel for per-endpoint counts + `code=` distribution + `retried=` ratio + `canvasSize` correlation on `/canvas/rebuild`. Director shares Supabase plan tier. Propose + ship the highest-leverage of (a)-(e) below based on the data. (a) is likely cheapest if Supabase plan-tier change is feasible; (c) [withRetry parity wrap of unwrapped routes including the silent multiplier helpers in `lib/auth.ts` + `lib/workflow-status.ts`] is broad and cheap; (d) [atomic-rebuild differentialization] is highest-effort and most leveraged on the apply-pipeline specifically.
...
**Estimated effort (remaining from 2026-05-04 endpoint):** 1-2 sessions for first rate-fix grounded in live data; (b)-(d) can be deferred and stacked over time.
