...
4. **W#2 Chrome extension build — RECOMMENDED FIRST.** Now the natural next priority since #3's PLOS-side build is essentially complete (slice (a.x) viewer + slice (b) Detailed User Guide all shipped). Multi-session (5–7 sessions): WXT init → auth (`signInWithPassword`) → Module 1 (URL capture with floating "+ Add" button) → Module 2 (text save with popover + image save + region-screenshot) → image upload (signed-URL direct flow consuming `images/requestUpload` + `images/finalize` from API session-2) → WAL + reconciler (consuming `reconcile` GET) + offline queue per `PLATFORM_REQUIREMENTS.md §10.1.1` sync-reliability requirements → distribution. All extension-callable PLOS endpoints are live on this branch. Extension build also unblocks: (d) live verification of admin reset + janitor (gated by `CRON_SECRET` in Vercel + captured data from extension); (f) walk-through of `COMPETITION_SCRAPING_VERIFICATION_BACKLOG.md` slice (a.1)/(a.2)/(a.3)/(a.4) checklists (gated by extension-populated seed data).
