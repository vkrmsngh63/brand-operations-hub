...
**Last updated:** April 19, 2026 (Phase 1g-test follow-up Part 2 — stale-closure + missing-await bugs fixed, deployed, and validated live across 7 clean Bursitis batches; quantitative trajectory analysis proves Mode A alone cannot finish a full 2,304-keyword run — proactive Mode A→B switch promoted to functional prerequisite)
...
- 🎯 **Phase 1g-test follow-up REMAINING (for next session or later):**
  1. **Finish or re-launch the full Bursitis clustering run with proactive Mode A→B switch shipped.** Active run currently continuing in browser tab as of 2026-04-19 session wrap-up — left running independently of session. Projected to hit 200k context wall at ~120-140 canvas nodes (240-600 of 2,304 keywords placed, 10-26%) unless reactive Mode A→B switch fires in time. See §Phase 1-polish "Proactive Mode A→B switch" item below — **this is now a functional prerequisite for a complete run**, not just a cost-optimization.
...
- **Phase 1-polish items — accumulated across kickoff + follow-up sessions:**
  ...
  - 🚨 **Proactive Mode A → Mode B switch after batch 1** (PRIORITY ELEVATED 2026-04-19 to functional prerequisite; originally added 2026-04-18 follow-up): once the topics table is non-empty (i.e., after batch 1 applies), automatically use Mode B for all subsequent batches without waiting for a Mode A failure. **Reframed 2026-04-19:** this is no longer just a cost-optimization — quantitative trajectory analysis during the 2026-04-19 session showed that Mode A alone hits the 200k context wall at ~120-140 canvas nodes (only 10-26% of a 2,304-keyword dataset like Bursitis), and relying on the reactive Mode A→B switch to fire in time is a race condition (Mode A may stay "clean" past the point where switching is still safe — once input tokens alone approach 150k+, even Mode B's small delta outputs can't keep total context under 200k). **A proactive switch is therefore a functional prerequisite for any full-dataset clustering run**, not a nice-to-have. See `CORRECTIONS_LOG.md` 2026-04-19 entry "Mode-A-alone cannot complete" for the trajectory math. Complements (doesn't replace) the reactive auto-switch already in place.
