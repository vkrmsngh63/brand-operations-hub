...
| W#2 Competition Scraping & Deep Analysis | 🔄 Active dev — schema migrated 2026-05-06 (7 new tables live in Supabase Pro); first PLOS-side page slice live composing the components library; API routes + custom multi-table viewer + Detailed User Guide content authoring + admin reset endpoint wiring + Chrome extension build all deferred to follow-up sessions | `workflow-2-competition-scraping` | 2026-05-06 w2-plos-side-build-first-slice (CODE + SCHEMA session — three commits: `f59111a` merge of main into branch [Phase-1 components library + 2026-05-04 doc reframes + W#1 2026-05-05-b/c/d work]; `d45f976` residual stale-scaffold-wording sweep on COMPETITION_SCRAPING_DESIGN.md §A.0/§A.7/§A.9/§A.16/§A.18; `701775f` schema migration via `prisma db push` [7 W#2 tables + 2 back-relations; additive only; W#1 untouched] + new page at `/projects/[projectId]/competition-scraping` composing 7 library components. 349/349 tests pass; tsc clean; build clean [34 routes; was 33]; lint at exact baseline parity. **Schema-change-in-flight flag flipped Yes → No this session.** No deploy [branch is not main; vklf.com unaffected]. Three commits local; push pending Rule 9 approval.) | (a) **API routes per `COMPETITION_SCRAPING_STACK_DECISIONS.md §11` — RECOMMENDED FIRST.** ~17 endpoints across `urls`, `urls/[urlId]/sizes`, `urls/[urlId]/text`, `urls/[urlId]/images:requestUpload` + `:finalize`, `vocabulary`, `reset`, `reconcile`. Authoritative spec frozen; multi-session build (likely 2-3 sessions). Recommended split: session-1 = read paths + `urls` POST/PATCH/DELETE + vocabulary; session-2 = text + sizes + image-upload two-phase flow + reconcile; session-3 = admin reset + janitor cron. / (b) **Custom React multi-table viewer for content area** — workflow's own concern per `PLATFORM_REQUIREMENTS.md §12.6` shared component pattern #2. Platforms → URLs → captured-rows nav + sort/filter + image expand. Builds against (a)'s API routes. Multi-session. Today's placeholder block in the page can be replaced incrementally as (a) ships endpoints. / (c) **Detailed User Guide content authoring** per `COMPETITION_SCRAPING_STACK_DECISIONS.md §13.1.2`. Plain-language step-by-step + screenshots + printable PDF. Independent of (a)+(b). / (d) **Admin reset endpoint wiring** — page already has the dialog but `handleResetConfirm` throws explicitly until the endpoint ships in (a)'s session-3. ~30-50 LOC server-side once (a) is in flight. / (e) **W#2 Chrome extension build** — multi-session (5-7 sessions): WXT init → auth (`signInWithPassword`) → Module 1 (URL capture with floating "+ Add" button) → Module 2 (text + image + region-screenshot) → image upload (signed-URL) → WAL + reconciler + offline queue per `PLATFORM_REQUIREMENTS.md §10.1.1` → distribution. Can run in PARALLEL with (a)+(b) once schema is locked (it is). | No (flipped Yes → No 2026-05-06 after `prisma db push` landed the W#2 schema in the live Supabase Pro DB; future schema work on any workflow is unblocked from a DB-conflict perspective. The flag effectively becomes "No" platform-wide once this branch merges to main at a future milestone.) |
...
3. **W#2 PLOS-side build** (Phase-1 library now available; current session) — schema migration via `prisma db push` (flips schema-change-in-flight from "Yes" → "No" upon push) → API routes per `COMPETITION_SCRAPING_STACK_DECISIONS.md §11` → custom React content component for the multi-table viewer (composed alongside imported library components per `PLATFORM_REQUIREMENTS.md §12.6` shared component pattern #2 — content area is the workflow's own concern) → admin reset (uses `<ResetWorkflowButton>` + `<ResetConfirmDialog>` from the library) → image expand viewer → Detailed User Guide content authoring (per §13.1.2) → (Phase 2) admin assignment UI.
