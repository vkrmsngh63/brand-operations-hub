...
**Last updated in session:** session_2026-05-04_w2-workflow-requirements-interview (Claude Code, on `workflow-2-competition-scraping` branch)
...
1. D3 status flips PARTIAL → ✅ COMPLETE in `INPUT_CONTEXT_SCALING_DESIGN.md` §6 + Scale Session E formally closes.
2. **W#1 is declared production-stable.** Per `HANDOFF_PROTOCOL.md` Rule 22 (Graduated-Tool Re-Entry Protocol), this becomes the W#1 graduation candidacy event — `KEYWORD_CLUSTERING_ACTIVE.md` is split into `KEYWORD_CLUSTERING_ARCHIVE.md` + `KEYWORD_CLUSTERING_DATA_CONTRACT.md` once a downstream tool needs the data (the trigger condition for graduation; not D3 alone).
3. The cold-start render-layer entry + the underlying-flake-rate entry both flip from open → ✅ COMPLETE assuming their respective verification passed during D3.
4. W#1's row in the Active Tools table flips from 🔄 Active dev — stabilization → ✅ Graduated (assuming graduation trigger met) or stays 🔄 Active dev with reduced session cadence (if graduation trigger not yet met).
...
**Original prerequisite:** Phase 1α — Shared Workflow-Tool Scaffold must be designed and built BEFORE W#2 PLOS-side build begins. Per `PLATFORM_REQUIREMENTS.md §12.4` + §12.6 + `HANDOFF_PROTOCOL.md` Rule 20. Per Q14 of the interview, scaffold fit is PARTIAL on the PLOS side; the Chrome extension is wholly outside the scaffold (different deployment model). The interview surfaced 3 new scaffold-extension-points captured in `PLATFORM_REQUIREMENTS.md §12.6`.

**Recommended next session sequence (UPDATED 2026-05-04 post-Stack-and-Architecture; original sequence in `COMPETITION_SCRAPING_DESIGN.md` §A.18):**
...
2. **Shared Workflow-Tool Scaffold design + build** (cross-workflow concern, NOT W#2-only). RECOMMENDED FIRST among remaining items. Incorporates `PLATFORM_REQUIREMENTS.md §12.6` extension-points. Multiple sessions. Every workflow #3-#14 inherits the scaffold; building it is gate-blocking for W#2 PLOS-side build.
3. **W#2 PLOS-side build** (after scaffold ships) — schema migration via `prisma db push` (flips schema-change-in-flight from "Yes" → "No" upon push) → API routes per `COMPETITION_SCRAPING_STACK_DECISIONS.md §11` → custom React content component plugged into scaffold per §12.6 #2 → admin reset → image expand viewer → Detailed User Guide content authoring (per §13.1.2) → (Phase 2) admin assignment UI.
4. **W#2 Chrome extension build** — can run in PARALLEL with #3 once schema is locked AND scaffold is done. Multi-session: WXT scaffold init → auth (`signInWithPassword`) → Module 1 (URL capture with floating "+ Add" button) → Module 2 (text save with popover + image save + region-screenshot) → image upload (signed-URL direct flow) → WAL + reconciler + offline queue per `PLATFORM_REQUIREMENTS.md §10.1.1` NEW sync-reliability requirements → distribution.
