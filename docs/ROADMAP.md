...
| W#2 Competition Scraping & Deep Analysis | 🔄 Active dev — schema live 2026-05-06; first PLOS-side page slice live; **API-routes session-1 SHIPPED 2026-05-07** (read paths + `urls` POST/PATCH/DELETE + vocabulary endpoints); **API-routes session-2 SHIPPED 2026-05-07** (sizes + text + image two-phase upload + reconcile + `competition-storage.ts` helper + Supabase storage bucket created via dashboard); **API-routes session-3 SHIPPED 2026-05-08** (admin reset endpoint + Vercel daily janitor cron + page handleResetConfirm wired); **custom multi-table viewer first slice SHIPPED 2026-05-06** (platforms→URLs sidebar nav + sortable URL list with free-text search + click-row-to-open-in-new-tab + empty/filter-empty/loading/error states; visual-verified live on dev server against a real Project); URL detail page (`/url/[urlId]`) + inline editing of URL fields + per-column filter dropdowns + captured-text + captured-image rows + image expand viewer + Detailed User Guide content authoring + Chrome extension build remain | `workflow-2-competition-scraping` | 2026-05-06 w2-plos-side-viewer-first-slice (CODE session — single commit `4481bd1` on `workflow-2-competition-scraping`. **No schema; no DB writes; no deploy. Visual-verified live on dev server against a real Project.** First slice of the custom React multi-table viewer for the W#2 content area per `COMPETITION_SCRAPING_DESIGN.md §A.7 + §A.14` and `PLATFORM_REQUIREMENTS.md §12.6` shared component pattern #2 (the content area is the workflow's own concern, not imposed by the library). W#2 is the first workflow to author such a custom content component. ~4 new/modified files (606 insertions, 28 deletions): new `src/app/projects/[projectId]/competition-scraping/components/CompetitionScrapingViewer.tsx` (top-level — fetches `GET .../urls`, owns selected-platform + free-text search state, syncs `?platform=…` to the URL bar so refresh preserves the view, derives per-platform counts client-side, renders sidebar + table); new `PlatformSidebar.tsx` (sticky left nav with `All Platforms` row + seven supported platforms, each with per-row count, click-to-scope, current selection highlighted in `#1f6feb22`); new `UrlTable.tsx` (sortable columns — URL · Product Name · Brand Name · Category · Product Stars · # Reviews · Added On — with click-to-toggle-direction, free-text search across URL+Product+Brand, click-row-to-open-in-new-tab via `window.open(target='_blank')` + keyboard Enter handler for accessibility, separate empty / filter-empty / loading / error states); modified `page.tsx` to replace the prior session's placeholder content section with `<CompetitionScrapingViewer projectId={ctx.project.id} />`. **Slice scope was director-approved at session start via Read-It-Back per Rule 18 mid-build directive pattern;** explicitly deferred to follow-up slices: the `/url/[urlId]` detail page, inline editing of URL fields, per-column filter dropdowns, captured-text + captured-image rows, image expand viewer, Phase-2 admin assignments view. **Director feedback captured during visual verification (3 deferred items):** (1) `/plos` dashboard top nav has no `Projects` link — captured as new Phase-1 polish item below; (2) `/projects` list should have copy-to-clipboard affordance for Project ID + Project page URL — captured as new Phase-1 polish item below; (3) W#2 card-flip on `/plos` dashboard + Project detail page (currently `active: false, route: null`) — kept disabled today; release-gate decision deferred to a future session, captured in `COMPETITION_SCRAPING_DESIGN.md §B`. **Verification scoreboard:** 380/380 src/lib tests pass (no test changes; pure UI work didn't touch anything tested under `src/lib`); tsc clean; build clean (47 routes; existing `competition-scraping` route now ships viewer instead of placeholder); lint at exact baseline parity (16e/40w; zero new errors or warnings; none of the new files appear in lint output). Schema-change-in-flight stays "No". Multi-workflow: pull-rebase clean at session start; no parallel chat; W#1 row untouched per Rule 3 ownership. **TaskList sweep:** 7 session tasks tracked + 4 completed; 3 `DEFERRED:` tasks closed only after their destination doc entries were written this session per Rule 26. One code commit (`4481bd1`) + one doc-batch commit local; push pending Rule 9 approval — push to `workflow-2-competition-scraping` does not deploy vklf.com.) | (a.1) **Next slice — `/url/[urlId]` detail page + per-URL captured-text rows display — RECOMMENDED FIRST.** Builds against session-2's `GET .../urls/[urlId]/text` + `GET .../urls/[urlId]/images` (or whichever read paths apply). Deep-linkable per `COMPETITION_SCRAPING_STACK_DECISIONS.md §10`. Continues the multi-table viewer's natural drill-down from URL row → URL detail. ~1 session. / (a.2) **Following slice — image expand viewer + image rendering inside URL detail.** Image rows + click-to-expand modal; signed-URL fetch path. ~1 session. / (a.3) **Inline editing of URL fields** (Competition Category, Product Name, Brand Name, ratings, custom fields) using existing `PATCH .../urls/[urlId]`. Adds vocabulary picker UX for category/product/brand fields. ~1-2 sessions. / (a.4) **Per-column filter dropdowns** on the URL table to complement the free-text search. ~1 session. / (b) **Detailed User Guide content authoring** per `COMPETITION_SCRAPING_STACK_DECISIONS.md §13.1.2`. Plain-language step-by-step + screenshots + printable PDF. Independent of (a.x). / (c) **W#2 Chrome extension build** — multi-session (5-7 sessions): WXT init → auth (`signInWithPassword`) → Module 1 (URL capture with floating "+ Add" button) → Module 2 (text + image + region-screenshot) → image upload (signed-URL — extension consumes session-2's `images/requestUpload` + `images/finalize`) → WAL + reconciler (consumes session-2's `reconcile` GET) + offline queue per `PLATFORM_REQUIREMENTS.md §10.1.1` → distribution. Can run in PARALLEL with (a.x)/(b) — all extension-callable endpoints are now live on this branch (session-1 + session-2 + session-3). / (d) **Live verification of admin reset + janitor** — once `CRON_SECRET` is set in Vercel and W#2 has captured data on a real Project, exercise the reset path end-to-end (typed-name guard happy path + mismatch path + storage wipe verification) and confirm the cron runs at next 03:00 UTC. Janitor logs are grep-able via Vercel's log search. / (e) **W#2 card-flip release-gate decision** — currently `active: false, route: null` on both PLOS dashboard and Project detail page; flip when (a.1)+(a.2)+(c) are sufficient that a card-click lands somewhere with enough to do. Captured in `COMPETITION_SCRAPING_DESIGN.md §B`. | No |
