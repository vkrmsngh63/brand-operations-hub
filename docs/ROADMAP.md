...
**Last updated in session:** session_2026-05-07-d_w2-plos-side-viewer-column-filters (Claude Code, on `workflow-2-competition-scraping` branch)
...
| W#2 Competition Scraping & Deep Analysis | 🔄 Active dev — schema live 2026-05-06; first PLOS-side page slice live; **API-routes session-1 SHIPPED 2026-05-07** (read paths + `urls` POST/PATCH/DELETE + vocabulary endpoints); **API-routes session-2 SHIPPED 2026-05-07** (sizes + text + image two-phase upload + reconcile + `competition-storage.ts` helper + Supabase storage bucket created via dashboard); **API-routes session-3 SHIPPED 2026-05-08** (admin reset endpoint + Vercel daily janitor cron + page handleResetConfirm wired); **custom multi-table viewer first slice SHIPPED 2026-05-06** (platforms→URLs sidebar nav + sortable URL list with free-text search + click-row-to-open-in-new-tab + empty/filter-empty/loading/error states; visual-verified live on dev server against a real Project); **URL detail page slice (a.1) SHIPPED 2026-05-07** (chrome + sub-breadcrumb + URL metadata read-only grid + sizes sub-section + sortable captured-text rows table + image-count placeholder + 4 new GET read paths + UrlTable click-row navigation rewired in-app); **URL detail page slice (a.2) SHIPPED 2026-05-07-b** (image gallery 200×200 thumbnails + full-size viewer modal with metadata sidebar + arrow-key prev/next + Esc/backdrop/✕ close + region-screenshot badge + broken-image fallback; signed-URL minting added to `GET .../urls/[urlId]/images` via existing `competition-storage.ts` helpers); **URL detail page slice (a.3) SHIPPED 2026-05-07-c** (per-field inline editing of URL metadata: vocabulary picker for Product Name / Brand Name / Category; numeric inputs with min/max/integer validation for the four ratings/counts + Results Page Rank; key/value editor for customFields with rename-preserves-order; one-PATCH-per-save with optimistic update + rollback via authoritative server response; 3 new components `EditableField` / `VocabularyPicker` / `CustomFieldsEditor`; `UpdateCompetitorUrlRequest` extended additively to allow null on nullable fields); **URL detail page slice (a.4) SHIPPED 2026-05-07-d** (per-column filter dropdowns on the URL list table — multi-select for Product Name/Brand Name/Category with `(blank)` pseudo-row + search-within-list when ≥ 7 options; min/max numeric for Product Stars/# Reviews; from/to date for Added On; URL-state persistence via repeated keys for multi-select with `__blank__` token + single-key for ranges + NEW `?q=` for the free-text search box with 250ms debounce; back/forward + deep-link round-trip; new `ColumnFilters.tsx` component with `MultiSelectFilter`/`NumericRangeFilter`/`DateRangeFilter` primitives + popover shell + URL-query serialization helpers + pure `applyColumnFilters` row filter); Detailed User Guide content authoring + Chrome extension build remain | `workflow-2-competition-scraping` | 2026-05-07-d w2-plos-side-viewer-column-filters (CODE session — single code commit `81728ec` on `workflow-2-competition-scraping`. **No schema; no DB writes; no live run; no deploy. Visual verification deliberately deferred per the standing 2026-05-07 director call** until the Chrome extension provides a data path. 4 files changed (1242 insertions, 52 deletions): NEW `ColumnFilters.tsx` (popover shell with outside-click + Esc dismiss; `MultiSelectFilter` for vocabulary cols with `(blank)` pseudo-row + Apply/Clear footer + search-within-list when ≥ 7 options; `NumericRangeFilter` with min/max validation + range validation; `DateRangeFilter` with from/to YYYY-MM-DD inputs + range validation; URL-query serialization helpers `readFiltersFromQuery`/`writeFiltersToQuery`; pure `applyColumnFilters` row filter; `computeDistinctValues` helper for option lists); modified `UrlTable.tsx` (filterable header `<th>`s now render label-button-for-sort + funnel-icon-for-filter as two interactive zones; funnel turns blue with a small dot when filter active; column filters apply BEFORE sort; "Clear all filters (N active)" toolbar button; "Showing N of M" reflects all active filters); modified `CompetitionScrapingViewer.tsx` (filter state derived from URL query each render via `readFiltersFromQuery`; on filter change, `router.replace` writes via `writeFiltersToQuery`; multi-select option lists derive from PLATFORM-scoped row set NOT search-narrowed so filters don't shrink other filters' options; new `?q=` for free-text search box with 250ms debounce write so each keystroke doesn't trigger router flush; back/forward + deep-link round-trip); `COMPETITION_SCRAPING_VERIFICATION_BACKLOG.md` slice (a.4) section appended with 30 walked-through tests + seed-data prerequisites + API-side confirmation summary (zero API surface change). Filters cover 6 columns (Product Name/Brand Name/Category multi-select; Product Stars/# Reviews numeric range; Added On date range); URL column covered by free-text search; Platform column covered by sidebar — both out of scope. All filters AND-combine with each other and with the search box. Director-approved scope at session start via Rule 14f `AskUserQuestion` (Option A "all visible columns + URL-state persistence (recommended)" picked — 4 options surfaced including vocabulary-cols-only variant + visible-plus-3-hidden-fields variant + escape hatch); concrete-scope read-back per Rule 18 mid-build directive Read-It-Back covering trigger location (funnel-icon-inside-header vs filter-row-below-headers picked inline icon for sort-affordance consistency + table compactness; flagged as a judgment call), 6-column matrix, AND-combine semantics, URL-query convention, deferral list, verification approach approved before code. **Verification scoreboard:** 393/393 src/lib tests pass (no test changes — pure UI work); tsc clean; build clean (49 routes; same as slice (a.3) — zero new API routes); lint at exact baseline parity (project-wide 13e/39w; the 13 errors all live in pre-existing files outside the W#2 surface) AND `npx eslint` on JUST the slice (a.4) files (`ColumnFilters.tsx`, `UrlTable.tsx`, `CompetitionScrapingViewer.tsx`) reports clean — zero errors, zero warnings. Schema-change-in-flight stays "No". Multi-workflow: pull-rebase clean at session start; explicit `git fetch` confirmed branch in sync with origin right before commit; no parallel chat; W#1 row untouched per Rule 3 ownership. **TaskList sweep:** 10 tasks tracked + completed; zero `DEFERRED:` items at any point per Rule 26. One code commit (`81728ec`) + one doc-batch commit local; push pending Rule 9 approval — push to `workflow-2-competition-scraping` does not deploy vklf.com.) | (b) **Detailed User Guide content authoring** per `COMPETITION_SCRAPING_STACK_DECISIONS.md §13.1.2`. Plain-language step-by-step + screenshots + printable PDF. Independent of (a.x). / (c) **W#2 Chrome extension build** — multi-session (5-7 sessions): WXT init → auth (`signInWithPassword`) → Module 1 (URL capture with floating "+ Add" button) → Module 2 (text + image + region-screenshot) → image upload (signed-URL — extension consumes session-2's `images/requestUpload` + `images/finalize`) → WAL + reconciler (consumes session-2's `reconcile` GET) + offline queue per `PLATFORM_REQUIREMENTS.md §10.1.1` → distribution. Can run in PARALLEL with (a.x)/(b) — all extension-callable endpoints are now live on this branch (session-1 + session-2 + session-3 + slice (a.1)'s 4 GETs). / (d) **Live verification of admin reset + janitor** — once `CRON_SECRET` is set in Vercel and W#2 has captured data on a real Project, exercise the reset path end-to-end (typed-name guard happy path + mismatch path + storage wipe verification) and confirm the cron runs at next 03:00 UTC. / (e) **W#2 card-flip release-gate decision** — currently `active: false, route: null` on both PLOS dashboard and Project detail page; flip when (a.1)+(a.2)+(c) are sufficient that a card-click lands somewhere with enough to do. Captured in `COMPETITION_SCRAPING_DESIGN.md §B`. / (f) **Walk through `COMPETITION_SCRAPING_VERIFICATION_BACKLOG.md` slice (a.1) checklist** — the 12 deferred visual checks for slice (a.1) + any additional checks future slices append; runs as a single dedicated session AFTER the extension can populate test data per (c) (or after seed data is otherwise available). | No |
