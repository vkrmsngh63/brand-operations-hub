...
| W#2 Competition Scraping & Deep Analysis | 🔄 Active dev — schema live 2026-05-06; first PLOS-side page slice live; **API-routes session-1 SHIPPED 2026-05-07** (read paths + `urls` POST/PATCH/DELETE + vocabulary endpoints); **API-routes session-2 SHIPPED 2026-05-07** (sizes + text + image two-phase upload + reconcile + `competition-storage.ts` helper + Supabase storage bucket created via dashboard); **API-routes session-3 SHIPPED 2026-05-08** (admin reset endpoint + Vercel daily janitor cron + page handleResetConfirm wired); custom multi-table viewer + Detailed User Guide content authoring + Chrome extension build remain | `workflow-2-competition-scraping` | 2026-05-08 w2-api-routes-session-3 (CODE session — single commit on `workflow-2-competition-scraping`. **No schema; no DB writes; no live run; no deploy.** Closes out the §11.1 route table. ~6 new/modified files: new `src/app/api/projects/[projectId]/competition-scraping/reset/route.ts` (POST — admin reset endpoint per §11.1 + §A.11; type-the-project-name `confirmProjectName` body guard with server-side `Project.name` re-verification; DB transaction deletes the W#2 ProjectWorkflow row [cascades CompetitorUrl + CompetitorSize + CapturedText + CapturedImage] + W#2-originated VocabularyEntry rows where `addedByWorkflow="competition-scraping"` [other workflows' entries on the same Project preserved per §A.11 + `PLATFORM_REQUIREMENTS.md §8.4`]; storage wipe via existing `wipeProjectImages()` helper outside the transaction with janitor backstop on partial failure; admin gate via `verifyProjectAuth` Project-ownership check — `verifyProjectWorkflowAuth` deliberately NOT used since reset shouldn't auto-create the workflow row only to delete it); new `src/app/api/cron/competition-scraping-janitor/route.ts` (GET — daily Vercel cron at 03:00 UTC per §3; auth via `Authorization: Bearer ${CRON_SECRET}` with `process.env.CRON_SECRET` unset → deny-all so the path can't be hit publicly; reads `CapturedImage.storagePath` rows into a Set, walks the bucket via new `listAllStorageImages()`, classifies orphans via the pure `findOrphans()` helper [no DB row + createdAt > 24h = orphan; younger files in 24h grace for in-flight uploads], bulk-deletes via new `deleteStorageImages()`); new top-level `vercel.json` (single `crons` entry — `0 3 * * *`); extended `src/lib/competition-storage.ts` with `listAllStorageImages()` + `deleteStorageImages()` (Supabase Storage SDK calls per `PLATFORM_REQUIREMENTS.md §10.4` portability boundary; widened internal `listAllEntries` return type to expose `created_at` for orphan classification); extended `src/lib/competition-storage-helpers.ts` with pure `findOrphans()` + `ORPHAN_GRACE_MS` constant + `StorageImageEntry` type (11 new unit tests); wired `src/app/projects/[projectId]/competition-scraping/page.tsx` `handleResetConfirm` from throw → `authFetch` POST + `window.location.reload()` on success. **§11.2 cross-cutting items applied uniformly:** `withRetry()` on every Prisma call; `recordFlake()` on every catch (with `retried: true` on the wrapped paths); `verifyProjectAuth` for the reset endpoint; cron uses dedicated `Bearer ${CRON_SECRET}` auth, not the JWT chain. **Reset operation order:** DB first (transactional, atomic via `$transaction`) then storage (outside transaction). Storage failure after DB success surfaces as 500 with explicit "janitor will mop up" message. **Verification scoreboard:** 380/380 src/lib tests pass (was 369 baseline on this branch; +11 new from `findOrphans` tests); tsc clean; build clean (47 routes; was 45 — added `reset` + `cron/competition-scraping-janitor`); lint at exact baseline parity (16e/40w; zero new errors or warnings). **Test count drift note:** prior session-2 entry said "382/382" but actual baseline on this branch is 369; INFORMATIONAL CORRECTIONS_LOG entry captures the discrepancy. **Director-action item before deploy:** add `CRON_SECRET` env var in Vercel (Settings → Environment Variables → CRON_SECRET = a long random string, e.g., `openssl rand -base64 32`); janitor returns 401 if unset, which is the safe failure mode but means the cron silently no-ops until configured. Schema-change-in-flight stays "No" — API + helper-only session, no schema changes. Multi-workflow: pull-rebase clean at session start; no parallel chat. One commit local; push pending Rule 9 approval — push to `workflow-2-competition-scraping` does not deploy vklf.com.) | (a) **Custom React multi-table viewer for content area — RECOMMENDED FIRST.** Workflow's own concern per `PLATFORM_REQUIREMENTS.md §12.6` shared component pattern #2. Platforms → URLs → captured-rows nav + sort/filter + image expand. Builds against the GET `.../urls` (session-1) + sizes/text/image/reconcile endpoints (session-2). Multi-session. Today's placeholder block in the page can be replaced incrementally. / (b) **Detailed User Guide content authoring** per `COMPETITION_SCRAPING_STACK_DECISIONS.md §13.1.2`. Plain-language step-by-step + screenshots + printable PDF. Independent of (a). / (c) **W#2 Chrome extension build** — multi-session (5-7 sessions): WXT init → auth (`signInWithPassword`) → Module 1 (URL capture with floating "+ Add" button) → Module 2 (text + image + region-screenshot) → image upload (signed-URL — extension consumes session-2's `images/requestUpload` + `images/finalize`) → WAL + reconciler (consumes session-2's `reconcile` GET) + offline queue per `PLATFORM_REQUIREMENTS.md §10.1.1` → distribution. Can run in PARALLEL with (a)/(b) — all extension-callable endpoints are now live on this branch (session-1 + session-2 + session-3). / (d) **Live verification of admin reset + janitor** — once `CRON_SECRET` is set in Vercel and W#2 has captured data on a real Project, exercise the reset path end-to-end (typed-name guard happy path + mismatch path + storage wipe verification) and confirm the cron runs at next 03:00 UTC. Janitor logs are grep-able via Vercel's log search. | No |
