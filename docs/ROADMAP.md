...
- 🎯 **Phase 1g-test follow-up REMAINING (post-Session-3b-verify):**
  ...
  3. 🎯 **NEW: Blank-canvas visual verification of canvas-layout engine** — see Phase-1 polish item above. Schedules with Session 4 or standalone quick task.
  4. **Continue with Session 4** (Changes Ledger foundation) per session plan below — OR director may insert "P3-F7 root-cause audit" or "keyword accounting / ghost detection panel" or the blank-canvas visual verification ahead of Session 4.
  5. Add UI hint recommending Direct mode for large-keyword Projects (deferred from kickoff session — still applicable).
  6. Add warning when Adaptive Thinking is selected with a large prompt (0-output-tokens risk — deferred from kickoff session).
