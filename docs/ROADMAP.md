...
**Last updated:** May 3, 2026 (cold-start render-layer fix — option (a) from the 2026-05-02-e standing instructions. **CODE-FIX session — no schema, no DB, no live run, no deploy yet.** New pure helper `src/lib/cold-start-fetch-retry.ts` (~115 lines) + 11 unit tests + KeywordWorkspace rewiring (centralized cold-start retry-state across all three mount-time fetches with banner above topbar — yellow "Retrying load…" / red "Could not load X — Click here to retry") + CanvasPanel slim-down (mount fetch moved up to KeywordWorkspace). "NEW HIGH — Cold-start hard-refresh" entry below flipped from open HIGH to "🟡 CODE FIX SHIPPED 2026-05-03 — pending live verification". Active Tools row updated. 284/284 src/lib tests pass; tsc clean; build clean (17/17 routes); lint at exact baseline parity (16e/41w; zero new — mid-session 3-error trip on React purity rule captured to CORRECTIONS_LOG, fixed via IIFE wrapper). Multi-workflow: schema-change-in-flight stays "No"; W#2 still 🆕 about-to-start; no parallel chat. Commit local; push pending Rule 9 approval.)
...
|---|---|---|---|---|---|
| W#1 Keyword Clustering | 🔄 Active dev — stabilization (tracked toward 🎯 W#1 PRODUCTION-READINESS GATE — D3 RETRY entry below; 3 prerequisites in dependency order: cold-start verification → recency-stickiness fix → underlying flake-rate fix → D3 retry attempt) | `main` | 2026-05-03 cold-start-render-layer-fix (CODE-FIX session — option (a) from the prior STATE block. New pure helper `src/lib/cold-start-fetch-retry.ts` (~115 lines incl. extensive header comment) + 11 unit tests in `src/lib/cold-start-fetch-retry.test.ts` mirroring the `post-rebuild-fetch-retry.ts` pattern (3 attempts, 2s + 5s backoffs, sleep injection, `onAttemptFailed` callback). On exhaust throws plain `Error` with self-explanatory "Could not load X after 3 attempts. Click here to retry. (Underlying error: …)" message — NO partial-apply annotations (deliberate semantic separation from sibling helper). `KeywordWorkspace.tsx` rewired with centralized `coldStartRetry` state (3 states per fetch key: `idle | retrying | exhausted`); three retry-wrapped useCallbacks; three mount useEffects with IIFE wrappers (per CORRECTIONS_LOG 2026-05-03 entry on React purity rule's same-file useCallback tracing); new `renderColdStartBanner()` rendered above the topbar with yellow "Retrying load… (X)" while any fetch is mid-retry, red "Could not load X — Click here to retry" with one button per exhausted fetch on exhaust; click handlers run in React events so synchronous setState is safe. `CanvasPanel.tsx` slim-down: mount-time `fetchCanvas()` useEffect removed (canvas fetch now lives in KeywordWorkspace so all three mount fetches share the centralized retry-state + banner); `fetchCanvas` removed from destructured `canvas` props. **NEW HIGH "Cold-start hard-refresh" ROADMAP entry below flipped from open HIGH → 🟡 CODE FIX SHIPPED 2026-05-03 — pending live verification.** 284/284 src/lib tests pass (was 273; +11 new); tsc clean; build clean (17/17 routes); lint at exact baseline parity (16e/41w; zero new — mid-session 3-error trip on React purity rule when initial wiring synchronously called setState before await; fixed by (a) dropping the synchronous pre-await 'loading' state entirely and (b) IIFE wrapper around the useEffect call; CORRECTIONS_LOG informational entry captures the rule's tracing scope as an operational lesson). Multi-workflow: schema-change-in-flight stays "No"; W#2 still 🆕 about-to-start; no parallel chat. Pull-rebase clean at session start. Commit local; push pending director's discretionary Rule 9 approval — push gates the live verification on production vklf.com.) | (a) **Live-verify the cold-start render-layer fix on production vklf.com — RECOMMENDED.** After push approval + Vercel auto-redeploy, hard-refresh the Bursitis Test 2 workspace 5-10 times. Expected: most refreshes populate normally with no banner; some show the yellow "Retrying load…" banner briefly then populate; rare refreshes show the red "Could not load X — Click here to retry" banner — clicking the retry button should populate the missing data without a full hard refresh. ~10-20 min wall-clock; no AI cost; no DB writes. **Most thorough next step** — same pattern as 2026-05-02-d → 2026-05-02-e sequence; deferring would mean accumulating two unverified fixes. / (b) Underlying flake-rate investigation — measure rate per-endpoint with structured telemetry; investigate Supabase plan tier / pgbouncer pool sizing / Prisma client management; ~2-3 sessions. / (c) Recency-stickiness fix — sister-link deferral + Q5→B touch-semantics. / (d) GoTrueClient multi-instance fix — ~15 LOC. / (e) Phase-1 UI polish bundle. / (f) Action-by-action feedback workflow design. / (g) V3-era cleanup pass (deferred from Session E D4). | No |
...
1. 🟡 **Cold-start render-layer fix verification** — code shipped 2026-05-03 (commit `52db8e3`); pending live verification on vklf.com (next-session task per `KEYWORD_CLUSTERING_ACTIVE.md` POST-2026-05-03 STATE block, item (a)). ~10-20 min hands-on. Closes the "🟡 CODE FIX SHIPPED 2026-05-03 — Cold-start hard-refresh" entry below upon pass.
