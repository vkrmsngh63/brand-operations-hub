...
**Last updated in session:** session_2026-05-04-c_d3-retry-partial-pool-exhaustion-finding (Claude Code)
...
5. ❌ **Pool-exhaustion investigation on Pro tier (NEW 2026-05-04-c after correction — required for D3 to clear the wall on Pro)** — three-step investigation + fix path: (i) Verify Pro tier shows active in Supabase dashboard (5 min — sanity check that the upgrade actually propagated). (ii) Check the connection pool size configuration knob (Pro tier exposes this; default may still be too small for our ~22-parallel-PATCH burst). If at default-small, increase to a value that comfortably covers the burst (e.g., 30-50). 5-10 min config change in the Supabase dashboard. (iii) Run a small batch test (1-3 batches, ~$1-3 cost) to see if the pool wall clears. **If clears → proceed to D3 retry.** **If does NOT clear → pivot to companion code-fix (apply-pipeline parallel-PATCH-burst reduction; was the deferred companion item, now becomes primary).** Estimated 1 session for the investigation + tuning + small-batch test; an additional 1-2 sessions if the code-fix path is needed. (Replaces the 2026-05-04-c original prerequisite #4 framing of "Supabase Pro upgrade" — that's now done; the actual remaining work is the investigation + possible code-fix.)
...
## 🚨 NEW HIGH (2026-05-04-c) — Pool exhaustion under apply-pipeline burst (`FATAL: Max client connections reached`) — fires ON PRO TIER

**Status:** DISCOVERED 2026-05-04-c during D3 retry batch 1. **Correction post-original-commit:** the original 2026-05-04-c entry framed Pro upgrade as the primary fix — but director clarified at end-of-session that Pro upgrade was already done at end of 2026-05-04-b. Tonight's failure was therefore ON Pro tier, not Free. **Pro alone is not sufficient.** Active fix path is now: tune Pro's pool size config knob (5-10 min Supabase dashboard change), AND/OR ship the apply-pipeline parallel-PATCH-burst reduction code-fix (was the deferred "companion" item; now promoted to primary code-side path).

**The problem:** the apply-pipeline fires ~22 parallel PATCH /keywords/[id] requests per batch (one per touched keyword) when applying batch results to the canvas. Each parallel request opens a Prisma connection through Supabase's pgbouncer transaction pooler at port 6543. Supabase Free-tier pool capacity is overwhelmed. Postgres returns `FATAL: Max client connections reached` to every request beyond the limit. The error surfaces as `PrismaClientUnknownRequestError` or `PrismaClientInitializationError` with NO Prisma error code — just the FATAL message embedded in `error.message`. Cascade: ~28 cascading 500s within ~0.1 sec at 8:30:40 PM in the 2026-05-04-c D3 batch 1 (~22 PATCH /keywords/[id] failures + 4 GET /canvas + 2 GET /canvas/nodes failures).
...
**Fix layers (in order — REVISED post-correction):**

1. ✅ **Supabase Pro upgrade — DONE 2026-05-04-b.** Was framed as primary fix in the original 2026-05-04-c entry; corrected after director clarified Pro was already in place. Pro provides higher pool-size limits + configurable pool-size knobs + dedicated pgbouncer compared to Free. **But tonight's failure proves Pro alone is not sufficient** — fix layer (1a) and/or (2) below is now needed.

1a. **Tune Pro tier's pool size config knob (PROBABLE PRIMARY FIX — NEW after correction).** Pro tier exposes a configurable pool size (Free is fixed-small). The default Pro setting may still be too small for our ~22-parallel-PATCH burst. Investigation step: check Supabase dashboard's connection pooling settings; if default is at e.g., 15, increase to 30-50. ~5-10 min Supabase dashboard change; no code; reversible. May fully clear the wall if default is the issue.

2. **Apply-pipeline parallel-PATCH-burst reduction (PRIMARY CODE-SIDE FIX if (1a) doesn't clear the wall — promoted from "deferred companion" status post-correction).** Even with Pro-tier capacity tuned high, ~22 parallel connections per batch is heavy for normal traffic. Three options to investigate:
   - (a) Serialize the parallel PATCHes via a small client-side worker queue (~5-10 LOC change). Lowest-risk; keeps existing endpoint shape; just controls concurrency.
   - (b) Collapse into a single bulk PATCH /keywords endpoint (already accepts arrays — director's setup may already use bulk in some apply paths; needs audit). Medium-risk; one endpoint per batch instead of 22.
   - (c) Atomically batch the keyword updates into the canvas rebuild transaction (heavier refactor; broadest semantic change). Highest-risk; one transaction per batch.

3. **Pre-flight pool-config visibility check (LOW priority addition; reframed post-correction).** Pre-flight runner could surface the active Supabase plan tier AND the configured pool size at run start, with a warning if the pool size looks too small for the expected batch's parallel-PATCH count. Catches the "Pro is active but the knob is at default-small" failure mode that bit us 2026-05-04-c. ~10-20 LOC; nice-to-have once (1a) is verified.
