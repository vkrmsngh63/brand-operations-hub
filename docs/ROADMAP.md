...
| W#2 Competition Scraping & Deep Analysis | 🔄 Active dev — stack decisions FROZEN; ready for build sessions | `workflow-2-competition-scraping` | 2026-05-04 w2-stack-and-architecture (DESIGN-ONLY session — produced `docs/COMPETITION_SCRAPING_STACK_DECISIONS.md` resolving all 13 §A.17 deferred questions; appended §B entry to `docs/COMPETITION_SCRAPING_DESIGN.md` per Rule 18; ran Platform-Truths Audit per Rule 19 with director-approved promotion of sync-reliability pattern to `PLATFORM_REQUIREMENTS.md §10.1.1` NEW; resolved §10.1 deferred non-web-client auth decision in same edit (direct `signInWithPassword`). Stack decisions FROZEN: Q1 WXT framework / Q2 direct signInWithPassword auth / Q3 signed-URL direct upload / Q4 captureVisibleTab + canvas crop / Q5 floating "+ Add" button on hover (director override of recommendation) / Q6 20-color WCAG AA palette / Q7 click "Add Text" in popover (director override) / Q8 10s polling Phase 1 + Realtime Phase 2 + full sync-reliability safety net / Q9 7 W#2 tables + 2 cross-workflow tables / Q10 PLOS routes / Q11 API routes / Q12 monorepo at `extensions/competition-scraping/` / Q13 Phase 1 unpacked + Phase 2 Chrome Web Store Unlisted + Detailed User Guide always-visible. **Schema-change-in-flight stays "Yes"** — covers next implementation session. Cross-doc compliance: only W#2-relevant edits per MULTI_WORKFLOW_PROTOCOL Rule 3 ownership; no W#1 sections touched.) | (a) **Shared Workflow-Tool Scaffold design + build (cross-workflow concern, NOT W#2-only) — RECOMMENDED FIRST per `PLATFORM_REQUIREMENTS.md §12.4`.** Must be built before W#2 PLOS-side build begins. Incorporates `PLATFORM_REQUIREMENTS.md §12.6` extension-points (always-visible deliverables, custom React content components, external-client companion pattern — all surfaced by W#2). Multiple sessions. NOT a W#2-specific track — every workflow #3-#14 inherits the scaffold. / (b) **W#2 PLOS-side build** (after scaffold ships) — multi-session: schema migration via `prisma db push` (this flips schema-change-in-flight from "Yes" → "No" upon push) → API routes per `COMPETITION_SCRAPING_STACK_DECISIONS.md §11` → custom React content component plugged into scaffold per §12.6 #2 → admin reset → image expand viewer → Detailed User Guide content authoring (per §13.1.2) → (Phase 2) admin assignment UI. / (c) **W#2 Chrome extension build** — multi-session: WXT scaffold (`pnpm dlx wxt@latest init` inside `extensions/competition-scraping/`) → auth (`signInWithPassword` + token storage) → Module 1 (URL capture flow with floating "+ Add" button + per-platform DOM modules + URL-recognition badge) → Module 2 (text save flow with popover Add Text button + image save flow + region-screenshot via `chrome.tabs.captureVisibleTab`) → image upload (signed-URL direct flow per §3) → WAL + reconciler + offline queue (per §8.3 / `PLATFORM_REQUIREMENTS.md §10.1.1` NEW sync-reliability requirements) → distribution. Can run in PARALLEL with (b) once schema is locked AND scaffold is done — extension talks to PLOS API routes which are independent of scaffold work. | **Yes — flipped 2026-05-04 at start of W#2 Stack-and-Architecture session per MULTI_WORKFLOW_PROTOCOL Rule 4 (director approved Option A: cover design phase + next implementation session). Flips back to "No" only after the W#2 PLOS-side build session that lands the actual `prisma/schema.prisma` edits + `prisma db push`. While "Yes" any other workflow's session must avoid schema work — capture as deferred per Rule 14e + Rule 26 if it surfaces.** |
