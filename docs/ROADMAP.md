...
**Last updated:** May 4, 2026-c (D3 retry attempt — partial; pool-exhaustion finding — third session of 2026-05-04, follow-up to `session_2026-05-04-b_rate-fix-first-pass`. **DOC + LIVE-RUN session — no code changes** but a SIGNIFICANT FINDING surfaced live on production vklf.com. D3 retry launched after the 2026-05-04-b rate-fix shipped. Batch 1 ran cleanly model + apply-pipeline side ($0.312, 15 ops applied, canvas at 124 nodes, 0 new sister links — quiet confirmation V4 prompt paste was correct). Post-rebuild UI refresh exhausted 3 retry attempts on HTTP 500; the post-rebuild fetch retry helper from 2026-05-02-d paused the run cleanly; Vercel logs revealed root cause: **`FATAL: Max client connections reached`** — Supabase Postgres connection pool exhaustion under the apply-pipeline's parallel keyword PATCH burst (~22 PATCH /keywords/[id] failures within ~0.1 sec at 8:30:40 PM, plus 4 GET /canvas + 2 GET /canvas/nodes failures, all same error). **Different failure mode than the rate-fix targets** — pool exhaustion has no Prisma error code (just the FATAL message); withRetry retries P1001/P1002/P1008/P2034 transient codes; retrying pool exhaustion makes it worse. Director chose Path B at the in-session decision fork: pause D3 here, prioritize Supabase Pro upgrade FIRST (director confirmed 2026-05-04-b on Free tier), retry D3 in fresh session. Open question — `[FLAKE]` line visibility: telemetry from 2026-05-04 should have logged a `[FLAKE]` line per error; director's Vercel paste showed full stack traces but NO `[FLAKE]` lines visible. Investigation deferred. W#1 PRODUCTION-READINESS GATE prerequisite #3 status reframed: covers transient flakes; does NOT cover pool exhaustion. **NEW prerequisite #4 added:** Supabase Pro upgrade for pool-capacity headroom. Active Tools row updated; new HIGH-severity "Pool exhaustion under apply-pipeline burst" entry; new MEDIUM-severity "[FLAKE] visibility puzzle" entry. Multi-workflow: schema-change-in-flight stays "No"; W#2 still 🆕; no parallel chat. Pull-rebase clean. Doc-only commit; push pending Rule 9 approval.)
...
4. ❌ **Supabase Pro upgrade for pool-capacity headroom (NEW 2026-05-04-c — required for D3 to complete reliably)** — discovered live during the 2026-05-04-c D3 retry attempt: batch 1's apply pipeline triggered ~22 parallel PATCH /keywords/[id] requests that overwhelmed Supabase Free-tier connection pool capacity. Postgres returned `FATAL: Max client connections reached` to every request beyond the limit; ~28 cascading 500s within ~0.1 sec at 8:30:40 PM. **Director-action item, not a code change.** Estimated cost $25/month; reversible (downgrade allowed). Pro tier provides higher connection-pool capacity + configurable pool-size knobs (currently fixed-small on Free) + dedicated pgbouncer. Without this upgrade, D3 cannot complete reliably regardless of code-side fixes — pool exhaustion is a CAPACITY problem that retries cannot solve. Director confirmed 2026-05-04-b: NOT currently on Pro. **Why this is gate-blocking:** even with all three code-side prerequisites perfect, the apply pipeline's parallel PATCH burst will continue hitting the pool wall until capacity is increased. Step-by-step upgrade instructions in the 2026-05-04-c session handoff. (NEW item (a) gate condition; was not part of the gate before today.)

**Companion architectural item (deferred to post-Pro investigation):** the apply-pipeline's ~22 parallel PATCH /keywords/[id] burst per batch is heavy even with Pro-tier capacity. Investigate serializing the calls via a small client-side worker queue (~5-10 LOC) OR using the existing bulk PATCH /keywords endpoint OR atomically batching the keyword updates into the canvas rebuild transaction (heavier refactor). Reduces pool pressure linearly. Captured as ROADMAP architectural item — see "🚨 Pool exhaustion under apply-pipeline burst (DISCOVERED 2026-05-04-c)" entry below.
...
## 🚨 NEW HIGH (2026-05-04-c) — Pool exhaustion under apply-pipeline burst (`FATAL: Max client connections reached`)

**Status:** DISCOVERED 2026-05-04-c during D3 retry batch 1; primary fix is the Supabase Pro upgrade (now W#1 PRODUCTION-READINESS GATE prerequisite #4); companion architectural fix deferred to post-Pro investigation.
...
1. **Supabase Pro upgrade (PRIMARY, REQUIRED — NEW prerequisite #4 of the W#1 PRODUCTION-READINESS GATE).** Director-action item; ~$25/month; reversible. Pro provides higher pool-size limits + configurable pool-size knobs (currently fixed-small on Free) + dedicated pgbouncer. Without this, D3 cannot complete reliably regardless of code-side fixes. Step-by-step upgrade instructions in the 2026-05-04-c session handoff.

2. **Apply-pipeline parallel-PATCH-burst reduction (COMPANION, deferred to post-Pro investigation).** Even with Pro-tier capacity, ~22 parallel connections per batch is heavy for normal traffic. Three options to investigate:
...
3. **Pre-flight Pro-tier check (LOW priority addition).** Pre-flight runner could check Supabase plan tier and surface "you're on Free tier; D3-scale runs not recommended" before director kicks off a costly run that will hit the pool wall. ~10-20 LOC; nice-to-have once Pro is in place.
