...
|---|---|---|---|---|---|
| W#1 Keyword Clustering | 🔄 Active dev — stabilization (tracked toward 🎯 W#1 PRODUCTION-READINESS GATE — D3 RETRY entry below; **7 prerequisites** in dependency order: cold-start verification [#1 PARTIAL — happy path confirmed live 2026-05-03-b; banner UI deferred to natural flake event during D3] → recency-stickiness fix [#2 ✅ VERIFIED LIVE 2026-05-05] → underlying flake-rate fix [#3 ✅ VERIFIED LIVE 2026-05-05] → Supabase Pro upgrade [#4 ✅ DONE 2026-05-04-b] → pool-exhaustion mitigation [#5 🟡 pool tune still in place as secondary buffer (size 40)] → #5b parallel-PATCH-burst reduction code-fix [✅ DONE 2026-05-05 — VERIFIED LIVE] → **#6 sister-link consolidation drift cleanup ✅ DONE 2026-05-05-b — Option A (sister links invisible to consolidation model; pending live verification at next D3 resume; applier-side rejection as silent backstop)** | `main` | 2026-05-05-b sister-link-consolidation-drift-cleanup (CODE + DOCS session — no schema, no DB, no live run, no deploy yet; commit local with Rule 9 push pending. Item (a) of prior STATE block executed. **Mid-session pivot from defense-in-depth to Option A (invisibility):** original framing was "drop sister-link ops from consolidation prompt + tighten consolidationMode in applier"; director surfaced "don't think about pink elephants" concern about explicit forbiddance. Option A: consolidation prompt sections 1+2 stripped of all sister-link references (Sister Nodes column dropped from TIER 0 spec; FORBIDDEN list stays as ADD_TOPIC + ADD_KEYWORD only); new `omitSisterNodesColumn?: boolean` flag on `buildOperationsInputTsv` drops the column from TSV input; consolidation call site in `AutoAnalyze.tsx` passes `omitSisterNodesColumn: true` + inline vocabulary fragment stripped of sister-link op names; applier `consolidationMode` rejects 4 ops (ADD_TOPIC + ADD_KEYWORD + ADD_SISTER_LINK + REMOVE_SISTER_LINK) as silent backstop. ~115 LOC across 5 files. 336/336 src/lib tests pass [+9 new]; tsc + build + lint clean at exact baseline parity. Director MUST re-paste updated Consolidation Initial Prompt + Primer before next D3 resume.) | (a) **D3 retry resume from batch 53 onward — RECOMMENDED FIRST.** Checkpoint preserved at canvas 194 / 50 of 257 batches done. Need ~205 more batches at ~$0.45 average ≈ ~$92 in Anthropic credits. Director needs another ~$70 top-up before resuming AND must re-paste updated Consolidation Initial Prompt + Primer into the Auto-Analyze UI Consolidation slots. ~7-10 hours wall-clock; recommend splitting across 2-3 sessions. **Only thing standing between W#1 and graduation.** **(a.1) Option A invisibility verification folded into D3 resume (NEW 2026-05-05-b — GATE-CRITICAL).** Across the ~20 consolidation passes that fire during D3 resume (cadence 10 in the new run; consolidations after batches 10, 20, ..., 200 of the new run): zero ADD_SISTER_LINK / REMOVE_SISTER_LINK ops emitted by model + zero applier rejection events (`is not allowed in consolidation mode (sister links` substring) + sister-link count stays at 82 throughout. Early-warning checkpoint at first new consolidation (after batch 10 of the new run). **Run-numbering note:** director cancelled the paused run at end of 2026-05-05-b session; new D3 resume launches as a fresh run with UI batch numbering restarting at 1 (database state intact; cumulative count = 50 prior + new run's count). Failure modes + recovery paths captured in `KEYWORD_CLUSTERING_ACTIVE.md` POST-2026-05-05-B STATE block item (a.1) + the Continuity note above (a.1). When D3 resume completes with all signals green, GATE prerequisite #6 flips from "✅ DONE — pending live verification" to "✅ VERIFIED LIVE." / (b) `[FLAKE]` visibility investigation — same gap as 2026-05-04-c/d; independent of D3; standalone session. / (c) Phase-3 scaling reconsideration for atomic-batch fold-in (single-tx connection-hold model becomes its own ceiling at canvas 700+ × 50 workers); future architectural work. / (d) Backend integration tests for canvas/rebuild fold-in code paths (no Prisma route-level test harness exists yet); pickup when test harness is set up. / (e) Pre-flight visibility on Resume (carried over). / (f) Canvas-size in workspace top-bar (carried over). / (g) Auto-fire toggle absence investigation (carried over). / (h) Wrap remaining unwrapped routes; ~30-50 LOC (carried over). / (i) GoTrueClient multi-instance fix (~15 LOC) (carried over). / (j) Phase-1 UI polish bundle (carried over). / (k) V3-era cleanup pass (carried over). / (l) **Action-by-action feedback workflow + Prompt Refining button — EXPLICITLY LAST** per director's 2026-05-03-b directive; ~5-7 sessions. | No | (DOC + LIVE-RUN session — no code changes — but a SIGNIFICANT FINDING surfaced live on production vklf.com. D3 retry launched; batch 1 ran cleanly model + apply-pipeline side ($0.312, 15 ops, canvas at 124 nodes, 0 new sister links — quiet confirmation V4 prompt paste was correct). Post-rebuild UI refresh exhausted on 3 consecutive HTTP 500s; the post-rebuild fetch retry helper from 2026-05-02-d paused the run cleanly. Vercel logs revealed `FATAL: Max client connections reached` — Supabase Postgres pool exhaustion under the apply-pipeline's parallel keyword PATCH burst (~22 PATCH /keywords/[id] failures within 0.1 sec at 8:30:40 PM, plus 4 GET /canvas + 2 GET /canvas/nodes failures, all same error). Different failure mode than the 2026-05-04-b rate-fix targets — pool exhaustion has no Prisma error code (just FATAL message); withRetry only retries P1001/P1002/P1008/P2034; retrying pool exhaustion makes it worse. **Pool exhaustion fired ON PRO TIER** (correction post-original-commit: director upgraded at end of 2026-05-04-b; tonight's failure was on Pro, not Free). Implication: Pro alone is not sufficient — pool config needs tuning OR parallel-burst pattern needs code-side reduction. Director chose Path B: pause D3 here. Open question — `[FLAKE]` telemetry visibility: ~28 errors at 8:30:40 with full stack traces but NO `[FLAKE]` lines visible in director's Vercel paste; investigation deferred to next-session item (b). Multi-workflow: schema-change-in-flight stays "No"; no parallel chat. Pull-rebase clean.) | (a) **Pool-exhaustion investigation on Pro tier — RECOMMENDED FIRST (NEW 2026-05-04-c after correction).** Three-step plan: verify Pro shows active in Supabase dashboard (5 min) → check pool size config knob and tune higher if at default (5-10 min; Pro tier exposes this) → small-batch test (1-3 batches, ~$1-3) to see if pool wall clears. If clears → proceed to (a.2). If not → pivot to (d) before (a.2). / (a.2) **D3 retry attempt — GATED BY (a) and/or (d).** Same plan as 2026-05-04-b's recommended path but blocked until pool wall is verified clear. ~$30-50 + 3-5 hr wall-clock. / (b) **`[FLAKE]` visibility investigation** — verify telemetry actually firing on PATCH /keywords/[keywordId] (and other 18 wired routes); three-step check (Vercel expanded view → deployed bundle inspection → targeted dev-server test). Independent of D3; can run as standalone session before/after pool investigation. / (c) Wrap remaining unwrapped routes — `/projects` GET (N+1), `/project-workflows/*` GETs, `/admin-notes/*`, `/user-preferences/*`; ~30-50 LOC. / (d) **Apply-pipeline parallel-PATCH-burst reduction (NEW 2026-05-04-c — promoted to PRIMARY status post-correction).** Was a deferred companion fix; now the primary code-side path if pool tuning in (a) doesn't clear the wall on Pro tier. Investigate serializing the ~22 parallel PATCH /keywords/[id] calls per batch via small worker queue OR using existing bulk PATCH /keywords endpoint OR atomically batching into the canvas rebuild transaction; ~1-2 sessions. / (e) GoTrueClient multi-instance fix (~15 LOC). / (f) Phase-1 UI polish bundle. / (g) V3-era cleanup pass. / (h) **Action-by-action feedback workflow + Prompt Refining button — EXPLICITLY LAST** per director's 2026-05-03-b directive; ~5-7 sessions. | No |
