...
| W#2 Competition Scraping & Deep Analysis | 🔄 Active dev — stack decisions FROZEN; doc-reframe SHIPPED; branch-housekeeping (a.1.i merge of main) DONE 2026-05-06; ready for (a.1.ii) doc sweep + (b) PLOS-side build start | `workflow-2-competition-scraping` | 2026-05-05 w2-doc-reframe (DOC-ONLY session — no schema, no code, no live run, no deploy. Executed Tasks #8 + #9 deferred from `session_2026-05-04_workflow-tool-scaffold-design` on main per Rule 26. Four reframes shipped: (1) `docs/COMPETITION_SCRAPING_DESIGN.md` §A.14 reframed from "Scaffold fit" to "Components library fit" — full table rewritten as library-component imports (`useWorkflowContext()`, `<WorkflowTopbar>`, `<StatusBadge>`, `<DeliverablesArea>` Resources, `<CompanionDownload>`, `<ResetWorkflowButton>` + `<ResetConfirmDialog>`, `<NotReadyBanner>` skipped, `<WorkerCompletionButton>` Phase 1, `<AdminReviewControls>` skipped, `useEmitAuditEvent()` Phase 3) plus reframed three-patterns sub-section + sequencing list; (2) `docs/PLATFORM_REQUIREMENTS.md` §12.6 reframed from "Scaffold extension-points" to "Shared component patterns" — per-pattern body realigned with components-library framing (`<DeliverablesArea>` for #1, content-as-workflow's-own-concern for #2, `<CompanionDownload>` for #3); §12.1-§12.5 left untouched on this branch as main's rewrite will flow in via merge; (3) `docs/ROADMAP.md` Current Active Tools W#2 row Next Session items (a) + (b) reframed — (a) now "Shared Workflow Components Library Phase-1 build" with full Phase-1 component list; (b) PLOS-side build wording aligned with library-imports framing; (4) `docs/COMPETITION_SCRAPING_STACK_DECISIONS.md` four lines reframed (line 18 TOC + line 629 routes table + lines 635-636 cross-cutting items). All edits semantic-content-unchanged + framing-aligned with the components-library architecture per `WORKFLOW_COMPONENTS_LIBRARY_DESIGN.md` on main. Drift surfaced: TaskList came back empty (Tasks #8 + #9 didn't persist across main → workflow-2 branch boundary; user prompt fully specified task content so work was not blocked) — captured as INFORMATIONAL CORRECTIONS_LOG entry. Additional stale-scaffold-wording references on this branch outside today's explicit scope (5 lines in COMPETITION_SCRAPING_DESIGN.md other sections + ~10 lines in ROADMAP non-W#2 sections + §12.1-§12.5 of PLATFORM_REQUIREMENTS.md) deferred to next session as item (a.1) below. **Schema-change-in-flight stays "Yes"** — doc-only session does not change schema state; flag continues to cover next implementation session. Multi-Workflow compliance per Rule 3: only the explicitly-named locations were edited; W#1 / cross-workflow sections of ROADMAP + PLATFORM_REQUIREMENTS §12.1-§12.5 + DOCUMENTATION_ARCHITECTURE / PROJECT_CONTEXT / CHAT_REGISTRY-other-than-this-row were not touched. Pull-rebase clean at session start. Doc-only commit; push pending Rule 9 approval.) | (a.1.ii) **Residual stale-scaffold-wording sweep on W#2-owned sections — IN PROGRESS in current session.** Sweeps the lines that main's `8cbc0c6` did not reach: ~5 remaining lines in `docs/COMPETITION_SCRAPING_DESIGN.md` (line 49 in §A.0; lines 163-165 in §A.7; line 297 in §A.9; line 521 in §A.16; line 556 in §A.18) plus header line 4 of `ROADMAP.md`'s W#2 last-updated note. Per the deferred TaskCreate from `session_2026-05-05_w2-doc-reframe`. / (b) **W#2 PLOS-side build** — multi-session: schema migration via `prisma db push` (this flips schema-change-in-flight from "Yes" → "No" upon push) → API routes per `COMPETITION_SCRAPING_STACK_DECISIONS.md §11` → custom React content component for the multi-table viewer (composed alongside imported library components per `PLATFORM_REQUIREMENTS.md §12.6` shared component pattern #2 — content area is the workflow's own concern) → admin reset (uses `<ResetWorkflowButton>` + `<ResetConfirmDialog>` from the library) → image expand viewer → Detailed User Guide content authoring (per §13.1.2) → (Phase 2) admin assignment UI. Today's session's scope per director: schema migration only + first vertical slice of the page route composing the components library; API routes + content viewer deferred to follow-up sessions. / (c) **W#2 Chrome extension build** — multi-session: WXT scaffold (`pnpm dlx wxt@latest init` inside `extensions/competition-scraping/`) → auth (`signInWithPassword` + token storage) → Module 1 (URL capture flow with floating "+ Add" button + per-platform DOM modules + URL-recognition badge) → Module 2 (text save flow with popover Add Text button + image save flow + region-screenshot via `chrome.tabs.captureVisibleTab`) → image upload (signed-URL direct flow per §3) → WAL + reconciler + offline queue (per §8.3 / `PLATFORM_REQUIREMENTS.md §10.1.1` NEW sync-reliability requirements) → distribution. Can run in PARALLEL with (b) once schema is locked AND Phase-1 library has shipped — extension talks to PLOS API routes which are independent of components-library work. (Note: "WXT scaffold" here is the WXT framework's scaffolding tool for initializing Chrome-extension projects — unrelated to the now-retired Shared Workflow-Tool Scaffold concept.) | **Yes — flipped 2026-05-04 at start of W#2 Stack-and-Architecture session per MULTI_WORKFLOW_PROTOCOL Rule 4 (director approved Option A: cover design phase + next implementation session). Flips back to "No" only after the W#2 PLOS-side build session that lands the actual `prisma/schema.prisma` edits + `prisma db push`. While "Yes" any other workflow's session must avoid schema work — capture as deferred per Rule 14e + Rule 26 if it surfaces.** |
