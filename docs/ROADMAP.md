...
**Last updated:** April 20, 2026 (Phase 1g-test follow-up Part 3 — full 51-batch Bursitis run narrated + analyzed; Mode A qualitative superiority confirmed by director; Mode B can silently overwrite Mode A's better work; batch 52 Mode B "Lost 6" core keywords including "bursa" itself; massive design session covering stability scoring, Changes Ledger, Human-in-Loop mode, Feedback Repository, AI Tool Feedback Protocol, stable topic IDs, scoring algorithm, prompt comprehensiveness, cross-canvas consolidation, cost ledger with manual entry, model registry, bursa/Turkey-city homograph handling, Irrelevant Keywords free-floating topic, Pending Deletion canvas region, salvage-ignored-keywords mechanism, and a multi-session execution plan)
...
- 🎯 **Phase 1g-test follow-up REMAINING (for next session or later):**
  1. **Execute the Session 2 investigation plan** per §Phase 1g-test Part 3 session plan below. Directly query the Bursitis canvas via DATABASE_URL for qualitative analysis. Investigate keyword-left-out bug (per director's Part 3 observation). Investigate Removed Terms display bug. Diagnose canvas layout regression using `keyword_sorting_tool_v18.html` (which director will upload to repo root after 2026-04-20 session).
...
**Session 2 — Investigations (no code commits)** — IMMEDIATE NEXT SESSION:
- Verify DATABASE_URL works in Codespaces env; run first direct canvas query on Bursitis project. Produce qualitative analysis alongside director.
- Investigate P3-F7 keyword-left-out bug: read `AutoAnalyze.tsx` batching code, identify root cause, draft proposed fix for director review.
- Investigate Removed Terms display bug: read `ASTTable.tsx`, trace why removed terms aren't appearing.
- Diagnose P3-F8 canvas layout regression using `keyword_sorting_tool_v18.html` (director uploads to repo root before Session 2). Compare HTML tool's layout code to React `resolveOverlap`.
- Draft proposed prompt changes from `AUTO_ANALYZE_PROMPT_V2_PROPOSED_CHANGES.md` for director review.
- End-of-session commit: diagnosis docs only.

**Session 3 — Tier-1 code fixes (requires Session 2 sign-off on diagnoses):**
- P3-F7 fix: keyword re-queuing + independent post-batch AST verification + status sync fix.
- Removed Terms display fix.
- Canvas layout regression fix.
- Salvage-ignored-keywords mechanism (targeted follow-up prompt per `AUTO_ANALYZE_PROMPT_V2_PROPOSED_CHANGES.md` Change 6).
- Opus 4.7 + Haiku 4.5 added to model dropdown (interim hardcoded additions; full model-registry migration deferred).
- Cost tracker fix for failed-attempt costs (per Q8 design).
