...
| W#2 Competition Scraping & Deep Analysis | 🔄 Active dev — schema live 2026-05-06; first PLOS-side page slice live; **API-routes session-1 SHIPPED 2026-05-07** (read paths + `urls` POST/PATCH/DELETE + vocabulary endpoints; session-2/3 + custom multi-table viewer + Detailed User Guide content authoring + admin reset endpoint wiring + Chrome extension build remain) | `workflow-2-competition-scraping` | 2026-05-07 w2-api-routes-session-1 (CODE session — single commit `4166a4f` on `workflow-2-competition-scraping`. **No schema; no DB; no live run; no deploy.** 7 new files, ~1015 LOC: shared types at `src/lib/shared-types/competition-scraping.ts`; pure CORS helpers at `src/lib/cors.ts` + Next-aware response factories at `src/lib/cors-response.ts` + 11 unit tests at `src/lib/cors.test.ts`; W#2 url collection routes at `src/app/api/projects/[projectId]/competition-scraping/urls/route.ts` (GET + POST + OPTIONS); W#2 url per-row routes at `src/app/api/projects/[projectId]/competition-scraping/urls/[urlId]/route.ts` (PATCH + DELETE + OPTIONS); project-scoped vocabulary routes at `src/app/api/projects/[projectId]/vocabulary/route.ts` (GET + POST + OPTIONS). All routes apply §11.2 cross-cutting items: `verifyProjectWorkflowAuth("competition-scraping")` for urls / `verifyProjectAuth` for vocabulary; `withRetry()` on every Prisma call; `markWorkflowActive()` after url mutations; `recordFlake()` on every catch; `chrome-extension://*` CORS preflight via OPTIONS. POST `.../urls` is idempotent on the `(projectWorkflowId, platform, url)` unique constraint — P2002 collision returns existing row with 200 instead of 500; same create-then-catch-P2002 pattern on POST `/vocabulary`. PATCH `.../urls/[urlId]` surfaces P2025 as 404 + P2002 (re-target collision) as 409. DELETE is idempotent. Vocabulary route does NOT call `markWorkflowActive` — project-scoped per `PLATFORM_REQUIREMENTS.md §8.4`; calling workflow's own routes handle activity stamping. **Verification scoreboard:** 360/360 src/lib tests pass (was 349; +11 new from cors.test.ts); tsc clean; build clean (37 routes; was 34 — added 3 new W#2 API routes); lint at exact baseline parity (16e/40w; zero new). Mid-implementation refactor: initial draft put both pure CORS logic AND NextResponse factories in `cors.ts`; the test runner blew up on `import { NextRequest, NextResponse } from 'next/server'` at module-eval time (Node ESM resolves `next/server` differently than Next.js bundler). Split into `cors.ts` (pure; unit-testable) + `cors-response.ts` (Next-aware) — pattern to remember for any future test-coverage helpers that touch the request/response surface. **Out of scope (deferred to session 2/3):** text endpoints, sizes endpoints, `images:requestUpload` + `:finalize` two-phase flow, `reconcile` GET, admin `reset` endpoint, janitor cron, `competition-storage.ts` helper. Schema-change-in-flight stays "No" — API-only session, no schema changes. Multi-workflow: no parallel chat; pull-rebase clean at session start. One commit local; push pending Rule 9 approval — push to `workflow-2-competition-scraping` does not deploy vklf.com.) | (a) **API routes session-2 — RECOMMENDED FIRST.** Per the recommended split: text endpoints (POST `.../urls/[urlId]/text` + PATCH/DELETE `.../text/[textId]`); sizes endpoints (POST `.../urls/[urlId]/sizes` + PATCH/DELETE `.../sizes/[sizeId]`); image-upload two-phase flow (POST `.../urls/[urlId]/images:requestUpload` returning signed-URL + `:finalize` creating the DB row; PATCH/DELETE `.../images/[imageId]`) per §3 + §11.1; `reconcile` GET per §8.3.2. Introduces the `src/lib/competition-storage.ts` helper wrapper per `PLATFORM_REQUIREMENTS.md §10.4` + the Supabase `competition-scraping` storage bucket. Probable single-session build. / (a.2) **API routes session-3 — admin reset endpoint + janitor cron.** POST `.../competition-scraping/reset` (admin-only; type-the-project-name guard; wipes all W#2 data + storage files for the Project per §A.13) + Vercel daily cron for storage-orphan cleanup per §3. Wires the page's existing `handleResetConfirm` (today throws). ~50-100 LOC. / (b) **Custom React multi-table viewer for content area** — workflow's own concern per `PLATFORM_REQUIREMENTS.md §12.6` shared component pattern #2. Platforms → URLs → captured-rows nav + sort/filter + image expand. Builds against today's GET `.../urls` + (a)'s text/image GETs. Multi-session. Today's placeholder block in the page can be replaced incrementally as endpoints come online. / (c) **Detailed User Guide content authoring** per `COMPETITION_SCRAPING_STACK_DECISIONS.md §13.1.2`. Plain-language step-by-step + screenshots + printable PDF. Independent of (a) + (b). / (d) **W#2 Chrome extension build** — multi-session (5-7 sessions): WXT init → auth (`signInWithPassword`) → Module 1 (URL capture with floating "+ Add" button) → Module 2 (text + image + region-screenshot) → image upload (signed-URL) → WAL + reconciler + offline queue per `PLATFORM_REQUIREMENTS.md §10.1.1` → distribution. Can run in PARALLEL with (a)/(b) since schema is locked + today's URL + vocabulary endpoints are extension-callable already. | No |
