...
| W#2 Competition Scraping & Deep Analysis | 🔄 Active dev — schema live 2026-05-06; first PLOS-side page slice live; **API-routes session-1 SHIPPED 2026-05-07** (read paths + `urls` POST/PATCH/DELETE + vocabulary endpoints); **API-routes session-2 SHIPPED 2026-05-07** (sizes + text + image two-phase upload + reconcile + `competition-storage.ts` helper + Supabase storage bucket created via dashboard); **API-routes session-3 SHIPPED 2026-05-08** (admin reset endpoint + Vercel daily janitor cron + page handleResetConfirm wired); **custom multi-table viewer first slice SHIPPED 2026-05-06** (platforms→URLs sidebar nav + sortable URL list with free-text search + click-row-to-open-in-new-tab + empty/filter-empty/loading/error states; visual-verified live on dev server against a real Project); **URL detail page slice (a.1) SHIPPED 2026-05-07** (chrome + sub-breadcrumb + URL metadata read-only grid + sizes sub-section + sortable captured-text rows table + image-count placeholder + 4 new GET read paths + UrlTable click-row navigation rewired in-app); inline editing of URL fields + per-column filter dropdowns + image expand viewer + Detailed User Guide content authoring + Chrome extension build remain | `workflow-2-competition-scraping` | 2026-05-07 w2-plos-side-viewer-detail-page-slice (CODE session — single code commit `439a240` on `workflow-2-competition-scraping`. **No schema; no DB writes; no live run; no deploy. Visual verification deliberately deferred per director's call** until the Chrome extension provides a data path; pending checks captured in NEW Group B doc `docs/COMPETITION_SCRAPING_VERIFICATION_BACKLOG.md`. 9 new/modified files (1168 insertions, 16 deletions): 4 new GET handlers (`urls/[urlId]`, `urls/[urlId]/text`, `urls/[urlId]/sizes`, `urls/[urlId]/images` — last is a new file; first three extend existing route files); new page at `/projects/[projectId]/competition-scraping/url/[urlId]` + new `UrlDetailContent.tsx` (parallel four-fetch with cancelled-flag race guard, sub-breadcrumb `Competition Scraping › [Platform] › [URL]`, URL metadata read-only grid + customFields, "Open original URL ↗" button preserves prior new-tab affordance, sizes sub-section, sortable captured-text table with `(N)` count, image-count placeholder); `UrlTable` click-row rewired from `window.open(url, '_blank')` to `onRowOpen(urlId)` callback; `CompetitionScrapingViewer` `router.push`es to detail page; shared types extended with 4 new response types (additive). §11.2 cross-cutting items applied uniformly to the 4 GETs. Director-approved scope at session start via Rule 14f `AskUserQuestion` (Option B "A + sizes + image count" picked, recommended). Mid-session pivot: verification checklist surfaced "no way to add data via UI"; director chose "Other — defer all tests, keep a running tally" rather than build a manual-URL-add affordance now. **Verification scoreboard:** 393/393 src/lib tests pass (no test changes); tsc clean; build clean (49 routes; was 47 — +1 page route + 1 API list route); lint at exact baseline parity (16e/40w; zero new errors or warnings). Schema-change-in-flight stays "No". Multi-workflow: pull-rebase clean at session start AND right before doc commit; no parallel chat; W#1 row untouched per Rule 3 ownership. **TaskList sweep:** 10 tasks tracked + 8 completed; 2 `DEFERRED:` tasks closed only after destination doc entries were written this session per Rule 26. One code commit (`439a240`) + one doc-batch commit local; push pending Rule 9 approval — push to `workflow-2-competition-scraping` does not deploy vklf.com.) | (a.2) **Next slice — image expand viewer + image rendering inside URL detail — RECOMMENDED FIRST.** Replaces slice (a.1)'s image-count placeholder with actual rendered thumbnails fed by `GET .../urls/[urlId]/images` (shipped today); click-to-expand modal; signed-URL fetch path via the existing `competition-storage.ts` helper. ~1 session. / (a.3) **Inline editing of URL fields** (Competition Category, Product Name, Brand Name, ratings, custom fields) using existing `PATCH .../urls/[urlId]`. Adds vocabulary picker UX for category/product/brand fields. ~1-2 sessions. / (a.4) **Per-column filter dropdowns** on the URL table to complement the free-text search. ~1 session. / (b) **Detailed User Guide content authoring** per `COMPETITION_SCRAPING_STACK_DECISIONS.md §13.1.2`. Plain-language step-by-step + screenshots + printable PDF. Independent of (a.x). / (c) **W#2 Chrome extension build** — multi-session (5-7 sessions): WXT init → auth (`signInWithPassword`) → Module 1 (URL capture with floating "+ Add" button) → Module 2 (text + image + region-screenshot) → image upload (signed-URL — extension consumes session-2's `images/requestUpload` + `images/finalize`) → WAL + reconciler (consumes session-2's `reconcile` GET) + offline queue per `PLATFORM_REQUIREMENTS.md §10.1.1` → distribution. Can run in PARALLEL with (a.x)/(b) — all extension-callable endpoints are now live on this branch (session-1 + session-2 + session-3 + slice (a.1)'s 4 GETs). / (d) **Live verification of admin reset + janitor** — once `CRON_SECRET` is set in Vercel and W#2 has captured data on a real Project, exercise the reset path end-to-end (typed-name guard happy path + mismatch path + storage wipe verification) and confirm the cron runs at next 03:00 UTC. / (e) **W#2 card-flip release-gate decision** — currently `active: false, route: null` on both PLOS dashboard and Project detail page; flip when (a.1)+(a.2)+(c) are sufficient that a card-click lands somewhere with enough to do. Captured in `COMPETITION_SCRAPING_DESIGN.md §B`. / (f) **Walk through `COMPETITION_SCRAPING_VERIFICATION_BACKLOG.md` slice (a.1) checklist** — the 12 deferred visual checks for slice (a.1) + any additional checks future slices append; runs as a single dedicated session AFTER the extension can populate test data per (c) (or after seed data is otherwise available). | No |
