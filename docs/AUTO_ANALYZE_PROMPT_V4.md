# Auto-Analyze V4 Prompts — Operation-Based Output Contract with Tiered Canvas Serialization
...
**Why a new V4 file (not an in-place edit of V3):** V3 is the historical record of what was canonical through Scale Sessions B/C and the prior small-batch tests. Preserving V3 lets us cite "behaviour X happened on V3 prompts" in any future post-mortem. V4 is the new canonical the director re-pastes after Scale Session D; V3 stays untouched as historical reference until a future cleanup session archives it.

**Predecessor file:** `docs/AUTO_ANALYZE_PROMPT_V3.md` (last canonical 2026-04-26; remains intact as historical reference). Predecessor's predecessor: `docs/AUTO_ANALYZE_PROMPT_V2.md` (still preserved).
...
**Canonical input format:** `src/lib/auto-analyze-v3.ts` `buildOperationsInputTsv(... { serializationMode: 'tiered', tierContext })`. The Primer's INPUT TABLE COLUMNS section below describes what the model sees; if prompt and serializer ever drift, the serializer wins.
...
(3) Topic Splitting → SPLIT_TOPIC operation.
   ...
   CROSS-CANVAS INTENT-EQUIVALENCE DETECTION VIA INTENT FINGERPRINTS. When scanning for intent-equivalence violations, compare against intent fingerprints across the ENTIRE canvas — including topics shown to you only at Tier 1 (their fingerprints are the load-bearing field for cross-canvas detection precisely because the rest of their detail is compressed away). A topic shown at Tier 1 that you don't see at full detail can still be flagged as bundling-the-same-compound-intent as another topic on the canvas. If two distinct topics have intent fingerprints that describe the same searcher in the same situation seeking the same outcome, that's an intent-equivalence violation across topic boundaries — emit a MERGE_TOPICS operation (rather than SPLIT_TOPIC) to consolidate them. If you suspect such a duplicate exists but the Tier 1 row lacks enough detail for a confident merge call, leave it for now (the consolidation pass that runs separately on the full canvas will catch it). Do not invent a violation from a fingerprint similarity alone — fingerprints can describe related-but-distinct intents (e.g., older-women bursitis pain vs. older-men bursitis pain share most words but differ on the binding gender dimension).
...
## 2. Topics Layout Table Primer V4
...
INPUT TABLE STRUCTURE — TIERED CANVAS SERIALIZATION (V4)

Starting in V4, the canvas TSV may arrive in up to three labelled sections. Every topic on the canvas appears in exactly one section, but how much detail you see per topic varies. The tool decides per-topic per-batch which tier to ship based on three signals: whether the topic is in the current batch's relevant subtree, whether the topic has been touched by recent batches' operations, and the topic's stability score.

The three sections, in order:

  === TIER 0 ===
  (full detail — 9 columns; the format you knew from V3)

  === TIER 1 ===
  (compressed summary — 6 columns; the topic's intent fingerprint and a keyword count)

  === TIER 2 ===
  (skeleton — 3 columns; just stable ID + title + parent)

Section delimiter lines (`=== TIER N ===`) appear on their own line; the column header row follows on the next line; data rows follow after that. Empty tiers are omitted entirely (you may see only Tier 0, or Tier 0 + Tier 1, or all three, etc.). On an empty canvas (the very first batch of a fresh project), no tier sections appear — only the Tier 0 column header row, with no data rows.

CRITICAL — every topic in any tier is part of THE SAME CANVAS. Tier 1 / Tier 2 topics are not "less real" or "less placeable" than Tier 0 topics. They are simply being shown to you in compressed form because the tool's heuristic decided you don't need their full detail to make this batch's placement decisions. You can:

- Place new keywords as primary or secondary at any topic regardless of which tier shows it (reference by Stable ID).
- Move keywords TO a Tier 1 / Tier 2 topic (the tool applies it; the topic's full state, including any keywords that were already there, is preserved).
- Restructure (rename, merge, split, move, delete) any topic regardless of which tier shows it. Tier compression is a presentation choice; it does NOT mean the topic is locked.
- Walk the parent chain from any topic to a root topic via Parent Stable ID — present at every tier.

Tier 1 / Tier 2 topics are NOT exempt from the Reevaluation Pass. Trigger (3a) intent-equivalence detection across the entire canvas uses Tier 1 fingerprints — see the Initial Prompt's Reevaluation Pass section for the cross-canvas detection rule.
...
TIER 1 COLUMNS (compressed summary — 6 columns)

Stable ID — Same shape as Tier 0.

Title — Same shape as Tier 0.

Parent Stable ID — Same shape as Tier 0. Use this to walk the hierarchy when you need to reach a root through Tier 1 / Tier 2 ancestors.

Stability Score — Same shape as Tier 0.

Intent Fingerprint — A short canonical phrase, 5–15 words, in searcher-centric language, capturing the topic's compound intent (the audience + situation + goal). Example: *"Older bursitis sufferers seeking gentle, low-cost home relief."* This is essentially a one-line searcher-centric expansion of a good topic title — same voice and granularity. The fingerprint is the load-bearing field at Tier 1: it carries the compressed-away description's intent signal so you can still reason about whether this topic might be intent-equivalent to another topic on the canvas.

Keyword Summary — A short string of the form `'{N} keywords ({P}p + {S}s), top volume kw: "{text}" ({V})'`, where N is the total keyword count, P/S are primary/secondary counts, and the top-volume keyword is the highest-volume primary placement (ties broken alphabetically). An empty topic emits just `'0 keywords'` (no top-volume suffix).

What you do NOT see at Tier 1 (compared to Tier 0): description; conversion path; sister nodes; full keyword list. If you need any of these to make a confident decision about a Tier 1 topic, treat the absence as a signal to NOT restructure that topic this batch — the consolidation pass that runs separately on the full canvas will catch any structural issues you can't confidently judge from the Tier 1 view.


TIER 2 COLUMNS (skeleton — 3 columns)

Stable ID — Same shape as Tier 0.

Title — Same shape as Tier 0.

Parent Stable ID — Same shape as Tier 0. Use this to walk the hierarchy when you need to reach a root through Tier 2 ancestors.

What you do NOT see at Tier 2 (compared to Tier 0): description; relationship; conversion path; stability score; intent fingerprint; sister nodes; keyword list. Tier 2 topics are deeply settled, not recently touched, and not in this batch's relevant subtree — the tool's tier decider judged that you can proceed without their detail. Same guidance as Tier 1: do not restructure a Tier 2 topic this batch unless the case is overwhelming. The consolidation pass handles structural issues on Tier 2 topics on a slower cadence.
...
- Empty cells (e.g., empty Sister Nodes at Tier 0) appear as a single empty string between tab delimiters.
- Each tier section starts with `=== TIER N ===` on its own line, then the tier's column header row, then data rows. Empty tiers are omitted entirely.
- On an empty canvas, you'll see ONLY the Tier 0 column header row and no `=== TIER N ===` delimiter (no rows means no need to label sections). Every keyword's primary placement requires you to ADD_TOPIC the full chain.
- The order of topics within a tier is roughly creation order (sorted by stable ID's numeric suffix). The order of tiers is always Tier 0 → Tier 1 → Tier 2.


HOW TO READ THE TABLE

Each row's Parent Stable ID points back to its parent in the same canvas — possibly in a different tier section. Walking from any topic up its parent chain (via Parent Stable ID) reaches a root topic (one whose Parent Stable ID is empty). The walk is unaffected by which tier shows each ancestor — Parent Stable ID is present at every tier precisely so that the chain is always traversable. Example: a Tier 0 topic's parent might be at Tier 1; that grandparent might be at Tier 2; that great-grandparent (root) might be at Tier 0. The walk follows Stable IDs, not tier sections.

Within a tier, topics are ordered by Stable ID numeric suffix (roughly creation order). The arrangement does NOT correspond to conversion-funnel order — topics in any given tier may belong to many different funnels. Use Parent Stable ID + the topic's title and (when at Tier 0) description to reason about funnel placement.

For example (showing only Tier 0 here for clarity — Tier 1 / Tier 2 topics share the same Stable ID / Parent Stable ID semantics):
...
=== OPERATIONS ===
{"op": "ADD_TOPIC", "id": "$new1", "title": "Bursitis triggers", "description": "Common situations and activities that bring on bursitis pain", "parent": "t-42", "relationship": "nested", "intent_fingerprint": "Bursitis sufferers identifying activities and situations that bring on their pain"}
...
Fields:
  ...
  - intent_fingerprint: REQUIRED. A short canonical phrase, 5–15 words, in searcher-centric language, capturing this topic's compound intent (audience + situation + goal). Example: "Older bursitis sufferers seeking gentle, low-cost home relief." Same voice as the title — basically a one-line searcher-centric expansion. This is the load-bearing field for cross-canvas intent-equivalence detection when the topic later gets compressed to Tier 1.
...
Fields:
  ...
  - intent_fingerprint: REQUIRED. A renamed topic's intent expression has shifted (otherwise why rename?), so refresh the fingerprint to match the new title. Same format as ADD_TOPIC's intent_fingerprint.
...
Fields:
  ...
  - to: new description
  - intent_fingerprint: OPTIONAL. Most description rewrites are pure refinement and the existing fingerprint stays accurate; in those cases omit this field and the existing fingerprint is preserved. Supply a new fingerprint ONLY if the description rewrite has shifted the topic's compound intent (audience + situation + goal) in a way the existing fingerprint no longer captures. Same format as ADD_TOPIC's intent_fingerprint.
...
Fields:
  ...
  - merged_intent_fingerprint: REQUIRED. The intent fingerprint the surviving topic should carry after the merge. The merged topic's intent should be a clean expression of the consolidated compound intent — typically derived from the merged_title with the same searcher-centric voice. Same format as ADD_TOPIC's intent_fingerprint.
  - reason: plain-English audit reason (non-empty)
...
Fields:
  ...
  - into: array of objects, each {id (alias), title, description, intent_fingerprint, keyword_ids (array of keyword UUIDs)} — at least two entries required
    - Each entry's `intent_fingerprint` is REQUIRED — every new topic created by a split needs its own fingerprint, since the split is justified by distinct compound intents being separated. Same format as ADD_TOPIC's intent_fingerprint.
  - reason: plain-English audit reason (non-empty)
