...
**Last updated:** 2026-05-20-b (P-27 captured-videos design session — 2 platform-truths updates per `docs/CAPTURED_VIDEOS_DESIGN.md` §A.14: §10.2 line 427 workflow-deliverable-storage tech-debt note PARTIAL CLOSE for video — the "dedicated bucket strategy = private + signed URLs + per-workflow scoping" pattern is now affirmed in code for video via the new `competition-scraping-videos` bucket spec; new video-storage scale-projections paragraph appended after the existing image-storage paragraph — Phase 3 ~1 TB/year + Phase 4 ~2 TB/year direct-bytes only, with embed-URL references at zero-bytes-stored. NEW Group B design doc `docs/CAPTURED_VIDEOS_DESIGN.md` shipped 2026-05-20-b is the binding source for both updates. Schema-change-in-flight stayed "No" this session — design-only, no code, no schema, no deploy.)
...
- Workflow-deliverable storage (videos, design files, etc.) likely needs a dedicated bucket strategy — possibly private + signed URLs (already on the tech-debt list). **PARTIAL CLOSE 2026-05-20-b for video** per `docs/CAPTURED_VIDEOS_DESIGN.md` §A.9 + §A.14 — W#2 captured-videos feature (P-27) ships a dedicated `competition-scraping-videos` bucket (private + signed URLs only; 100 MB per-file cap; allowlist `video/mp4` + `video/webm` + `video/quicktime`). The "dedicated bucket strategy = private + signed URLs + per-workflow scoping" pattern is now affirmed in code for video; design-files and other workflow deliverables follow the same pattern as those workflows graduate.
...
**Video-storage scale projections (NEW 2026-05-20-b — surfaced by W#2 P-27 captured-videos design session — see `docs/CAPTURED_VIDEOS_DESIGN.md` §A.14):**

W#2 Competition Scraping captures inline `<video>` element bytes (direct-MP4 / direct-WebM uploads) AND stores URL-references for embeds (YouTube / Vimeo / etc. where bytes can't be downloaded per platform ToS). Only direct-bytes uploads consume Supabase Storage; embed-URL references are pointer-only (zero bytes stored). Per-Project volume estimate: ~30 captured videos / Project (mix of inline-bytes ~50 MB average for direct uploads at the 100 MB per-file cap + embeds at 0 MB; assume ~30% direct + ~70% embed mix; ~30 × 0.3 × 50 MB ≈ ~450 MB direct-bytes / Project average). Aggregate direct-bytes projections:

- Phase 3 (~70 Projects/wk × ~450 MB direct-bytes / Project × 52 weeks) ≈ ~1.6 TB/year of video storage (direct-bytes only; embeds are zero-byte URL references). Conservative estimate: ~1 TB/year (assuming lower direct-vs-embed mix).
- Phase 4 (~140 Projects/wk × ~450 MB direct-bytes / Project × 52 weeks) ≈ ~3.3 TB/year of video storage (direct-bytes only). Conservative estimate: ~2 TB/year.

These projections are direct-bytes only — embed-URL references (the majority of captured videos per the design session's per-platform investigation) consume zero Supabase Storage. Add to image-storage projections above for total Supabase Storage footprint per Phase.
