        ...
        if (!autoDetectedSeller) {
          try {
            ...
            if (resp.ok) {
              ...
              // FF#5 2026-05-30 — primary path is the JSON data island
              // ("sellerUserName":"<value>") confirmed reliable on modern
              // eBay listing pages via the 2026-05-30 director-uploaded
              // HTML evidence. DOM /usr/-link fallback preserved for
              // regional / classic-view variants.
              autoDetectedSeller = extractSellerFromListingHtml(html);
              if (!autoDetectedSeller) {
