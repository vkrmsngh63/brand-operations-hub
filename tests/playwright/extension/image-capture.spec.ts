...
// Playwright regression spec deferred). This file is the first slice of
// the P-22 build: ONE platform (walmart, freshest context from manual
// smoke), ONE test (happy path), full Save flow with all 5 API endpoints
// mocked via context.route. Cross-platform parametrization + error-path
// coverage are explicit follow-up slices for future P-22 sessions.
//
// What this test covers end-to-end:
//   1. Extension loads on a route-intercepted walmart product page.
...
const MOCK_HTML_PATH = path.resolve(
  process.cwd(),
  'tests/playwright/extension/walmart-image-product-page.html',
);
...
const PRODUCT_URL =
  'https://www.walmart.com/ip/Cool-Heat-Medicated-Patches/12345';
...
test.describe('P-22 image-capture happy path (walmart)', () => {
  test('right-click image → form renders → fill → Save → 3-phase upload completes; form closes', async ({
    ...
    // ─── Route 1: walmart product page → mock HTML ───────────────────────
    const html = await readFile(MOCK_HTML_PATH, 'utf8');
    await context.route('**://*.walmart.com/**', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    // ─── Route 2: walmart image CDN → fake JPEG bytes ────────────────────
    await context.route('**://*.walmartimages.com/**', async (route) => {
      await route.fulfill({
        status: 200,
    ...
    // ─── Route 3: GET listCompetitorUrls → seeded saved walmart URL ──────
    await context.route(
    ...
    await context.route(
      ...
      async (route, request) => {
        ...
        phase1Body = raw ? (JSON.parse(raw) as Record<string, unknown>) : null;
        const response = {
    ...
    await context.route(
      ...
      async (route, request) => {
        ...
        const raw = request.postData();
        phase3Body = raw ? (JSON.parse(raw) as Record<string, unknown>) : null;
        const row = {
          id: FAKE_IMAGE_ID,
    ...
    await serviceWorker.evaluate(
      ...
      [FAKE_IMAGE_URL, PRODUCT_URL] as const,
    );
    ...
    // URL picker should populate. The form's auto-pre-select via pickInitialUrl
    // depends on walmart canonicalProductUrl matching pageUrl against the
    // saved row's URL — a separate assertion class from the Save flow this
    // spec covers. Pre-select coverage belongs in a dedicated P-15-style
    // spec; here we just verify the picker is populated and explicitly
    // select the saved row before proceeding.
    ...
    expect(phase1Body, 'Phase 1 requestUpload was not called').not.toBeNull();
    ...
    expect(phase2Method, 'Phase 2 PUT was not fired').toBe('PUT');
    expect(phase2ContentType).toBe('image/jpeg');
    expect(phase2BodySize).toBe(FAKE_JPEG_BYTES.length);

    expect(phase3Body, 'Phase 3 finalize was not called').not.toBeNull();
