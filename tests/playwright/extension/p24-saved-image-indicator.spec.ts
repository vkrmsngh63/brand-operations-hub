...
//
// Per-platform variation: PRODUCT_URL + SAVED_IMG_SRC + NON_MATCHING_IMG_SRC
// use the platform's host (e.g. `m.media-amazon.com` for amazon, `i.ebayimg.com`
// for ebay) — see image-capture.spec.ts:54-58 for the per-platform image CDN
// host_permissions table. The exact-string `<img>.src ===
// savedRow.originalSrcUrl` match is the same code path on every platform; this
// extension is defensive against any future per-platform divergence in the
// saved-image-icon scanning loop or per-platform recognition path.
//
...
const PLATFORMS: readonly PlatformCase[] = [
  ...
  {
    platform: 'ebay',
    productUrl: 'https://www.ebay.com/itm/123456001234',
    savedImgSrc: 'https://i.ebayimg.com/images/g/fake/s-l1600.jpg',
    nonMatchingImgSrc: 'https://i.ebayimg.com/images/g/other/s-l1600.jpg',
    pageRouteGlob: '**://*.ebay.com/**',
  },
  {
    platform: 'etsy',
    productUrl:
      'https://www.etsy.com/listing/123456001234/sample-product',
    savedImgSrc:
      'https://i.etsystatic.com/fake/r/il/fake-cool-heat-patches.jpg',
    nonMatchingImgSrc:
      'https://i.etsystatic.com/other/r/il/other-product.jpg',
    pageRouteGlob: '**://*.etsy.com/**',
  },
  {
    platform: 'walmart',
    productUrl:
      'https://www.walmart.com/ip/Sample-Product/123456001234',
    savedImgSrc:
      'https://i5.walmartimages.com/asr/fake-cool-heat-patches.jpg',
    nonMatchingImgSrc:
      'https://i5.walmartimages.com/asr/other-product.jpg',
    pageRouteGlob: '**://*.walmart.com/**',
  },
];

function buildMockPageHtml(savedImgSrc: string, nonMatchingImgSrc: string): string {
  return `<!DOCTYPE html>
<html lang="en">
<head><meta charset="utf-8"><title>P-24 test fixture</title></head>
<body>
  <h1>P-24 saved-image indicator harness</h1>
  <img id="saved" src="${savedImgSrc}" width="200" height="200" alt="saved image fixture">
  <img id="unsaved" src="${nonMatchingImgSrc}" width="200" height="200" alt="unsaved image fixture">
...
for (const pl of PLATFORMS) {
  test.describe(`W#2 P-24 — saved-image indicator — platform=${pl.platform}`, () => {
    test.beforeEach(async ({ context, serviceWorker }) => {
      ...
      // Page fulfillment.
      const mockHtml = buildMockPageHtml(pl.savedImgSrc, pl.nonMatchingImgSrc);
